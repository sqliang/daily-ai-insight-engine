...
tldr: 智谱 Z.ai 发布 GLM 5.2 开源权重模型，支持 100 万 token 上下文窗口，在编程、物理模拟和推理任务上接近付费闭源模型表现，API
  价格约每百万输出 token 4.40 美元。OpenAI 为 Codex 推出 Record & Replay 功能，允许用户录制一次工作流后复用为可编辑的技能。
objective_summary: 2026 年 7 月 21 日前后，Z.ai 通过官网、Hugging Face 和 OpenRouter 发布了 GLM 5.2
  开源权重模型，支持 1M token 上下文窗口，开发者可下载权重、量化、微调或本地运行，已有开发者用 MLX 在双 M3 Ultra Mac Studio 上本地运行该模型。Scaling01
  指出其 API 价格约每百万输出 token 4.40 美元。OpenAI 为 macOS 版 Codex 推出 Record & Replay 功能，允许用户录制一次工作流后保存为可检查和编辑的可复用技能。此外，Cursor
  支持将本地编码代理迁移至云端隔离 VM，HumanLayer 发布了面向工程团队的智能 IDE 与协作平台。
...
entities:
  ...
  - Sen. Mark Warner
key_logic_flow:
- Z.ai 发布了 GLM 5.2 开源权重模型，支持 100 万 token 上下文窗口，开发者可下载权重、量化、微调或本地运行。
- 早期对比测试显示 GLM 5.2 在编程、物理模拟和推理任务上可与更昂贵的闭源模型竞争。
- Scaling01 指出 GLM 5.2 的 API 价格约为每百万输出 token 4.40 美元，远低于许多前沿模型。
- OpenAI 为 macOS 版 Codex 推出了 Record & Replay 功能，允许用户录制一次工作流后保存为可检查和编辑的可复用技能。
- Cursor 推出了将本地编码代理迁移至云端隔离虚拟机的功能，使编码工作可在关闭笔记本后继续运行。
- HumanLayer 发布了面向工程团队的智能 IDE 与协作平台，包含任务管理、版本化制品和人与代理协作功能。
extract_result: success
object_mentions:
- object_type: model
  name: GLM 5.2
  canonical_name: GLM 5.2
  url: https://huggingface.co/THUDM/glm-5.2
  confidence: high
  article_role: primary_subject
  evidence_snippets:
  - Z.ai 发布了 GLM 5.2 开源权重模型，支持 100 万 token 上下文窗口，权重可下载供开发者运行、修改、量化或微调。
  - 早期对比测试显示 GLM 5.2 在编程、物理模拟和推理任务上可与更昂贵的闭源模型竞争。
  - Scaling01 指出 GLM 5.2 的 API 价格约每百万输出 token 4.40 美元，远低于许多前沿旗舰模型。
  article_id: c4eaa063b0e917e2
- object_type: product
  ...
  article_role: primary_subject
  evidence_snippets:
  - OpenAI 为 Codex 推出 Record & Replay 功能，允许用户演示一次工作流后将其保存为可复用的技能。
  - 该功能在 macOS 版 Codex 中可用，录制完成后技能可被检查和编辑，而非不可见的神秘宏。
  article_id: c4eaa063b0e917e2
- object_type: product
  name: Cursor Cloud VM
  canonical_name: Cursor Cloud VM
  url: null
  confidence: medium
...
- object_type: product
  ...
  article_role: mentioned_reference
  evidence_snippets:
  - HumanLayer 为工程团队提供了任务管理、版本化制品和人与代理协作的智能 IDE 与协作平台，小团队免费，Pro 版每用户每月 100 美元。
  article_id: c4eaa063b0e917e2
- object_type: project
  name: pool
  canonical_name: pool
  ...
  article_role: mentioned_reference
  evidence_snippets:
  - pool 为开发者提供了一个支持 ACP 编辑器、斜杠命令、MCP 工具和回退功能的终端与编辑器编码代理，为开源项目。
  article_id: c4eaa063b0e917e2
- object_type: project
  name: ML Intern
  canonical_name: ML Intern
  url: null
  confidence: low
  article_role: mentioned_reference
  evidence_snippets:
  - ML Intern 自动化了训练后研究循环，涵盖论文、数据集、GPU 沙箱训练、评估和迭代，目前未公开定价详情。
  article_id: c4eaa063b0e917e2
- object_type: project
  name: Open Design
  ...
  article_role: mentioned_reference
  evidence_snippets:
  - Open Design 为设计师提供了一个本地优先的画布，支持 BYOK 编辑、设计系统插件和代码交接，为开源项目。
  article_id: c4eaa063b0e917e2
- object_type: project
  name: Lore
  canonical_name: Lore
  ...
  article_role: mentioned_reference
  evidence_snippets:
  - Lore 为二元重型团队提供了一个内容寻址版本控制系统，支持去重和稀疏工作空间加载，为开源项目。
  article_id: c4eaa063b0e917e2
- object_type: product
  name: Retool
  canonical_name: Retool
  url: null
  confidence: medium
  article_role: mentioned_reference
  evidence_snippets:
  - Retool 允许团队在 Claude Code、Codex、Replit、Lovable 或 Retool 中构建内部应用，通过统一运行时进行管理，免费应用托管至
    7 月 1 日。
  article_id: c4eaa063b0e917e2
- object_type: product
  name: LM Studio
  ...
  article_role: mentioned_reference
  evidence_snippets:
  - LM Studio 展示了从四台 Mac Studio 流式传输私有前沿推理到 MacBook 和 iPhone 的功能，未公开定价详情。
  article_id: c4eaa063b0e917e2
- object_type: model
  name: xAI Grok TTS
  canonical_name: xAI Grok TTS
  url: null
  confidence: medium
  article_role: mentioned_reference
  evidence_snippets:
  - xAI 的 Grok TTS 在 Vapi 的盲测语音模型人类化排行榜上以 96 分位居榜首，未公开定价详情。
  article_id: c4eaa063b0e917e2
- object_type: project
  name: Cua
  canonical_name: Cua
  ...
  article_role: mentioned_reference
  evidence_snippets:
  - Cua 运行后台 Linux 电脑使用代理，可通过 CLI 或 MCP 操作桌面应用，为开源项目。
  article_id: c4eaa063b0e917e2
- object_type: product
  name: Hyperagent
  ...
  article_role: mentioned_reference
  evidence_snippets:
  - Hyperagent 能够将视频、仪表盘和每日简报转化为生成的代理演示，目前未公开定价信息。
  article_id: c4eaa063b0e917e2
---
