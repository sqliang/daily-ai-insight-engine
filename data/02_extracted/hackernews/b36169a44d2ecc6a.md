...
tldr: Anthropic 在一篇新研究论文中报告，其模型 Claude 在训练过程中自发形成了一组称为"J-space"的内部神经模式，它充当类似人脑"全局工作空间"的角色，用于有意识的推理和思考。研究者使用雅可比透镜技术发现该模式，并发布了开源代码实现和交互式演示。
objective_summary: Anthropic 的研究者在一篇新发表的研究论文中报告，其语言模型 Claude 在训练过程中自发形成了一组被称为"J-space"的特殊内部神经模式。研究者使用雅可比透镜（Jacobian
  lens）技术发现了这些模式，它们具有可报告性、可按需调节、可用于内部推理、可灵活用于多种任务等特征。实验表明，J-space 对 Claude 的高级认知功能至关重要，但流利说话和语法使用等大部分语言处理并不依赖它。研究者将
  J-space 与神经科学中的全局工作空间理论进行类比，并发布了开源代码实现和 Neuronpedia 交互式演示。
...
entities:
  ...
  key_people: []
key_logic_flow:
- Anthropic 的研究团队在 Claude 中发现了一组称为"J-space"的特殊内部神经模式，这些模式在训练过程中自发形成，而非人为设计。
- J-space 使用雅可比透镜技术发现，该技术通过计算雅可比矩阵找到使模型更可能在将来说出某个单词的内部活动模式。
- 实验证明 Claude 能够报告 J-space 中的内容，且直接编辑 J-space 中的表征可以改变 Claude 的输出结果。
- Claude 能够按要求主动调节 J-space 中的内容，例如在复制句子时于脑中默算数学题，中间步骤会在 J-space 中正确呈现。
- J-space 对 Claude 的高级推理能力至关重要，但流利说话、语法使用等大部分语言处理并不依赖它。
- 研究者将 J-space 与神经科学中的全局工作空间理论进行类比，认为它在 Claude 中充当类似的共享工作空间角色，将信息广播给其他神经网络系统。
...
- object_type: paper
  ...
  evidence_snippets:
  - 文章明确指出这是一篇更详尽研究论文的简短摘要，报告了 Claude 中发现的 J-space 模式及其与全局工作空间理论的关联。
  - 论文展示了 J-space 具有可报告性、可控性、内部推理能力和灵活使用等特性，且对高级认知功能至关重要。
  article_id: b36169a44d2ecc6a
- object_type: project
  name: Global workspace open-source implementation
  canonical_name: Anthropic global workspace code repository
  url: null
  confidence: medium
  article_role: ecosystem_context
  evidence_snippets:
  - 文章明确提到已发布一个包含核心方法开源实现的代码仓库，可供研究社区使用。
  article_id: b36169a44d2ecc6a
- object_type: product
  name: Neuronpedia interactive demo
  canonical_name: Neuronpedia global workspace demo
  url: null
  confidence: medium
  article_role: ecosystem_context
  evidence_snippets:
  - 文章宣布与 Neuronpedia 合作，在开放权重模型上提供该方法的交互式演示以供研究者探索。
  article_id: b36169a44d2ecc6a
---
