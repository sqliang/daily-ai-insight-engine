...
tldr: Alex Kritchevsky 提出"无底对数"概念，将对数视为类似向量的几何对象，通过无底对数之间的比值表示传统有底对数，从而将对数换底公式与向量坐标变换统一为同一数学结构。
objective_summary: Alex Kritchevsky 在其个人博客上发表了关于对数本质的数学论述。他引入"无底对数"概念，即不带底数的对数作为抽象代数对象，传统有底对数可视为两个无底对数的比值。文章通过类比向量代数与微分几何中的坐标变换，论证对数换底公式与向量单位换算在结构上完全一致。文章还指出
  p 进赋值（数论中的质因数指数提取）和复分析中极点阶数的计算均可视为对数在特定基底上的投影操作。
...
entities:
  ...
  - Alex Kritchevsky
key_logic_flow:
- Alex Kritchevsky 提出"无底对数"概念，将其定义为不带底数的抽象对数对象，而非一个具体的数值。
- 传统有底对数可表示为两个无底对数的比值，换底公式因此自然转化为不同"单位"（如 bits 和 nats）之间的换算。
- 作者将对数与向量进行类比：无底对数对应几何向量，底数对应坐标系或单位向量，对数换底对应向量的坐标变换。
- p 进赋值在数论中提取自然数中质因数 p 的指数，本质上是将对数在基底 log p 上的投影操作。
- 复分析中函数的零点/极点阶数计算也可视为对数在基底 log(z-a) 上的投影，与偏导数的投影操作类似。
