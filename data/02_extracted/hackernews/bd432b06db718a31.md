...
tldr: Dan Luu 通过多个案例展示了不同领域中由人为阈值或激励机制导致的"可疑不连续性"现象，包括美国大学 Pell 助学金门槛对低收入学生录取的反效果、俄罗斯选举造假、心理学
  p 值操纵、波兰高考评分畸变、以及体育选拔中的出生月份偏差等，揭示了离散化阈值如何扭曲数据分布与行为选择。
objective_summary: 本文通过分析多个领域的数据分布图，揭示了当存在明确阈值或分数线时，人为激励会引发数据分布中的异常不连续性。具体案例包括：美国大学利用
  Pell 助学金资格作为贫困生录取指标，导致刚好不符合资格的学生录取率骤降；俄罗斯选举中投票率和得票率在整数处出现异常尖峰，暗示数据造假；心理学论文的 p 值在
  0.05 处异常集中；美国《公平量刑法》将可卡因量刑门槛从 50 克提至 280 克后，起诉量在 280 克处出现尖峰；波兰高考语文 30 分及格线导致教师为考生"找分"从而分数堆积在
  30 分附近；以及 UEFA 青年联赛球员的出生月份与入选概率之间存在十倍差异。
...
entities:
  ...
  - Kawai
key_logic_flow:
- 美国大学将 Pell 助学金资格作为录取低收入学生的代理指标后，刚好达不到门槛的学生录取率大幅下降，而刚好达标的高收入家庭学生反而受益。
- 俄罗斯自 2004 年起的选举数据在投票率和得票率的整数（如 95%）处出现异常尖峰，暗示存在人为捏造的选举结果。
- 美国汽车拍卖价格在每 1 万美元边界处出现不连续断层，即使经模型年等因素校正后依然存在。
- 心理学论文的 p 值在 0.05 的显著性阈值下方异常集中，表明存在对 p 值的操纵以达到"统计显著"的标准。
- 美国《公平量刑法》将可卡因持有量刑门槛从 50 克提高到 280 克后，起诉数量在 280 克处出现锐利尖峰，执法行为随法律阈值同步调整。
- UEFA 青年联赛球员的出生月份与入选概率之间存在约十倍的差异，而实际场上价值与出生月份的相关性却很弱，说明早期选拔中存在年龄偏差。
