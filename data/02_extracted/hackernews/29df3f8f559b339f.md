...
tldr: 文章通过 CPU 缓存层级分析和基准测试证明，数据结构布局（Array of Structs 与 Struct of Arrays）对性能影响巨大，SoA
  在大结构体场景下可实现高达 30 倍加速，工作集大小直接决定随机访问延迟落入哪个缓存层级。
objective_summary: Farid Zakaria 在技术博客中详细解释了 CPU 缓存行（64 字节）和硬件内存层级（L1/L2/L3/DRAM）的工作原理，并通过
  Monster 结构体示例演示了两种数据布局方式对性能的影响。在 Array of Structs 布局下，每个 64 字节缓存行只容纳一个 Monster 对象，遍历
  is_alive 字段时浪费了大量缓存空间；而 Struct of Arrays 布局将同一字段连续存储，单个缓存行可容纳 64 个字段值。当 Monster
  结构体大小为 1KiB 时，SoA 相对 AoS 实现了最高 30 倍的性能提升。文章还通过指针追踪基准测试展示了随机访问模式下工作集大小与缓存层级的关系，指出控制对象大小可以将数据保留在更快缓存中。
...
entities:
  ...
  - Jeff Dean
key_logic_flow:
- CPU 缓存行大小为 64 字节，读取单个字节时硬件会自动将相邻 64 字节填充到缓存行中，利用数据的空间局部性和时间局部性。
- 不同缓存层级访问延迟差异巨大：L1d 缓存约 1-2 ns，L2 缓存约 4-5 ns，L3 缓存约 10-15 ns，主存 DRAM 约 60-100 ns。
- Array of Structs 布局下每个缓存行只包含一个 Monster 对象，遍历 is_alive 字段时大部分缓存数据（如 name、id 等）被浪费。
- Struct of Arrays 布局将同一字段连续存储，单个 64 字节缓存行可容纳 64 个 is_alive 值，缓存利用效率大幅提升。
- 在 Monster 结构体为 1KiB 的场景下，SoA 相对 AoS 实现了最高 30 倍的性能提升，结构体越小则差异越小。
- 随机访问模式下 CPU 预取器无法预测地址，工作集大小直接决定数据驻留的缓存层级，更大的结构体使数据更早落入慢速缓存。
