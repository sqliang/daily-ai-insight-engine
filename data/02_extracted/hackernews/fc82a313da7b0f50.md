...
tldr: 本文通过交互式3D演示，详细解析了机械手表机芯（movement）的工作原理，涵盖主发条储能、齿轮系增速和擒纵机构控速三大核心系统的设计与协作。
objective_summary: 作者 Bartosz Ciechanowski 于 2022 年在 ciechanow.ski 上发表了一篇关于机械手表的技术科普文章。文章使用可交互的
  3D 演示，从零开始逐步构建机械手表机芯，解释了主发条如何通过条盒储存能量、齿轮系（going train）如何将条盒的缓慢转动逐级放大至秒针的高速旋转（约 343:1
  的总传动比）、以及由擒纵轮和擒纵叉组成的擒纵机构如何精确控制能量释放速率。文章还介绍了游丝摆轮振荡系统、手动与自动上链机制等进阶部件。
...
entities:
  ...
  key_people: []
key_logic_flow:
- 机械手表完全依靠弹簧（主发条）储存机械能，无需电池或电子元件即可运行。
- 主发条被密封在条盒内，通过旋转条轴（arbor）上链，S 形设计用于平衡内外圈张力。
- 发条释放的能量驱动条盒旋转，再通过由四组齿轮组成的轮系（going train）逐级放大转速，使秒针在一圈发条储能下完成约 2400 次旋转。
- 齿轮系中使用的是钟表专用的摆线齿形（cycloidal profile），以及大小齿轮（wheel 与 pinion）配对的方式以实现紧凑结构。
- 擒纵机构由擒纵轮和擒纵叉组成，负责将连续旋转转换为间歇步进运动，从而精确控制能量释放的节奏。
- 游丝摆轮系统作为振荡器，通过其固有的谐振频率决定了擒纵机构释放能量的时间间隔，是整个手表的计时基准。
