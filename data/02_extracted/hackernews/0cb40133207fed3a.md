...
tldr: 本文对比 Costco 与 Amazon 两种零售模式：Costco 以 4000 个 SKU、整托盘交叉转运和到店自提实现超低运营成本（SG&A 占销售额
  10%），而 Amazon 以无限商品选择和复杂配送网络为代价承担高昂配送成本（占非 AWS 销售额 40%）。文章主张有限选择和简洁物流比无限商品和到家配送更具社会可推广性。
objective_summary: 文章从物流效率和社会成本两个维度对比 Costco 和 Amazon 的零售模式。Costco 仅维持约 4000 个 SKU，通过整托盘交叉转运和会员制实现负现金转换周期，其
  SG&A 费用仅占销售额 10%，员工平均时薪 21.29 美元且年 turnover 仅 6%。Amazon 提供超 3 亿种商品和两日达配送，配送成本占非
  AWS 销售额 40%，仓库员工 turnover 高达 150%。文章认为 Costco 的简洁运营模式在物流简单性、员工待遇和社会成本控制方面优于 Amazon
  的复杂配送体系。
...
entities:
  ...
  - Jim Sinegal
key_logic_flow:
- Costco 门店仅维持约 4000 个 SKU，远少于 Walmart 平均 13 万个 SKU，通过有限选择降低消费者决策焦虑。
- 低 SKU 数使 Costco 采购团队能深入调研每个产品，与供应商建立更紧密关系，从而以更低价格采购高质量商品。
- Costco 的 SG&A 费用仅占销售额 10%，远低于 Amazon 非 AWS 业务 40% 的配送成本，低 overhead 使其能支付更高工资。
- Costco 配送中心采用整托盘交叉转运模式，无需自动化分拣系统，物流运营极其简洁。
- Costco 员工平均时薪 21.29 美元，年 turnover 仅 6%，远低于 Amazon 仓库 150% 和零售业平均 60% 的 turnover
  率。
- 文章主张到店自提模式对基础设施压力小、社会成本低，比到家配送模式更具普遍可推广性。
