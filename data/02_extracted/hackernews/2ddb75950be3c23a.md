...
tldr: 小米发布并开源了 MiMo Code 代码编程工具。
objective_summary: 小米发布了 MiMo Code 并选择开源。文章以此为契机，讨论了科技公司开源策略的经济学动机（商品化互补品理论），以及 Claude
  Code 闭源、Gemini CLI 停止开发等业界对比。
...
entities:
  ...
  - Joel Spolsky
key_logic_flow:
- 小米发布了 MiMo Code 编程工具并将其开源，允许任何人使用和查看其代码。
- 文章以 Joel Spolsky 的"商品化互补品"理论解释科技公司开源的商业动机：通过在一个层面制造竞争来巩固另一层面的垄断地位。
- Anthropic 的 Claude Code 保持闭源，但通过低价捆绑订阅策略获取用户数据并建立事实标准。
- Google 的 Gemini CLI 被弃用并转向闭源的 Antigravity CLI，与开源趋势形成对比。
- 中国通过低价甚至免费提供 AI 产品，复用在太阳能行业使用的长期竞争策略以挤压竞争对手。
- 构建高质量编程工具链（coding harness）工程难度大，尤其在小模型场景下需要精细的上下文管理和工具调用设计。
---
