...
tldr: RFC 10008 定义了 HTTP QUERY 方法，这是一种安全且幂等的请求方式，允许客户端通过请求体向服务器提交查询内容并获取处理结果，填补了 GET
  和 POST 在复杂查询场景之间的空白。
objective_summary: IETF 于 2026 年发布 RFC 10008（Internet Standards Track 文档），由 J. Reschke、J.M.
  Snell 和 M. Bishop 共同编制，正式定义了 HTTP QUERY 方法。该方法允许客户端向目标资源发送包含查询内容的请求体，由服务器处理并返回结果，兼具
  GET 的安全幂等性和 POST 的请求体传参能力。规范还定义了等效资源 URI 分配机制、响应缓存策略、重定向处理规则以及 Accept-Query 响应头字段等配套机制。
...
entities:
  ...
  - M. Bishop
key_logic_flow:
- RFC 10008 定义了新的 HTTP QUERY 请求方法，其输入通过请求体（content）而非 URI 查询参数传递，解决了 GET 方法受 URI 长度限制的问题。
- QUERY 方法既安全又幂等，不改变目标资源状态，因此可以安全地自动重试或重复执行，消除了 POST 方法在查询场景中的语义歧义。
- 服务器可以为 QUERY 请求的等效资源分配 URI，通过 Location 响应头返回，客户端之后可改用 GET 请求获取相同结果，无需重新提交查询内容。
- QUERY 响应是可缓存的，缓存键必须包含请求内容及相关元数据，但缓存处理比 GET 更复杂，因为需要完整读取请求内容才能确定缓存键。
- Accept-Query 响应头字段允许服务器声明对 QUERY 方法的支持以及可接受的查询格式媒体类型，采用 Structured Fields 语法表达。
- QUERY 方法支持条件请求和重定向，301/308 表示永久重定向，303 表示客户端应改用 GET 请求到新 URI 获取查询结果。
extract_result: success
object_mentions:
- object_type: paper
  name: RFC 10008
  canonical_name: RFC 10008
  url: https://www.rfc-editor.org/info/rfc10008/
  confidence: high
  article_role: primary_subject
  evidence_snippets:
  - RFC 10008 是 IETF 发布的 Internet Standards Track 正式标准文档，由 J. Reschke、J.M. Snell 和
    M. Bishop 共同编制，已获得 IESG 批准发布。
  - 该规范完整定义了 HTTP QUERY 方法的安全与幂等性语义、请求体传参机制、等效资源 URI 分配、响应缓存策略以及 Accept-Query 头字段等核心内容。
  article_id: b58e940125d9dc15
---
