...
tldr: 该硕士论文提出了利用 Kolmogorov-Arnold Networks (KAN) 在 FPGA 上实现超快速机器学习推理和在线学习的方法。相关成果以两篇论文形式分别发表于
  FPGA 2026（获最佳论文奖）和 ICML 2026，实现了 2700 倍于此前 KAN-FPGA 实现的加速比。
objective_summary: 这篇文章介绍了一项硕士论文工作，设计基于 Kolmogorov-Arnold Networks (KAN) 的 FPGA 硬件架构，用于超快速推理和在线学习。作者利用
  KAN 求和单变量激活函数的特性，将每个激活函数实现为独立的查找表 (LUT)，避免了传统 LUT 神经网络中条目数随输入维度指数增长的问题。该方法在 FPGA
  2026 会议上获得最佳论文奖，性能超越现有最先进的 FPGA 神经网络加速器，相比此前 KAN-FPGA 方案实现了 2700 倍加速。此外，该工作还扩展至
  FPGA 上的实时在线学习，在 FPGA 内部同时运行前向传播、反向传播和梯度更新，实现了亚微秒时间尺度的模型参数更新。
...
entities:
  ...
  - Philip C. Harris
key_logic_flow:
- FPGA 通过查找表 (LUT) 实现自定义数字逻辑电路，适合需要亚微秒延迟和高硬件效率的机器学习推理场景。
- Kolmogorov-Arnold Networks (KAN) 将可训练的单变量函数放在每条边上而非使用固定激活函数，求和节点仅为简单加法，避免了 LUT 条目数随输入维度指数增长。
- 作者在软件中预训练 KAN 模型后将每个激活函数转换为独立 LUT 部署到 FPGA，实现了推理加速器。
- 该方法在 FPGA 2026 获得最佳论文奖，性能超越现有最先进的 FPGA 神经网络加速器，相比此前 KAN-FPGA 实现获得了 2700 倍加速。
- 该工作进一步将在线学习扩展到 FPGA 上，在 FPGA 内部同时运行前向传播、反向传播和梯度更新，实现亚微秒时间尺度的实时模型更新。
extract_result: success
object_mentions:
- object_type: paper
  name: 'KANELÉ: Kolmogorov–Arnold Networks for Efficient LUT-based Evaluation'
  canonical_name: KANELÉ
  url: https://dx.doi.org/10.1145/3748173.3779202
  confidence: high
  article_role: primary_subject
  evidence_snippets:
  - KANELÉ 论文提出了使用 KAN 作为构建可训练 LUT 神经网络的核心方法，并获得了 FPGA 2026 最佳论文奖。
  - 该方案利用 KAN 求和单变量激活函数的特性，避免了 LUT 条目数随输入维度指数增长的问题。
  - 该工作在性能上超越了现有最先进的 FPGA 神经网络加速器，实现了 2700 倍于此前 KAN-FPGA 实现的加速比。
  article_id: fc7c7b6c085b4c8b
- object_type: paper
  name: Ultrafast on-FPGA Online Learning via Spline Locality in Kolmogorov-Arnold
    Networks
  canonical_name: Ultrafast on-FPGA Online Learning via Spline Locality in Kolmogorov-Arnold
    Networks
  url: https://arxiv.org/abs/2602.02056
  confidence: high
  article_role: primary_subject
  evidence_snippets:
  - 该论文展示了将基于 LUT 的 KAN 推理架构扩展至 FPGA 上的实时在线学习，实现亚微秒时间尺度的模型更新。
  - 与仅运行前向推理的传统方案不同，该论文在 FPGA 内部同时实现了前向传播、反向传播和梯度更新的完整电路。
  - 论文在 FPGA 上存储 B 样条基函数而非已学习的激活函数，使得模型参数可以在推理过程中实时更新。
  article_id: fc7c7b6c085b4c8b
---
