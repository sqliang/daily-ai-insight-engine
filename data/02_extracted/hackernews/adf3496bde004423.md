...
tldr: 用Broadcom芯片的10Gtek模块替换过热的MikroTik Marvell SFP+模块，网络恢复稳定
objective_summary: 作者的家庭10GbE网络因MikroTik S+RJ10模块（Marvell芯片）过热至95°C导致反复断连，更换为Broadcom
  BCM84891芯片的10Gtek ASF-10G-T80模块后，交换机CPU温度下降约5°C，两周内运行稳定。
...
entities:
  ...
  key_people: []
key_logic_flow:
- 作者家庭网络使用MikroTik S+RJ10模块（Marvell芯片）实现10GBASE-T连接，模块长期运行在93°C高温下。
- 天气转暖后模块温度升至约95°C触发过热保护，导致网络频繁断开重连（flapping），开空调才能维持运行。
- 经研究确认存在两代10GBASE-T SFP+模块：老款Marvell芯片功耗高，新款Broadcom BCM84891芯片功耗更低。
- 作者购买了Broadcom芯片的10Gtek ASF-10G-T80-INT模块替换原有MikroTik模块，该模块伪装为Intel光纤模块。
- 新模块安装后不再通过SNMP报告温度数据，因此无法直接监控其运行温度。
- 更换后两周内网络连接稳定无中断，且交换机CPU温度降低了约5°C。
