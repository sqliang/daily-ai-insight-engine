...
tldr: Flipper 公布开源 Linux 硬件平台 Flipper One，与 Collabora 合作推进 RK3576 主线内核支持，
objective_summary: Flipper 团队在官方博客公开了历时多年研发的 Flipper One 项目。该设备是基于 RK3576 SoC 和 RP2350
  MCU 双处理器架构的开放式 Linux 硬件平台，支持 PCIe/USB 3.0/SATA 扩展及 5G 网络。
...
entities:
  ...
  key_people: []
key_logic_flow:
- Flipper 宣布 Flipper One 项目，这不是 Flipper Zero 的升级版，而是一个全新的开放式 Linux 硬件平台，定位为网络层 IP
  连接设备。
- 该设备采用双处理器架构：RK3576（8 核 CPU + Mali-G52 GPU + NPU）运行 Linux，RP2350 MCU 独立控制显示/按键/电源，即使主
  CPU 关机也能操作。
- Flipper 与 Collabora 合作将 Rockchip RK3576 SoC 完全支持推入主线 Linux 内核，目标是无二进制 blob、无闭源驱动、无厂商锁定的
  BSP。
- 团队正在构建 Flipper OS——基于 Debian 的系统层，引入配置文件快照（profiles）机制，允许用户在不同预配置环境间自由切换。
- 同时开发 FlipCTL 框架，为小屏幕 Linux 设备提供菜单式界面，将 ping/nmap 等 CLI 工具封装为 D-pad 可导航的 UI。
- Flipper 公开了开发者门户 wiki，任何人都可以编辑和参与硬件、机械、Linux 软件、MCU 固件、UI、文档、测试等子项目的开发。
