...
tldr: 首位图灵奖得主艾伦·佩利（Alan Perlis）于1982年发表117条编程箴言，涵盖编程语言设计、软件工程、系统架构和计算哲学等核心议题，至今仍是计算机领域引用最广的格言集之一。
objective_summary: 艾伦·佩利（Alan Perlis），耶鲁大学计算机科学教授、首位图灵奖获得者，于1982年通过耶鲁大学个人页面发布了117条关于编程与计算机科学的格言集。这些箴言以精炼语句总结了编程语言设计（涉及FORTRAN、LISP、ALGOL、SNOBOL、APL等语言）、软件工程实践、系统构建方法论、人机关系以及计算哲学等多个维度的深层洞见，至今仍在软件开发者社区中被广泛引用和讨论。
...
entities:
  ...
  - Alan Perlis
key_logic_flow:
- 佩利主张函数应延迟绑定，数据结构则强制绑定，因此数据结构应在编程过程后期进行设计，而非过早结构化数据。
- 佩利认为一个能影响程序员思考方式的语言才值得学习，否则不具备掌握的价值。
- 佩利强调简洁并非先于复杂，而是紧随复杂之后出现，真正的简洁是对复杂充分理解后的产物。
- 佩利指出度量程序员的标准应是案例分析（case analysis）的完整性，而非聪明才智或逻辑推理能力。
- 佩利认为计算的真正目标是模拟人类的综合创造能力，而非理解人类的分析能力。
- 佩利警告永远无法通过形式手段从非形式走向形式，暗示形式化方法存在固有局限。
