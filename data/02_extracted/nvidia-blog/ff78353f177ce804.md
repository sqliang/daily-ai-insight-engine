...
tldr: NVIDIA Blackwell 在 MLPerf Training 6.0 全部七项基准测试中取得最快训练时间
objective_summary: NVIDIA 在 MLPerf Training 6.0 全部七项基准测试中取得最快训练时间。GB300 NVL72 相比 GB200
  NVL72 性能提升达 1.6 倍，最大规模提交使用 8,192 GPU 完成 DeepSeek-V3 671B 训练。
...
entities:
  ...
  key_people: []
key_logic_flow:
- MLPerf Training 6.0 新增 DeepSeek-V3 671B 和 GPT-OSS-20B 两个 MoE 预训练基准，NVIDIA 是唯一在所有七个基准测试中都有提交的平台，并取得全部最快训练时间。
- GB300 NVL72 相比 GB200 NVL72 在相同规模下训练速度最高提升 1.6 倍，核心改进包括 NVFP4 高计算密度、扩展内存容量和更高功耗上限。
- NVIDIA 使用 GB200 NVL72 系统将 DeepSeek-V3 671B 扩展到 8,192 GPU，这是 MLPerf Training 中基于
  Blackwell 的最大规模提交。
- Microsoft Azure 在 8,192 GPU 规模下以 7.07 分钟完成 Llama 3.1 405B 训练，CoreWeave 使用 GB300
  NVL72 加 Spectrum-X 以 2.02 分钟完成 DeepSeek-V3 671B 训练。
- NVIDIA 通过 30 道以上制造测试、RAS 引擎自愈能力和 NVRx 弹性扩展实现大规模训练可靠性，Spectrum-X 可在毫秒级绕过故障链路。
- CoreWeave 上 Cohere 的 North 平台训练速度提升 3 倍，Midjourney v8 模型在 Blackwell 集群训练完成，Thinking
  Machines Lab 在 Google Cloud 上获得 2 倍加速，Higgsfield 在 Nebius 上训练时间减少 30%。
