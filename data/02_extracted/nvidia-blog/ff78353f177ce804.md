...
tldr: NVIDIA Blackwell 平台在 MLPerf Training 6.0 全部七项基准测试中均取得最快训练时间，GB300 NVL72 相比 GB200
  性能提升最高达 1.6 倍，并在 DeepSeek-V3 671B 上扩展至 8,192 块 GPU 的最大规模提交。
objective_summary: NVIDIA 在 MLPerf Training 6.0 行业基准测试中提交了 Blackwell 平台（GB200 NVL72
  和 GB300 NVL72）在全部七项基准上的结果，均取得最快训练时间。GB300 NVL72 凭借 NVFP4 精度、更大内存和更高功耗上限实现最高 1.6
  倍性能提升。NVIDIA 在 DeepSeek-V3 671B 上扩展至 8,192 块 GPU，为本次最大规模 Blackwell 提交。CoreWeave
  和 Microsoft Azure 分别在 DeepSeek-V3 和 Llama 3.1 405B 上以 8,192 GPU 规模取得最快成绩。生态系统合作伙伴如
  Cohere、Thinking Machines Lab 和 Higgsfield 均报告了显著加速效果。
...
entities:
  ...
  key_people: []
key_logic_flow:
- NVIDIA Blackwell 平台在 MLPerf Training 6.0 的全部七项基准测试中均取得最快训练时间，是唯一提交所有基准的厂商。
- GB300 NVL72 相比 GB200 NVL72 实现最高 1.6 倍训练性能提升，关键驱动因素包括 NVFP4 精度、更大内存容量和更高功耗上限。
- NVIDIA 在 DeepSeek-V3 671B（本次最大 MoE 模型）上扩展至 8,192 块 GPU，创下 MLPerf Training 中最大规模
  Blackwell 集群提交纪录。
- Microsoft Azure 以 8,192 块 GPU 在 Llama 3.1 405B 上取得 7.07 分钟的最快训练成绩；CoreWeave 以 8,192
  块 GB300 NVL72 在 DeepSeek-V3 671B 上取得 2.02 分钟的最快成绩。
- NVIDIA 平台从制造检测（30 余道测试工序）、芯片自愈能力和网络级毫秒级故障切换三个维度构建生产可靠性，NVRx 可在节点中断时从最近检查点恢复训练。
- CoreWeave 上 Cohere 的 North 平台获得 3 倍训练加速，Google Cloud 上 Thinking Machines Lab 获得
  2 倍加速，Nebius 上 Higgsfield 训练时间减少 30%。
extract_result: success
object_mentions:
- object_type: product
  name: NVIDIA Blackwell platform
  canonical_name: NVIDIA Blackwell
  url: null
  confidence: high
  article_role: primary_subject
  evidence_snippets:
  - NVIDIA Blackwell 平台在 MLPerf Training 6.0 的全部七项基准测试中均取得最快训练时间，是唯一提交了所有基准的平台。
  - NVIDIA Blackwell 平台同时提供 NVIDIA Quantum InfiniBand 和 Spectrum-X Ethernet 两种规模化组网方案，支持数据中心灵活构建集群。
  article_id: ff78353f177ce804
- object_type: product
  ...
  article_role: primary_subject
  evidence_snippets:
  - GB300 NVL72 在本轮测试中相比 GB200 NVL72 实现了最高 1.6 倍的训练性能提升。
  - CoreWeave 使用 8,192 块 GB300 NVL72 系统在 DeepSeek-V3 671B 上取得 2.02 分钟的最快训练成绩。
  article_id: ff78353f177ce804
- object_type: product
  name: NVIDIA GB200 NVL72
  canonical_name: NVIDIA GB200 NVL72
  url: null
  confidence: high
  article_role: primary_subject
  evidence_snippets:
  - Microsoft Azure 使用 8,192 块 GB200 NVL72 系统在 Llama 3.1 405B 上以 7.07 分钟取得最快训练成绩。
  - Cohere 在 GB200 NVL72 上为其 North 代理 AI 平台实现了 3 倍训练加速。
  article_id: ff78353f177ce804
- object_type: project
  name: MLPerf Training 6.0
  canonical_name: MLPerf Training 6.0
  url: null
  confidence: high
  article_role: mentioned_reference
  evidence_snippets:
  - MLPerf Training 6.0 新增了 DeepSeek-V3 671B 和 GPT-OSS-20B 两个混合专家（MoE）预训练基准测试，反映了
    MoE 架构日益增长的中心地位。
  - NVIDIA 是唯一在 MLPerf Training 6.0 全部七个基准测试中均提交了结果的平台厂商。
  article_id: ff78353f177ce804
- object_type: model
  ...
  article_role: mentioned_reference
  evidence_snippets:
  - DeepSeek-V3 671B 是 MLPerf Training 6.0 套件中最大的 MoE 模型，NVIDIA 将其训练扩展至 8,192 块 GPU。
  - CoreWeave 使用 GB300 NVL72 系统以 8,192 GPU 规模在 DeepSeek-V3 671B 上取得 2.02 分钟的最快成绩。
  article_id: ff78353f177ce804
- object_type: product
  name: NVIDIA Resiliency Extension (NVRx)
  canonical_name: NVIDIA NVRx
  url: null
  confidence: medium
  article_role: ecosystem_context
  evidence_snippets:
  - NVRx 可在发生故障时从最近检查点恢复训练，而不是重新启动作业，最大限度减少中断带来的时间损失。
  - NVRx 自动检测并管理性能低于标准的节点，防止其拖慢整个集群的运行速度。
  article_id: ff78353f177ce804
- object_type: product
  name: Cohere North
  canonical_name: Cohere North
  url: null
  confidence: medium
  article_role: mentioned_reference
  evidence_snippets:
...
- object_type: model
  ...
  article_role: mentioned_reference
  evidence_snippets:
  - NVIDIA 使用 NVFP4 对 5500 亿参数的 Nemotron 3 Ultra 模型进行了预训练，展示了低精度训练创新。
  article_id: ff78353f177ce804
---
