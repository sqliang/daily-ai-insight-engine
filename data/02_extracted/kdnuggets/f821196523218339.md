...
tldr: 本文介绍如何用 Python、DuckDB、Parquet、Redis 和 FastAPI 从零构建一个最小化特征存储系统，涵盖特征注册表、离线存储、在线存储、物化管道和检索
  API 五个核心组件，并展示如何将其用于 LLM 个性化推荐的实时上下文注入。
objective_summary: 作者在 KDNuggets 上发表技术教程，系统性地拆解了特征存储的五个核心组件：特征注册表（用 dataclass 定义特征的契约）、基于
  Parquet 和 DuckDB 的离线存储（用于训练和回填）、基于 Redis 的在线存储（用于亚 10 毫秒推理时查找）、离线到在线的物化管道，以及基于 FastAPI
  的类型化检索 API。文章以流媒体服务的 LLM 个性化推荐为示例，说明特征存储如何同时解决经典的训练-服务特征偏差问题，以及 LLM 智能体和 RAG 管线的实时结构化上下文注入需求。
...
entities:
  ...
  key_people: []
key_logic_flow:
- 特征存储解决了两个核心问题：传统机器学习中的训练-服务特征偏差，以及 LLM 智能体和 RAG 管线需要的实时结构化上下文注入。
- 特征存储的五个组件分别是：特征注册表（代码定义特征）、离线存储（Parquet + DuckDB）、在线存储（Redis）、物化管道和 FastAPI 检索服务。
- 离线存储面向训练和批量回填，使用 Parquet 文件格式和 DuckDB 查询引擎。
- 在线存储面向推理时低延迟查找，基于 Redis 实现亚 10 毫秒级响应。
- 物化管道定期将最新特征值从离线存储推送到在线存储，保持两者数据一致。
- 文章的示例场景是流媒体服务的 LLM 个性化推荐，需要 user_segment、watch_count_30d 和 last_genre 三个用户特征。
