...
tldr: 布鲁斯·杨德尔（Bruce Yandle）1983年提出的"走私贩与清教徒"（Bootleggers and Baptists）监管联盟框架，精准描述了数据驱动决策文化中的双重动机——真正相信数据的"清教徒"与预先有结论再找数据支撑的"走私贩"形成隐蔽联盟，共同推动同一决策但动机截然相反。
objective_summary: 技术作家Nahla Davies于2025年在KDnuggets发表分析文章，将经济学家Bruce Yandle在1983年提出的"走私贩与清教徒"监管联盟框架映射到数据驱动决策领域。文章指出，任何数据驱动型组织中都存在两类人：一类是真正愿意让证据引导决策的数据清教徒，另一类是已预设结论再反向编造数据故事的数据走私贩。两者表面上使用相同的"数据驱动"语言并支持相同的决策，但动机截然不同。清教徒提供道德合法性，走私贩则利用这种合法性的掩护来推进预先确定的结果。文章建议通过观察当事人在数据与假设矛盾时的反应、区分谁呈现数据与谁选择数据、以及判断分析过程是否真正探索性来识别这两种角色。
...
entities:
  ...
  - Bruce Yandle
key_logic_flow:
- Bruce Yandle在1983年提出"走私贩与清教徒"框架，原用于解释美国南方周日酒类销售禁令的背后联盟——清教徒基于道德支持禁令，走私贩因消灭合法竞争而获利。
- 在数据驱动决策的组织中，数据清教徒真正相信证据引导决策，愿意在数据与假设矛盾时改变观点，并将数据视为照亮未知的探照灯。
- 数据走私贩预先有结论再反向编造数据故事，通过挑选时间范围选择利好指标、忽略不利数据、依赖相关性回避因果性等手段来支撑预设立场。
- 数据清教徒提供的道德正当性降低了其他人跟从决策的政治成本，而走私贩利用清教徒的信誉作为掩护，清教徒往往并未意识到自己是联盟的一部分。
- 区分两者的方法包括：观察数据与预设立场矛盾时的反应、区分谁呈现数据与谁选择数据、判断分析过程是真正探索性还是结论先行。
