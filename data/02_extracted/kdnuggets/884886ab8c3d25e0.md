...
tldr: KDnuggets 盘点 7 个提升 OpenCode AI 编码代理能力的核心插件
objective_summary: KDnuggets 作者 Abid Ali Awan 发表文章，系统介绍了 OpenCode 生态中 7 个最具实用性的插件，涵盖后台代理、身份认证、持久记忆、伪终端、引用搜索、编码统计和可重用技能等功能，展示插件如何扩展
  OpenCode 的编码、研究和自动化工作流。
...
entities:
  ...
  - Abid Ali Awan
key_logic_flow:
- Oh My Openagent 是 OpenCode 生态中最全面的插件，新增后台代理、LSP、AST 和 MCP 工具以及 Claude Code 兼容性。
- Antigravity Auth 插件通过 OAuth 集成 Antigravity 平台，允许用户用 Google 凭证在 OpenCode 内访问 Gemini
  3.1 Pro 和 Claude Opus 4.6 Thinking 等模型。
- Supermemory 插件为 OpenCode 提供跨会话和跨项目的持久记忆能力，使其从无状态助手转变为持续协作工具。
- Pty 插件为 OpenCode 带来交互式伪终端支持，可启动后台进程、发送后续输入并稍后读取输出。
- Websearch Cited 插件在 OpenCode 中添加带引用来源和行内引用的网络搜索工具，支持 Google、OpenAI 或 OpenRouter
  搜索配置。
- Wakatime 插件追踪 AI 辅助的编码活动、耗时和文件变更，集成 WakaTime 仪表盘；Agent Skills 插件支持从项目目录和 Claude
  兼容位置加载可重用技能。
