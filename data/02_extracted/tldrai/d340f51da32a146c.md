...
tldr: Perplexity 发布了名为 Brain 的自我改进记忆系统，为 AI 代理 Computer 构建工作上下文图，夜间自动回顾学习以优化表现。早期测试显示答案正确性提升
  25%，成本降低 13%，已向 Max 和 Enterprise Max 订阅用户开放 Research Preview。
objective_summary: Perplexity 于 2026 年 7 月 22 日发布了 Brain——一种面向 AI 代理的自我改进记忆系统。Brain
  通过构建可追踪的上下文图来记录 Computer 代理执行的工作内容（而非用户偏好），并在设定时间间隔（如夜间）自动回顾和学习优化。该上下文图以 LLM wiki
  形式加载到代理沙箱中，持续更新用户的项目、想法和来源信息。早期测试数据显示 Brain 将重复任务的答案正确性提升 25%，召回率提升 16%，同时将需要历史上下文的任务成本降低
  13%。Brain 以 Research Preview 形式向 Max 和 Enterprise Max 订阅用户开放。
...
entities:
  ...
  key_people: []
key_logic_flow:
- Perplexity 发布了名为 Brain 的自我改进记忆系统，其核心是为 AI 代理 Computer 构建可追踪的工作上下文图。
- 与传统 AI 记忆关注用户偏好不同，Brain 专注于记忆代理执行的工作内容，包括什么有效、什么失败以及用户做出的纠正。
- Brain 在设定的时间间隔（如夜间）自动回顾上下文图并从中学习，形成递归式自我改进的闭环反馈。
- 上下文图以 LLM wiki 的形式自动加载到代理沙箱中，包含用户的想法、人物、项目等元素，并随时间持续增量更新。
- 早期测试数据显示 Brain 将重复任务的答案正确性提升 25%，召回率提升 16%，并将历史上下文任务成本降低 13%。
- Brain 以 Research Preview 形式向 Max 和 Enterprise Max 订阅用户开放，未来将公布更多新功能。
...
- object_type: product
  ...
  evidence_snippets:
  - Brain 是一个自我改进的记忆系统，为 Computer 代理构建可追踪的上下文图，并在夜间自动回顾学习以优化工作表现。
  - Brain 通过记录代理执行的工作内容（而非用户偏好）来形成递归自我改进的反馈循环，从而持续提升任务执行效率。
  - 早期测试显示 Brain 将重复任务的答案正确性提升 25%，召回率提升 16%，并将历史上下文任务成本降低 13%。
  article_id: d340f51da32a146c
- object_type: product
  name: Computer
  ...
  evidence_snippets:
  - Computer 是 Perplexity 的 AI 代理，Brain 为其构建工作上下文图并持续优化其任务执行能力。
  - Computer 借助 Brain 的上下文图可以更快获取答案、访问最可靠的来源，并避免浪费时间和消耗 token。
  - Brain 使 Computer 能够从过去的会话、连接器结果、来源文档变更和用户修正中持续学习。
  article_id: d340f51da32a146c
---
