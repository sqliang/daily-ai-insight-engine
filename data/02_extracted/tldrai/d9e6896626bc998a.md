...
tldr: 字节跳动开源了 Lance，一个仅 3B 活跃参数的原生统一多模态模型，支持图像与视频的理解、生成与编辑，在 128 块 A100 GPU 上从头训练完成，权重通过
  Hugging Face 发布。
objective_summary: 字节跳动（ByteDance）在 GitHub 上开源了 Lance 模型项目。Lance 是一个仅 3B 活跃参数的原生统一多模态模型，其
  Transformer 骨干网络完全从零开始训练（仅 ViT 和 VAE 编码器使用预训练权重），训练预算为 128 块 A100 GPU。该模型支持文生图、文生视频、图像编辑、视频编辑、图像理解和视频理解六大任务。推理需要至少
  40GB VRAM 的 GPU，模型权重通过 Hugging Face 的 Lance-3B 仓库发布。项目提供统一命令行接口和 Gradio 交互演示。
...
entities:
  ...
  - Yongdong Zhang
key_logic_flow:
- 字节跳动开源了 Lance，这是一个仅 3B 活跃参数的原生统一多模态模型，其 Transformer 骨干网络完全从零开始训练。
- Lance 支持六大核心任务：文生图（t2i）、文生视频（t2v）、图像编辑、视频编辑、图像理解和视频理解，全部在单一框架内完成。
- 模型训练预算仅为 128 块 A100 GPU，除 ViT 和 VAE 编码器外全部从零训练，强调训练效率。
- 推理要求 GPU 至少 40GB VRAM，模型权重通过 Hugging Face 上的 Lance-3B 仓库分发展布。
- 项目提供统一的 inference_lance.sh 命令行接口和对应各任务的专用一键脚本，便于开发者快速上手。
- 项目还提供了基于 Gradio 的图形化演示界面，支持文生视频和视频理解交互。
...
- object_type: project
  ...
  evidence_snippets:
  - Lance 是 ByteDance 发布的一个 3B 参数的原生统一多模态模型，支持图像和视频的理解、生成与编辑任务。
  - 该模型在 128 块 A100 GPU 上从零开始训练，仅 ViT 和 VAE 编码器使用预训练权重，变换器骨干完全从头训练。
  - 项目提供统一命令行接口 inference_lance.sh，支持文生图、文生视频、图像编辑、视频编辑、图像理解和视频理解六大任务。
  article_id: d9e6896626bc998a
- object_type: model
  name: Lance-3B
  canonical_name: Lance-3B
  url: null
  confidence: medium
  article_role: mentioned_reference
  evidence_snippets:
  - 文章要求用户从 Hugging Face 的 Lance-3B 仓库下载所有必要的模型权重文件并放置到 downloads/ 目录中。
  - 推理配置支持 Lance_3B（图像任务）和 Lance_3B_Video（视频任务）两种权重版本。
  - 模型仅 3B 活跃参数，推理需要至少 40GB VRAM 的 GPU。
  article_id: d9e6896626bc998a
---
