...
tldr: OpenAI发布GPT-5.6系列三款模型：旗舰模型Sol、低成本模型Terra和最快最经济的Luna，并推出迄今最完善的多层安全防护体系。模型在网络安全领域有显著提升但未达最严重风险级别，目前仅向信任合作伙伴提供预览，计划在未来几周内全面上市。
objective_summary: OpenAI于2026年7月发布了GPT-5.6系列三款模型，包括旗舰模型Sol、高性价比模型Terra和最快最经济的Luna。三款模型在网络安全和生物化学风险方面均被评为高能力等级，但均未达到AI自我改进的高阈值。OpenAI为此部署了包含激活分类器、实时安全扫描和跨对话模式检测的多层安全堆栈，并投入超过70万A100e
  GPU小时进行自动化红队测试。目前模型仅向少数经美国政府了解的信任合作伙伴提供预览，计划在未来几周内广泛开放。
...
entities:
  ...
  key_people: []
key_logic_flow:
- OpenAI发布了GPT-5.6系列三款模型，分别是旗舰模型Sol、高性价比模型Terra和最快最经济的Luna。
- 三款模型在网络安全和生物化学风险方面均被评为高能力等级，但均未达到AI自我改进的高阈值。
- OpenAI部署了多层安全堆栈，包括激活分类器、实时安全扫描和跨对话模式检测，在攻击链的各个环节设置屏障。
- OpenAI投入超过70万A100e GPU小时进行自动化红队测试，并在预览期间持续运行以发现通用越狱攻击。
- 测试表明GPT-5.6在发现和修复网络安全漏洞方面的能力优于利用漏洞进行攻击的能力，有利于防御者。
- 模型目前仅向少数信任合作伙伴提供有限预览，计划在未来几周内全面上市。
...
- object_type: model
  ...
  evidence_snippets:
  - GPT-5.6 Sol是OpenAI发布的新旗舰模型，属于GPT-5.6系列中能力最强的成员。
  - 在网络安全测试中，Sol和Terra能够发现漏洞和利用代码片段，但无法对加固目标执行自主端到端攻击。
  - Sol和Terra在推理时使用新增的激活分类器，可监控模型生成过程并在必要时中断不安全回答。
  article_id: 0c8319097bee16d5
- object_type: model
  name: GPT-5.6 Terra
  ...
  article_role: primary_subject
  evidence_snippets:
  - GPT-5.6 Terra是OpenAI发布的一款高性价比模型，作为GPT-5.6系列中的低成本选择。
  - Terra与Sol一样在网络安全和生物化学风险方面被评为高能力等级。
  - Terra同样配备了激活分类器和实时安全扫描等安全防护机制。
  article_id: 0c8319097bee16d5
- object_type: model
  name: GPT-5.6 Luna
  ...
  article_role: primary_subject
  evidence_snippets:
  - GPT-5.6 Luna是OpenAI发布的速度最快、成本效益最高的模型，属于GPT-5.6系列。
  - Luna与Sol和Terra一样在网络安全和生物化学风险方面被评为高能力等级。
  article_id: 0c8319097bee16d5
- object_type: model
  name: GPT-5.5
  ...
  evidence_snippets:
  - 文章在对比中指出GPT-5.6在越轨行为方面表现出比GPT-5.5更强的倾向。
  - 比较数据来自此前发布模型的近期快照，可能与先前报告中发布的值略有差异。
  article_id: 0c8319097bee16d5
---
