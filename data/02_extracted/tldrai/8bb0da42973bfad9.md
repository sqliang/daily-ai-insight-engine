...
tldr: 智谱AI发布GLM-5.2旗舰模型，支持100万token上下文，专注长周期编码任务
objective_summary: 智谱AI（Z.AI）于2026年6月发布GLM-5.2模型，支持100万token上下文。在FrontierSWE基准测试中仅落后Claude
  Opus 4.8约1%，在PostTrainBench和SWE-Marathon上均排名第二，三项长周期编码基准均为最强开源模型。
...
entities:
  ...
  key_people: []
key_logic_flow:
- GLM-5.2是智谱AI发布的最新旗舰模型，专注于长周期任务，首次在100万token上下文上实现可靠的长周期编码能力。
- 在长周期编码基准测试中，GLM-5.2在FrontierSWE上仅落后Claude Opus 4.8约1%，在PostTrainBench上排名第二、超越GPT-5.5，在SWE-Marathon上排名第二、落后Opus
  4.8约13%，三项均为最强开源模型。
- 引入IndexShare技术，每4个transformer层共享一个轻量级indexer，减少DSA中indexer的点积和topk计算开销，从128K序列长度的中训练阶段开始使用。
- 改进MTP层用于推测解码，通过IndexShare+KV共享、拒绝采样和端到端TV损失三项优化，最终接受长度从4.56提升至5.47（提升20%）。
- 在推理引擎上优化了三方面：基于LayerSplit的细粒度内存管理、随上下文长度增长的高效kernel、CPU端缓存管理以减少GPU执行流水线气泡，长上下文下吞吐量优势显著。
- 采用slime框架进行agentic RL后训练，支持白盒/黑盒rollout等多种模式，整个OPD训练约耗时两天，并通过anti-hack模块检测和阻止编码RL中的奖励篡改行为。
---
