...
tldr: Subquadratic 发布 1200 万 token 上下文窗口模型，在检索基准上超越 GPT-5.5
objective_summary: Subquadratic 于 2026 年 5 月发布首个模型，采用 Subquadratic Selective Attention
  (SSA) 架构，支持 1200 万 token 上下文窗口，在 MRCR v2 基准上得 83 分（GPT-5.
...
entities:
  ...
  - Justin Mateen
key_logic_flow:
- Subquadratic 发布了首个 AI 模型，采用 Subquadratic Selective Attention (SSA) 架构，支持 1200 万
  token 上下文窗口，声称计算和内存开销随上下文长度线性扩展而非二次增长。
- 该模型在 MRCR v2 检索基准得 83 分，超过 GPT-5.5 的 74 分和 Claude Opus 4.7 的 32.2 分；在 SWE-Bench
  Verified 上得 82.4%，超过 Opus 4.6 的 81.4%。
- 所有此前尝试解决注意力二次复杂度问题的方法（固定模式稀疏注意力、状态空间模型、混合架构）均存在权衡，未能完全替代密集注意力。
- SSA 声称解决了 DeepSeek 稀疏注意力中索引器本身二次复杂度的陷阱，实现了内容相关的选择性注意力且选择机制本身非二次。
- Subquadratic 同时推出三个产品：12M token 上下文的 API、命令行编码代理 SubQ Code 和深度研究工具 SubQ Search，均运行在
  neocloud 而非主流云平台。
- 该公司已融资 2900 万美元，估值 5 亿美元，计划 Q4 推出 5000 万 token 上下文窗口的模型。
