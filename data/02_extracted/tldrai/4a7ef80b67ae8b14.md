...
tldr: ThinkingMachines.ai 通过高质量人工标注数据微调开源模型，训练出在金融信息过滤任务上超越 GPT、Claude、Gemini 等前沿模型的专有模型，且成本远低于后者。
objective_summary: ThinkingMachines.ai 评估了 GPT、Claude、Gemini 等前沿模型在六项金融信息过滤任务上的表现，发现其准确率仅约
  50%。通过专家提示工程优化后，准确率提升至 70% 左右但未能突破 80% 的可信阈值。该公司转而构建高质量人工标注数据集，微调开源权重模型，训练出的专有模型在信息准确率和召回率上超越所有被测前沿模型，同时成本大幅降低。作者将这一路径总结为面向特定组织需求的"差异化智能"愿景。
...
entities:
  ...
  key_people: []
key_logic_flow:
- 前沿 LLM（GPT、Claude、Gemini）在未经专门优化的六项金融信息过滤任务中平均准确率仅约 50%，表现与随机猜测相当。
- 专家提示工程将准确率提升至 70% 左右，但自动提示优化方法无法带来进一步增益，模型始终未能达到投资者信任所需的 80% 准确率阈值。
- ThinkingMachines.ai 构建了体现专家投资者判断力的高质量人工标注数据集，用于微调训练。
- 通过微调开源权重模型，该公司训练出专有模型，在信息准确率和召回率上全面超越 GPT、Claude、Gemini 等前沿模型。
- 该专有模型的运行成本远低于前沿模型，例如 GPT 5.4 比 GPT 5.2 贵 43% 但准确率提升微乎其微。
- 作者提出"差异化智能"愿景，认为针对特定组织需求微调的模型可在专业任务上实现比通用前沿模型更优的性能和成本效益。
extract_result: success
object_mentions:
- object_type: model
  name: ThinkingMachines.ai Proprietary Model
  canonical_name: ThinkingMachines.ai Proprietary Financial Filtering Model
  url: null
  confidence: medium
  article_role: primary_subject
  evidence_snippets:
  - ThinkingMachines.ai 通过微调开源权重模型训练出的专有模型在信息准确率和召回率上超越所有被测前沿模型，且成本大幅降低。
  - 该专有模型是为解决金融文档信息过滤任务而训练的，目标是从海量新闻、研究报告和公司文件中识别与投资决策相关的信号。
  - 公司测试了六项来自投资者日常工作的信息过滤任务，专有模型在这些任务上的表现优于 GPT、Claude 和 Gemini 的多个变体。
  article_id: 4a7ef80b67ae8b14
---
