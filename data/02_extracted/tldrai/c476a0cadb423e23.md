...
tldr: Strands 公司发布开源 AI Agent 构建工具包 Strands Agents，提供 Python（strands-agents）和 TypeScript（@strands-agents/sdk）SDK，核心功能包括钩子拦截机制、SteeringHandler
  引导插件和 MCP 服务器集成，Smartsheet、Landchecker、Swisscom 等企业已采用该框架部署生产级 AI 应用。
objective_summary: Strands 公司发布开源框架 Strands Agents，定位为生产级 AI Agent 构建工具包，同时提供 Python（pip
  install strands-agents）和 TypeScript（@strands-agents/sdk）SDK。框架核心机制包括 BeforeToolCallEvent/AfterToolCallEvent
  钩子拦截、SteeringHandler 引导插件系统（可自动检查工具调用质量并给出改进建议）以及内置可观测性与 OpenTelemetry 集成。该框架支持渐进式复杂度升级，并提供了
  strands-agents-mcp-server 以接入 Claude、Cursor、Kiro 等 AI 编码工具。Smartsheet、Landchecker
  和 Swisscom 等企业已基于该 SDK 构建生产级 AI 应用。
...
entities:
  ...
  key_people: []
key_logic_flow:
- Strands Agents 是一个开源的生产级 AI Agent 构建工具包，同时提供 Python（strands-agents）和 TypeScript（@strands-agents/sdk）两种
  SDK。
- 框架通过 BeforeToolCallEvent 和 AfterToolCallEvent 钩子机制，允许开发者在工具调用前后拦截、验证或修改 Agent 行为，实现安全检查与日志记录。
- 内置的 SteeringHandler 引导插件系统可在工具执行前自动检查调用质量，对不符合策略的调用返回 Guide 修正建议或直接阻止。
- 该框架支持渐进式复杂度，开发者可以从简单 Agent 起步，逐步引入 SummarizingConversationManager 等高级对话管理功能而无需重写代码。
- Strands Agents 提供 MCP 服务器（strands-agents-mcp-server），可接入 Claude、Cursor、Kiro 等 AI
  编码工具，提供 search_docs 和 fetch_doc 等工具能力。
- Smartsheet、Landchecker 和 Swisscom 等企业已基于 Strands SDK 构建生产级 AI 应用，其中 Landchecker
  结合 Amazon Bedrock 和 Bedrock Guardrails 实现了事件响应 MTTR 降低 60%。
...
- object_type: project
  ...
  evidence_snippets:
  - 文章标题指出 Steering 功能可帮助 Strands 的 Agent 实现 100% 准确率，并称该框架为构建生产级 Agent 的开源工具包。
  - 文章提供 Python SDK 安装命令 pip install strands-agents 和 TypeScript SDK 命令 npm install
    @strands-agents/sdk。
  - Smartsheet 和 Swisscom 均表示采用 Strands 框架快速构建了生产级 AI 能力，因其提供了企业级特性和开发效率的平衡。
  article_id: c476a0cadb423e23
- object_type: project
  name: '@strands-agents/sdk'
  canonical_name: '@strands-agents/sdk'
  url: https://strandsagents.com
  confidence: high
  article_role: ecosystem_context
  evidence_snippets:
  - 文章提供了完整的 TypeScript SDK 使用示例，包括 Agent 初始化、tool 定义、addHook 挂载 BeforeToolCallEvent
    等。
  - TypeScript SDK 同样支持 SummarizingConversationManager 对话管理器和 SteeringHandler 引导插件系统。
  article_id: c476a0cadb423e23
- object_type: project
  name: strands-agents-mcp-server
  canonical_name: strands-agents-mcp-server
  url: https://strandsagents.com
  confidence: high
  article_role: ecosystem_context
  evidence_snippets:
  - 文章提供 MCP 服务器安装命令 claude mcp add strands uvx strands-agents-mcp-server，用于接入 Claude
    编码工具。
  - 文章还提供了 Cursor 和 Kiro 的 MCP 配置示例，包括自动批准的 search_docs 和 fetch_doc 工具。
  article_id: c476a0cadb423e23
---
