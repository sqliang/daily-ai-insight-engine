...
tldr: Google 发布 Gemini 3.5 Flash，面向智能体和编程的前沿模型。
objective_summary: Google 于 2026 年 5 月 22 日发布 Gemini 3.5 Flash 模型，在智能体和编程基准上超越 Gemini
  3.1 Pro，输出速度是其他前沿模型的 4 倍，已通过 Gemini 应用、Google 搜索 AI Mode、Antigravity
...
entities:
  ...
  key_people: []
key_logic_flow:
- Google 发布 Gemini 3.5 系列首个模型 3.5 Flash，定位为面向智能体和编程的前沿模型。
- 3.5 Flash 在多项基准中超越 Gemini 3.1 Pro：Terminal-Bench 2.1（76.2%）、GDPval-AA（1656 Elo）、MCP
  Atlas（83.6%）、CharXiv Reasoning（84.2%）。
- 3.5 Flash 输出速度是其他前沿模型的 4 倍，在 Artificial Analysis 指数中位于右上象限。
- 该模型已通过 Gemini 应用、Google AI Studio、Android Studio、Gemini Enterprise 等平台向全球用户和开发者开放。
- 多家企业合作伙伴正将 3.5 Flash 用于实际场景：Shopify 并行运行子智能体进行增长预测，Macquarie Bank 加速客户入职流程，Salesforce
  集成到 Agentforce 中。
- Google 正在开发 Gemini 3.5 Pro，已内部使用并计划下个月推出。
