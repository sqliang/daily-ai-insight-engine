...
tldr: Anthropic 发布 Claude Opus 4.8，在编码、智能体、推理等基准测试上超越 Opus 4.7 和 GPT-5.5，新增推理努力程度调节与
  Claude Code 动态工作流功能，价格不变。
objective_summary: Anthropic 于 2026 年 7 月 22 日发布 Claude Opus 4.8，该模型在 Super-Agent、CursorBench、Legal
  Agent Benchmark 及计算机使用等多项基准测试中超越 Opus 4.7 与 GPT-5.5。新版本引入用户可调节的推理努力程度控制、Claude Code
  动态工作流特性，快速模式速度提升 2.5 倍，价格降至前代的三分之一。Devin、Databricks Genie、CoCounsel 和 Hebbia 等企业合作方均报告了显著的性能提升。
...
entities:
  ...
  key_people: []
key_logic_flow:
- Anthropic 发布 Claude Opus 4.8，该模型在编码、智能体能力、推理和知识工作等多项基准测试上均超越前代 Opus 4.7。
- Opus 4.8 在 Super-Agent 基准测试中成为首个完成所有端到端用例的模型，在成本相当的情况下击败 GPT-5.5。
- 新版本引入用户调节推理努力程度的功能，Claude Code 新增动态工作流特性以处理超大规模问题。
- Opus 4.8 的快速模式速度提升 2.5 倍，且价格降至前代模型的三分之一。
- 在计算机使用和浏览器代理方面，Opus 4.8 在 Online-Mind2Web 上取得 84% 的成绩，超越 Opus 4.7 和 GPT-5.5。
- 企业合作方 Devin、Databricks Genie、CoCounsel Legal 和 Hebbia 的测试反馈显示 Opus 4.8 在可靠性、推理质量和工具效率上均有显著提升。
...
- object_type: model
  ...
  evidence_snippets:
  - Anthropic 发布 Claude Opus 4.8，该模型在编码、智能体能力、推理和知识工作等基准测试上均超越前代 Opus 4.7。
  - 在 Super-Agent 基准测试中，Opus 4.8 是唯一完成所有端到端用例的模型，在成本相当的情况下击败 Opus 4.7 和 GPT-5.5。
  - Opus 4.8 在 Online-Mind2Web 上取得 84% 的成绩，超越 Opus 4.7 和 GPT-5.5，是测试过的最强计算机使用和浏览器代理模型。
  article_id: c81b408ab09859a4
- object_type: product
  name: Claude Code
  ...
  evidence_snippets:
  - Claude Code 新增动态工作流功能，使其能够处理超大规模问题，同时模型支持用户调节推理努力程度。
  - 早期测试者称 Opus 4.8 在 Claude Code 中会提出正确问题、自行发现错误，并在复杂多服务探索中建立信心后再做重大更改。
  article_id: c81b408ab09859a4
- object_type: model
  name: GPT-5.5
  ...
  evidence_snippets:
  - 在 Super-Agent 基准测试中，Opus 4.8 在成本相当的情况下击败 GPT-5.5。
  - Opus 4.8 在 Online-Mind2Web 上取得 84%，超过 GPT-5.5 的表现。
  article_id: c81b408ab09859a4
- object_type: product
  name: Devin
  ...
  article_role: mentioned_reference
  evidence_snippets:
  - Devin 团队表示 Opus 4.8 工具调用干净且指令遵循一致，修复了 Opus 4.7 的评论冗余和工具调用问题。
  - Anthropic 的这次发布直接转化为在 Devin 上构建的工程师更快的能力提升。
  article_id: c81b408ab09859a4
- object_type: product
  name: Genie
  canonical_name: Databricks Genie
  url: null
  confidence: high
  article_role: mentioned_reference
  evidence_snippets:
  - 在 Databricks 的 AI 智能体 Genie 中，Opus 4.8 实现了智能体推理的阶跃式改进，速度更快，多模态能力可直接处理 PDF 和图表。
  - Opus 4.8 的 Token 成本比 Opus 4.7 降低 61%。
  article_id: c81b408ab09859a4
- object_type: product
  name: CoCounsel
  ...
  article_role: mentioned_reference
  evidence_snippets:
  - CoCounsel 法律团队称 Opus 4.8 在一致性和推理质量上较前代有显著提升，有助于为法律和税务专业人士构建受托级 AI 系统。
  - Opus 4.8 在 Legal Agent Benchmark 上取得最高分，是首个突破 10% 全通标准的模型。
  article_id: c81b408ab09859a4
- object_type: product
  name: Hebbia
  ...
  article_role: mentioned_reference
  evidence_snippets:
  - 在 Hebbia 的金融文档工作流中，Opus 4.8 保持与 Opus 4.7 相同的高质量输出，同时引文精度和 Token 检索效率更优。
  article_id: c81b408ab09859a4
---
