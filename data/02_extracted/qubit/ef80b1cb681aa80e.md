...
tldr: OpenAI Codex团队成员Vaibhav Srivastav公开了Codex自我蒸馏提示词，用户复制粘贴后Codex可自动分析历史会话中的重复工作流并打包为Skill、Subagent或Automation等可复用工具。OpenAI总裁Greg
  Brockman转发点赞并强调Codex是开源的。
objective_summary: OpenAI Codex团队成员Vaibhav Srivastav于2025年5月至6月期间发布了两版Codex自我蒸馏提示词。第一版聚焦编码任务的重复工作流识别；第二版将数据源扩展至Memories和Chronicle，覆盖范围扩大到写作、规划、沟通、运营等全场景。该提示词利用Codex的Chronicle、Memory和Subagent等新功能，自动将识别出的重复操作打包为Skill、Subagent、Automation或直接跳过。OpenAI总裁Greg
  Brockman转发点赞了该提示词，并指出Codex本身是开源项目。
...
entities:
  ...
  - Greg Brockman
key_logic_flow:
- OpenAI Codex团队成员Vaibhav Srivastav公开发布了Codex自我蒸馏提示词，用户将其复制粘贴进Codex后，Codex会自动分析历史会话中的重复工作流并打包成可复用的工具。
- 第一版提示词仅针对编码任务中的重复工作流，第二版将数据源扩展至Memories和Chronicle，覆盖范围扩大到写作、规划、沟通和运营等全部工作场景。
- Codex将识别出的重复工作打包为四种形式：每周固定流程做成Skill，专门调查任务派Subagent，定时检查做成Automation，边界模糊的任务直接Skip。
- 该提示词依赖Codex近期发布的几项新功能：Chronicle（屏幕截取，4月20日上线）、Memory（用户偏好记忆，4月16日上线）和Subagent（多Agent协调，3月上线）。
- OpenAI总裁Greg Brockman亲自转发点赞了该提示词，并同时指出Codex本身是开源项目。
...
- object_type: product
  ...
  evidence_snippets:
  - OpenAI Codex团队成员Vaibhav Srivastav公开了自我蒸馏提示词，用户复制粘贴后Codex可自动分析历史会话并打包重复工作流为可复用工具。
  - OpenAI总裁Greg Brockman转发点赞了此提示词并强调Codex是开源的。
  - VB本人承认应该将这条提示词做成正式插件，并指出OpenAI最近一直在重置Codex的速率限制。
  article_id: ef80b1cb681aa80e
- object_type: product
  name: Chronicle
  ...
  evidence_snippets:
  - Chronicle是Codex于2025年4月20日上线的预览功能，可以屏幕截取用户在浏览器、Slack和邮件中的操作以发现重复工作。
  - Chronicle目前仅对macOS上的ChatGPT Pro订阅用户开放，且记忆未加密存储在本地。
  article_id: ef80b1cb681aa80e
- object_type: product
  name: Memory
  canonical_name: Codex Memory
  url: null
  confidence: high
  article_role: mentioned_reference
  evidence_snippets:
  - Memory功能于2025年4月16日随Codex大更新上线，可记住用户偏好、修正记录和项目专属知识并在未来会话中自动调用。
  article_id: ef80b1cb681aa80e
- object_type: product
  name: Subagent
  canonical_name: Codex Subagent
  url: null
  confidence: high
  article_role: mentioned_reference
  evidence_snippets:
  - Subagent功能于2025年3月正式上线，由一个管理Agent协调多个专门编码Agent在独立的云端沙盒中运行。
  - 第二版提示词可将需要专门角色去搞定的调查任务直接派Subagent负责。
  article_id: ef80b1cb681aa80e
---
