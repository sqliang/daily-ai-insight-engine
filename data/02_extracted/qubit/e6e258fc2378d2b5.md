...
tldr: 专为 AI Agent 设计的搜索层服务 AnySearch 于 2026 年 5 月 11 日上线，首月吸引 10 万开发者，搜索调用量超 400 万次，GitHub
  Star 突破 4000。
objective_summary: AnySearch 于 2026 年 5 月 11 日正式上线，上线首月即吸引 10 万名全球开发者，累计搜索调用量突破 400
  万次，GitHub Star 超 4000。该产品专为 AI Agent 设计，重构传统搜索逻辑，采用全链路智能搜索体系（查询理解、路由编排、多路并行检索、结构化融合），以自建索引加外部数据源的联邦架构覆盖金融、法律、学术、安全等专业领域。产品支持
  Skill、MCP、API 三种接入方式，其中 Skill 方案上线一周即冲上 Skills.sh 热榜第一。团队对个人开发者免费，Pro 付费版与企业定制方案同步推进中。
...
entities:
  ...
  - 韩广彤
key_logic_flow:
- AnySearch 于 2026 年 5 月 11 日正式上线，上线首月吸引 10 万名全球开发者接入，累计搜索调用量突破 400 万次，GitHub Star
  数突破 4000。
- AnySearch 专为 AI Agent 设计，核心解决传统搜索引擎以人类视觉相关性排序优化目标与 Agent 信息获取需求之间的错位问题。
- AnySearch 采用全链路智能搜索体系，包含查询理解、路由编排、多路并行检索和结构化融合四个环节，一次调用即可完成复杂专业调研。
- AnySearch 采用自建索引加外部数据源接入的联邦架构，在金融、法律、学术、安全、企业工商等高价值领域自建数据管线，并建立多源交叉验证机制。
- AnySearch 同时支持 Skill、MCP、API 三种接入方式，其中 Skill 方案几乎零配置、易部署，上线一周即冲上 Agent 技能市场 Skills.sh
  热榜第一。
- AnySearch 于 6 月 3 日发布 V2.1.0 版本，完成算法层混合排序与架构层领域划分及路由逻辑的核心升级。
...
- object_type: product
  ...
  evidence_snippets:
  - AnySearch 5月11日正式上线，上线首月已有10万名全球开发者接入使用，累计搜索调用量突破400万次。
  - AnySearch 专为 Agent 设计的 AI 搜索层服务，采用自建索引加外部数据源接入的联邦架构。
  - AnySearch 同时支持 Skill、MCP、API 三种接入方式，几乎零配置的 Skill 方案最先出圈。
  article_id: e6e258fc2378d2b5
- object_type: project
  name: AnySearch Skill
  canonical_name: AnySearch Skill
  url: null
  confidence: medium
  article_role: primary_subject
  evidence_snippets:
  - AnySearch 同时支持 Skill、MCP、API 三种接入方式，几乎零配置、易部署的 Skill 方案最先出圈。
  - AnySearch 上线仅一周便冲上 Agent 技能市场 Skills.sh 的热榜第一。
  article_id: e6e258fc2378d2b5
- object_type: project
  name: Skills.sh
  ...
  evidence_snippets:
  - AnySearch 上线仅一周便冲上 Agent 技能市场 Skills.sh 的热榜第一。
  - AnySearch Skill 在 GitHub 上的关注度高于 MCP，但在团队定位中 Skill 适合个人轻量化场景。
  article_id: e6e258fc2378d2b5
---
