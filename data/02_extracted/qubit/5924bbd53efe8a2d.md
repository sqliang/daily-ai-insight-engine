...
tldr: 复旦大学与通义实验室联合提出ToolCUA训练范式，解决GUI操作与工具调用混合动作空间中的路径选择难题。ToolCUA-8B在OSWorld-MCP上达到46.85%准确率，超越Claude-4-Sonnet，代码与模型已全面开源。
objective_summary: 复旦大学和通义实验室MobileAgent团队针对CUA在GUI与Tool混合动作空间中的路径选择问题，提出ToolCUA训练范式。该方法通过Interleaved
  GUI-Tool Trajectory Scaling Pipeline将纯GUI轨迹转化为混合轨迹，再经Tool-Bootstrapped RFT训练。实验发现直接给强模型同时接入GUI和Tool反而导致准确率下降，ToolCUA-8B在OSWorld-MCP基准上达到46.85%准确率，超过Claude-4-Sonnet并接近Claude-4.5-Sonnet。
...
entities:
  ...
  key_people: []
key_logic_flow:
- 复旦大学与通义实验室MobileAgent团队联合提出ToolCUA，旨在解决GUI操作与工具调用混合动作空间中模型无法正确选择路径的问题。
- 实验表明直接给强模型同时接入GUI和Tool反而导致准确率下降，例如Qwen3VL-235B加入工具后准确率从41.1%降至38.1%。
- ToolCUA提出Interleaved GUI-Tool Trajectory Scaling Pipeline，通过构建轨迹感知合成工具库、生成接地工具轨迹、交错混合三步将GUI-only数据转化为混合轨迹。
- 第一阶段通过Tool-Bootstrapped GUI RFT在约18万步数据上做warmup SFT，让模型掌握多模态工具调用基础能力。
- ToolCUA-8B在OSWorld-MCP基准上达到46.85%准确率，超过Claude-4-Sonnet并逼近Claude-4.5-Sonnet。
- ToolCUA的代码和模型权重已全面开源。
extract_result: success
object_mentions:
- object_type: project
  name: ToolCUA
  canonical_name: ToolCUA
  url: null
  confidence: high
  article_role: primary_subject
  evidence_snippets:
  - 复旦大学和通义实验室MobileAgent团队联合提出ToolCUA，一个面向GUI-Tool混合动作空间的Computer Use Agent训练范式。
  - ToolCUA-8B在OSWorld-MCP上达到46.85%准确率，超过Claude-4-Sonnet，逼近Claude-4.5-Sonnet。
  - ToolCUA提出Interleaved GUI-Tool Trajectory Scaling Pipeline，将GUI-only数据转化为interleaved
    GUI-Tool混合轨迹。
  article_id: 5924bbd53efe8a2d
- object_type: project
  name: OSWorld-MCP
  canonical_name: OSWorld-MCP
  url: null
  confidence: medium
  article_role: mentioned_reference
  evidence_snippets:
  - ToolCUA在OSWorld-MCP基准上测试，ToolCUA-8B达到46.85%准确率，超越多个强基线模型。
  - OSWorld-MCP是用于评估Computer Use Agent在GUI-Tool混合场景下能力的基准测试平台。
  article_id: 5924bbd53efe8a2d
---
