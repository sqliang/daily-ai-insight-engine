...
tldr: UniPat AI 发布 SaaS-Bench 评测，将 23 个真实开源 SaaS 系统部署在 Docker 中测试 AI Agent 的办公能力。最强模型
  Claude Opus 4.7 完全通过率仅 3.8%（106 个任务只通过 4 个），Kimi K2.5 和 Gemini 3.1 Pro 完全通过率为零，AI
  全自动办公远未落地。
objective_summary: UniPat AI 于 2026 年发布 SaaS-Bench 基准评测，选取 23 个开源 SaaS 系统通过 Docker
  本地部署，构建 106 个真实办公任务对主流大模型进行测试。任务覆盖软件研发、业务财务、医疗管理等六个专业领域，93.4% 的任务跨越至少两个应用，最长执行轨迹超过
  300 步。评测结果显示，Claude Opus 4.7 的检查点分数为 43.9%，但端到端完全通过率仅 3.8%；Kimi K2.5 和 Gemini 3.1
  Pro 完全通过率为零。研究揭示了 AI Agent 在长流程任务中的四种结构性失败模式：随任务推进通过率持续下降、单步错误导致连锁偏离、以及跨应用和长轨迹场景下性能显著退化。
...
entities:
  ...
  key_people: []
key_logic_flow:
- UniPat AI 发布 SaaS-Bench 基准评测，将 23 个真实开源 SaaS 系统部署在 Docker 中，构建了 106 个跨应用、长流程的真实办公任务。
- SaaS-Bench 覆盖软件研发、业务财务、医疗管理、团队协作、农业供应链和独立媒体六个专业领域，93.4% 的任务跨越至少两个应用。
- Claude Opus 4.7 在评测中检查点分数为 43.9%，但端到端完全通过率仅 3.8%，106 个任务只完整通过了 4 个。
- Kimi K2.5 和 Gemini 3.1 Pro 的完全通过率为零，没有一个任务能够完整走完。
- Pass@3 相比 Pass@1 整体提升约 8 个百分点，但多轮运行无法根本解决 Agent 路径依赖导致的不稳定性问题。
- SaaS-Bench 揭示了四种结构性失败模式：长轨迹任务通过率随推进持续下降、单步错误导致后续轨迹完全分叉、跨应用数从 1 增至 4 时平均分从 53% 降至
  20%。
extract_result: success
object_mentions:
- object_type: project
  name: SaaS-Bench
  canonical_name: SaaS-Bench
  url: null
  confidence: high
  article_role: primary_subject
  evidence_snippets:
  - UniPat AI 发布 SaaS-Bench 评测，选取 23 个开源 SaaS 系统通过 Docker 本地部署，覆盖六个专业领域共 106 个真实办公任务。
  - SaaS-Bench 采用真实系统直接部署在 Docker 中，保留了完整的前后端逻辑、数据库状态和业务约束，而非仿真环境。
  - SaaS-Bench 的 106 个任务中 93.4% 跨越至少两个应用，97.3% 的文本任务操作步数超过 100 步，最长轨迹达 300 步以上。
  article_id: a06d3a4492eba6ee
- object_type: project
  name: Browser-Use
  ...
  evidence_snippets:
  - SaaS-Bench 允许 Agent 使用 Browser-Use 在 SaaS 环境中操作计算机，并通过 Resolved Score 和 Checkpoint
    Score 两个指标评估性能。
  article_id: a06d3a4492eba6ee
---
