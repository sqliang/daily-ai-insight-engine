...
tldr: 百度开源全新OCR模型Unlimited OCR，采用参考滑动窗口注意力机制（R-SWA）实现长文档连续解析，在OmniDocBench上以93.23%综合得分刷新SOTA，超越DeepSeek
  OCR约6.22%。
objective_summary: 百度于2026年6月开源了Unlimited OCR模型，其核心创新是参考滑动窗口注意力机制（R-SWA），将视觉Token始终保留为参考信息、仅对输出文本Token使用滑动窗口，使KV
  Cache保持恒定大小。在OmniDocBench v1.5上取得93.23%综合得分，超越DeepSeek OCR约6.22%；在OmniDocBench v1.6上进一步达到93.92%，刷新当前SOTA。生成6000
  Token时推理速度比DeepSeek OCR提升约35%，且延迟保持稳定。模型支持一次性处理40页以上长文档而不出现明显性能下降。
...
entities:
  ...
  key_people: []
key_logic_flow:
- 百度开源了全新的OCR模型Unlimited OCR，在OmniDocBench v1.5上取得93.23%综合得分，刷新行业SOTA。
- Unlimited OCR的核心创新是参考滑动窗口注意力机制（R-SWA），它将视觉Token始终保留为完整参考信息，仅对输出文本Token使用滑动窗口，从而保持KV
  Cache大小恒定。
- 与传统OCR逐页处理+结果拼接的方案不同，Unlimited OCR支持一次性连续阅读几十页长文档，无需中断上下文。
- 在生成6000 Token时，Unlimited OCR的推理速度相比DeepSeek OCR提升约35%，且调用延迟基本保持稳定。
- R-SWA的设计思路与行业主流的长上下文扩容思路相反，它通过让模型学会'软遗忘'来避免KV Cache无限增长。
- 从DeepSeek发布OCR2、到智谱开源GLM-OCR、再到百度推出Unlimited OCR，头部公司正将OCR视为AI时代的重要数据入口。
...
- object_type: model
  ...
  evidence_snippets:
  - 百度开源了全新的OCR模型Unlimited OCR，该模型在OmniDocBench上刷新SOTA，整体成绩超过此前的DeepSeek OCR。
  - Unlimited OCR借助创新的参考滑动窗口注意力机制，即便文档越来越长，显存占用和注意力计算开销也几乎不会增长。
  - 在OmniDocBench v1.5上Unlimited OCR取得93.23%的综合得分，相比DeepSeek OCR提升6.22%。
  article_id: bd27b18eda87ca56
- object_type: model
  name: DeepSeek OCR
  canonical_name: DeepSeek OCR
  url: null
  confidence: medium
  article_role: mentioned_reference
  evidence_snippets:
  - Unlimited OCR在OmniDocBench上的整体成绩超过此前的DeepSeek OCR。
  - 生成6000个Token时Unlimited OCR的推理速度相比DeepSeek OCR提升约35%。
  article_id: bd27b18eda87ca56
- object_type: model
  name: GLM-OCR
  ...
  article_role: mentioned_reference
  evidence_snippets:
  - 从DeepSeek年初发布OCR2，到智谱开源GLM-OCR，再到百度推出Unlimited OCR，头部公司纷纷重兵投入OCR领域。
  article_id: bd27b18eda87ca56
- object_type: dataset
  name: OmniDocBench
  canonical_name: OmniDocBench
  url: null
  confidence: high
  article_role: ecosystem_context
  evidence_snippets:
  - 研究团队采用OmniDocBench v1.5和v1.6评估模型的文档解析能力。
  - 在OmniDocBench v1.5上Unlimited OCR取得93.23%的综合得分，相比DeepSeek OCR提升6.22%。
  article_id: bd27b18eda87ca56
---
