...
tldr: Claude Fable 5虽然单价是Opus 4.8的两倍，但在低努力档位下因token效率更高，实际完成任务的总成本反而低于Opus 4.8，并在多项基准测试中取得领先成绩。
objective_summary: Anthropic发布的Claude Fable 5模型定价为每百万输入/输出token分别为10/50美元，是上一代旗舰Opus
  4.8的两倍。但开发者发现将努力程度调至low档后，模型在SWE-bench Pro上得分75.0，超过Opus 4.8最强档的68.6，且因更高效而消耗更少token，实际任务成本反而更低。Claude
  Code之父Boris Cherny解释了这一经济账，指出Fable 5完成同一任务平均用的token更少。第三方评测机构TrueFoundry的数据显示，Fable
  5在SWE-Bench Pro上以80.3%成绩领先所有受测模型。
...
entities:
  ...
  - Boris Cherny
key_logic_flow:
- Claude Fable 5定价为每百万输入token 10美元、输出50美元，恰好是上一代旗舰Opus 4.8的两倍。
- 将Fable 5的努力程度调至low档后，在SWE-bench Pro上得分75.0，超过Opus 4.8最强xhigh档的68.6。
- Claude Code之父Boris Cherny解释，Fable 5每token价格虽是Opus两倍，但完成同一任务平均使用更少token，实际花费反而更低。
- 第三方评测机构TrueFoundry的数据显示，Fable 5在SWE-Bench Pro上以80.3%成绩取得所有受测模型中的最高分。
- Fable 5搭载安全分类器，在网络安全、生物化学、模型蒸馏等敏感请求时会自动切换到Opus 4.8回答，触发率不到5%。
- 免费使用窗口截至6月22日，之后将开始消耗用量积分，且使用Fable 5需要开启30天数据保留以满足安全监控要求。
...
- object_type: product
  ...
  evidence_snippets:
  - Claude Fable 5定价为每百万输入token 10美元、输出50美元，是上一代旗舰Opus 4.8的两倍。
  - 将Fable 5努力程度调至low档后，在SWE-bench Pro上得分75.0，超过Opus 4.8最强xhigh档的68.6。
  - Fable 5搭载安全分类器，在敏感请求时会自动切换到Opus 4.8回答并按照Opus价格计费。
  article_id: 9d01bda5c224885a
- object_type: product
  ...
  evidence_snippets:
  - Opus 4.8定价为每百万输入token 5美元、输出25美元，是Fable 5单价的一半。
  - Fable 5在最低努力档位下SWE-bench Pro得分75.0，压过Opus 4.8开到最强xhigh档的68.6。
  - Fable 5的安全分类器在检测到敏感请求时会自动切换到Opus 4.8回答并按Opus价格计费。
  article_id: 9d01bda5c224885a
---
