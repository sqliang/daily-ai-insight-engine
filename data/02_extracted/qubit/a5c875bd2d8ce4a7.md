...
tldr: AI网关OrcaRouter上线可编程路由策略Routing DSL，支持多模型并行作答与自动仲裁，组合常规模型即可在综合胜率上反超Claude Fable
  5。
objective_summary: OrcaRouter在2026年6月上线了可编程路由策略Routing DSL，支持将多个AI模型编排为并行面板并自动仲裁最优结果。测试数据显示Opus
  4.8与GPT-5.5组合的综合得分约67.5%，反超Fable 5的65.5%；甚至同一模型自组（Opus×2）也能追平Fable 5。Routing DSL使用YAML规则和CEL表达式进行条件路由，提供lint校验、dry-run试跑、影子模式和灰度放量等安全机制，降低了多模型调度的工程门槛。
...
entities:
  ...
  key_people: []
key_logic_flow:
- AI网关OrcaRouter上线了可编程路由策略Routing DSL，支持通过YAML规则和CEL表达式定义多模型调度逻辑。
- Routing DSL的核心能力是"并行扇出+仲裁"：让2到5个模型同时作答，再由一个裁判模型从结果中选出最优解。
- 测试数据显示Opus 4.8与GPT-5.5组合的得分约67.5%，同时高于各自单跑成绩（58.5%和60%），并反超Fable 5约65.5%的基准线。
- 一组低价模型（Gemini 3 Flash、Kimi K2.6、DeepSeek V4 Pro）组合后得分约64.5%，同样逼近Fable 5但成本大幅降低。
- Routing DSL支持按难度智能分流、置信度级联兜底、lint校验、dry-run试跑、影子模式与灰度放量等工程安全机制。
- 文章认为"怎么编排"正在成为与"用哪个模型"同等重要的性能杠杆，协作式路由比单体模型更高效。
extract_result: success
object_mentions:
- object_type: product
  name: OrcaRouter
  canonical_name: OrcaRouter
  url: https://www.orcarouter.ai
  confidence: high
  article_role: primary_subject
  evidence_snippets:
  - AI网关OrcaRouter最近上线了一套可编程路由策略Routing DSL，多个模型同时答题，自动仲裁出最优解。
  - 几个你现在就能调用的常规模型给它来个组合编排，跑出来的综合胜率直接掀翻了Fable 5的单体基准线。
  - OrcaRouter把这套编排逻辑交给你自己写，规则用YAML，条件用Google的CEL表达式。
  article_id: a5c875bd2d8ce4a7
- object_type: project
  name: Routing DSL
  canonical_name: OrcaRouter Routing DSL
  url: https://docs.orcarouter.ai/routing/routing-dsl
  confidence: high
  article_role: primary_subject
  evidence_snippets:
  - Routing DSL的核心是parallel并行扇出加arbiter仲裁：让2到5个模型并行作答，再由一个裁判模型挑出最优解。
  - Routing DSL支持四种仲裁策略和置信度级联兜底，当响应触发patch_invalid或self_doubt信号时自动改投更强模型重试。
  - Routing DSL配套自带lint校验、dry-run试跑、影子模式（只评估不生效）和灰度放量滑杆，改路由可以零风险跑起来再放量。
  article_id: a5c875bd2d8ce4a7
---
