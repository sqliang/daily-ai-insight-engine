...
tldr: 英伟达开源 NeMo AutoModel，在 Hugging Face Transformers v5 基础上增加专家并行、DeepEP 和 TransformerEngine，用户只需添加一行
  import 代码即可将 MoE 微调速度提升 3.4-3.7 倍，同时 GPU 显存占用降低 29%-32%。
objective_summary: 英伟达于 2026 年 6 月正式开源 NeMo AutoModel，这是一个专为大规模构建和微调生成式 AI 模型而设计的训练框架。它兼容
  Hugging Face Transformers v5 的 API，用户只需在现有代码中添加一行 import，即可引入专家并行（EP）、DeepEP 和 TransformerEngine
  三项优化技术。在 8 卡 H100 80GB GPU 上，Qwen3-30B-A3B 的每 GPU 每秒吞吐量从 3075 提升至 11340，达到 3.69
  倍提升，峰值内存占用从 68.2 GiB 降至 48.1 GiB。英伟达已将代码、配置和基准测试脚本全部发布在 GitHub 上。
...
entities:
  ...
  key_people: []
key_logic_flow:
- 英伟达开源了 NeMo AutoModel，这是一个基于 Hugging Face Transformers v5 构建的 MoE 大模型微调加速框架。
- NeMo AutoModel 在 Transformers v5 基础上增加了三项核心技术：专家并行（降低每 GPU 专家权重内存占用至 1/8）、DeepEP（实现通信与计算重叠）、TransformerEngine（加速注意力机制和线性层等核心运算）。
- 用户无需修改原有 Transformers v5 代码的 API，只需添加一行 import 即可获得加速效果。
- 在 Qwen3-30B-A3B 和 Nemotron 3 Nano 30B-A3B 上实测显示，训练吞吐量提升 3.4-3.7 倍，GPU 显存占用降低 29%-32%。
- 在 16 个 H100 节点、128 张 GPU 上，Nemotron 3 Ultra 550B A55B 的全参数微调实现了每 GPU 每秒 815 tokens
  的吞吐量，而 Transformers v5 在该规模下无法运行。
- 英伟达已将代码、配置和基准测试脚本全部托管在 GitHub（NVIDIA-NeMo/Automodel）并提供了详细使用文档。
...
- object_type: project
  ...
  - 英伟达已经把代码、配置和基准测试脚本都放在GitHub上了：https://github.com/NVIDIA-NeMo/Automodel/tree/blog/transformers-v5-automodel/blog_experiments
  article_id: e4044530eaf8bfa6
- object_type: project
  name: Transformers v5
  canonical_name: Hugging Face Transformers v5
  url: null
  confidence: high
  article_role: ecosystem_context
  evidence_snippets:
  - HuggingFace的Transformers v5是目前被用得比较多的MoE训练通用底座，v5增强了对MoE的原生支持。
  - NeMo AutoModel在Transformers v5的基础上，增加了专家并行（EP）、DeepEP和TransformerEngine。
  article_id: e4044530eaf8bfa6
- object_type: model
  name: Qwen3-30B-A3B
  ...
  evidence_snippets:
  - 在单节点8xH100 80GB GPU上，以Qwen3-30B-A3B为例，NeMo AutoModel直接把每GPU每秒吞吐量从3075拉到11340，提升达到3.69倍。
  - 对于Qwen3，专家并行技术能将峰值内存从68.2GiB降至48.1GiB，降幅29%。
  article_id: e4044530eaf8bfa6
- object_type: model
  ...
  article_role: mentioned_reference
  evidence_snippets:
  - 在Qwen3-30B-A3B和Nemotron 3 Nano 30B-A3B上，相较于Transformers v5，该方案可以实现3.4-3.7倍的训练吞吐量提升。
  article_id: e4044530eaf8bfa6
- object_type: model
  ...
  article_role: mentioned_reference
  evidence_snippets:
  - 英伟达还展示了Nemotron 3 Ultra 550B A55B在16个H100节点、128张GPU上的全参数微调结果，每GPU每秒吞吐量为815。
  article_id: e4044530eaf8bfa6
---
