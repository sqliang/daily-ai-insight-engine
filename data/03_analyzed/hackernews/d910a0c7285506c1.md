...
source: https://sbcl.org/all-news.html?2.6.7
author:
- '[[tmtvl]]'
...
- '2026-07-28'
- '2026-07-29'
description: 'Article URL: https://sbcl.org/all-news.html?2.6.7 Comments URL: https://news.ycombinator.com/item?id=49086971
  Points: 231 # Comments: 99'
tags:
- clippings
...
tldr: SBCL 于 2026-07-28 发布 2.6.7 版本：新增 SB-MANUAL 手册 contrib 模块、DOCUMENTATION 支持 DOC-TYPE
  DECLARATION、SB-SIMD 支持 ARM64、x86-64 新增 AVX512 指令支持，并修复多项编译器与平台 bug。
objective_summary: Steel Bank Common Lisp（SBCL）项目于 2026 年 7 月 28 日发布 2.6.7 版本。该版本新增
  SB-MANUAL contrib 模块，将 SBCL 手册整合进各节定义的 docstring 中，使其可在 Slime 中以交互方式浏览。平台方面，SB-SIMD
  开始支持 ARM64，x86-64 新增 AVX512 指令支持。此外还修复了 SAP-REF-N 在 ARM64 上的误编译、CONCATENATE 编译类型错误等多项
  bug，并对常量复数传参和 UTF-8 转换等场景做了性能优化。
...
entities:
  ...
  - Scott L. Burson
key_logic_flow:
- SBCL 2.6.7 于 2026 年 7 月 28 日正式发布，版本更新信息由项目官方新闻页对外公布。
- 本版本最大的新增点是 SB-MANUAL contrib 模块，它把 SBCL 手册内容放进 docstring 中，可通过 Slime 的 M-. 交互式浏览，也可用
  MGL-PAX 库浏览。
- 平台支持方面，SB-SIMD 新增对 ARM64 的支持，x86-64 平台新增 AVX512 指令支持，并扩展了两大架构上的 SIMD 指令覆盖范围。
- bug 修复涵盖 SAP-REF-N 在 ARM64 上的误编译、CONCATENATE 的编译类型错误、EQL 复数类型处理、LOG 对安静 NaN 输入的处理等多项问题。
- 性能优化包括常量复数传给局部函数无需 consing、UTF-8 转换使用增强 SIMD 例程、COUNT 变换支持更多关键字参数等。
- 文档方面修正了多处拼写与排版错误，并澄清数组按行优先（row-major）而非列优先排列。
object_mentions:
- object_type: project
  name: Steel Bank Common Lisp (SBCL)
  canonical_name: SBCL
  url: https://sbcl.org/
  confidence: high
  article_role: primary_subject
  evidence_snippets:
  - Steel Bank Common Lisp（SBCL）是一个开源 Common Lisp 实现，本文是其于 2026 年 7 月 28 日发布的 2.6.7
    版本更新日志。
  - SBCL 的新版本通常在每月月底发布，具体当前版本可通过 Sourceforge 的文件列表查看。
  article_id: d910a0c7285506c1
- object_type: project
  name: SB-MANUAL
  canonical_name: SB-MANUAL
  url: null
  confidence: high
  article_role: primary_subject
  evidence_snippets:
  - 新增的 contrib 模块 SB-MANUAL 把 SBCL 手册放入各节定义的 docstring 中，将普通 Lisp 定义的文档串联起来，可在 Slime
    中交互式浏览。
  - 官方手册仍从 Texinfo 生成，但 Texinfo 文件现已由 SB-MANUAL 生成，文档内部 docstring 也改为符合 Markdown 子集格式。
  article_id: d910a0c7285506c1
- object_type: project
  name: SB-SIMD
  canonical_name: SB-SIMD
  url: null
  confidence: high
  article_role: mentioned_reference
  evidence_snippets:
  - SB-SIMD contrib 模块在 2.6.7 版本中新增对 ARM64 架构的支持，x86-64 平台也新增对 AVX512 指令的支持。
  - 文章还提到对 ARM64 与 X86-64 上的 SIMD 指令做了额外支持，并优化了 UTF-8 转换所用 SIMD 例程。
  article_id: d910a0c7285506c1
- object_type: project
  name: MGL-PAX
  canonical_name: MGL-PAX
  url: null
  confidence: medium
  article_role: mentioned_reference
  evidence_snippets:
  - 文章提到 SB-MANUAL 手册可通过位于 SBCL 项目树之外的 MGL-PAX 库进行浏览，为开发者提供了另一种阅读方式。
  article_id: d910a0c7285506c1
- object_type: product
  name: fixnum.com
  canonical_name: fixnum.com
  url: https://fixnum.com/
  confidence: medium
  article_role: mentioned_reference
  evidence_snippets:
  - 文章提到 fixnum.com 提供 SBCL 手册的替代渲染版本，包括带大量链接的 PDF 与 HTML 文档，以及 Markdown 和纯文本版本。
  article_id: d910a0c7285506c1
extract_result: success
impact_score:
  score: 2.0
  reason: 这是一次 SBCL 社区例行的月度版本发布。虽然 ARM64 SIMD 支持、x86-64 AVX512 指令接入以及 SB-MANUAL 手册交互式浏览对
    Common Lisp 生态是实打实的进步，但 SBCL 本身在 AI 行业的直接影响力极小，既无 AI 训练/推理框架依赖它，也不改变任何局部竞争格局。属于小圈子内的基础设施迭代，对短期行业冲击力可忽略，故评分仅
    2 分。
sentiment: positive
developer_sentiment:
  tone: neutral
  primary_focus: SB-MANUAL 手册交互浏览体验与 ARM64/x86-64 SIMD 指令覆盖度
hype_assessment:
  level: low
  reason: '全文是标准的版本变更日志，逐条列出新增功能、平台支持、bug 修复与性能优化，全部指向具体可验证的技术事实（如 #2160747、#2160429
    等 bug 编号），没有任何''颠覆''''革命性''等 PR 滥用词汇，也不涉及商业叙事。炒作指数极低。'
information_entropy: medium
domain_disruption:
  technical_innovation: 将 AVX512 与 ARM64 高位宽 SIMD 指令支持引入动态类型语言运行时，并针对 UTF-8 转换、常量复数传参等场景做编译期优化，体现了'让高层语言贴近底层硬件'的工程思路；SB-MANUAL
    把官方手册以 docstring 形式内嵌进源码定义，可在 Slime 中通过 M-. 交互式跳转浏览，是一种文档基础设施的工程创新。整体是渐进式增量改进，而非范式突破。
  business_model: 无
engineering_complexity: production_ready
compound_value:
  score: 2.5
  reason: 从资本视角看，这是 SBCL（Steel Bank Common Lisp）这一历史悠久的开源语言运行时的一次常规增量版本更新，不具备成为 AI
    价值链核心基础设施的潜力。需求侧：AI 产业的计算栈几乎完全建立在 Python/C++/Rust 与 GPU/云生态之上，Common Lisp 属边缘小众语言；2.6.7
    新增的 ARM64/AVX512 SIMD 支持、UTF-8 SIMD 优化等，只服务于该语言内部的高性能场景，无法外溢为 AI 工作负载的通用基础能力，也没有对应的大规模商业化买单方。供给侧：SBCL
    是社区驱动的免费开源软件，无商业化主体、无付费转化路径、无网络效应与数据飞轮，版本迭代的复利仅在小圈子内积累，难以转化为可捕获的资本价值。格局侧：该事件不改变任何
    AI 竞争格局，对资金流向无影响。3-5 年后 SBCL 大概率仍是 Lisp 爱好者社区的基石，但不会成为 AI 行业的基础设施，故评分落在'昙花一现'与'细分基础设施'之间、明显偏向前者。
...
- 量化金融与符号计算领域可关注 SBCL 2.6.7 新增的 ARM64 与 AVX512 SIMD 支持，用于提升 Lisp 栈数值密集型工作负载（如定价模型、回测引擎）的性能
- SB-MANUAL 将文档直接嵌入 docstring 并通过 Slime M-. 交互式浏览的设计，为开发者工具与语言类产品提供了'文档即代码'的体验范式参考
- 建议 Lisp 生态开发者跟进 SB-SIMD 的指令抽象层设计思路，可迁移至其他小众编程语言的高性能计算库建设中
risk_matrix:
  regulatory: 无
  technological: Common Lisp 生态整体呈边缘化趋势，SB-SIMD 对 AVX512/ARM64 的支持相比 Rust、Julia 等原生语言仍滞后多年，存在架构迭代与生态萎缩的长期技术替代风险
  competitive: 高性能数值计算领域面临 Julia、Rust、Mojo 等现代语言的有力竞争，且 Lisp 在主流 AI 技术栈中的地位持续被 Python/C++
    生态挤压，SBCL 改进难以扭转生态分流趋势
  ethical: 无
  additional: []
confidence:
  impact: medium
...
- object_type: project
  ...
  canonical_name: SBCL
  url: https://sbcl.org/
  positioning: SBCL 是历史悠久且广泛使用的开源 Common Lisp 编译器实现，以稳定性与月度版本迭代著称，是 Lisp 社区的核心基础设施。
  technical_signal: SBCL 2.6.7 新增 SB-MANUAL 手册模块、DOCUMENTATION 支持 DOC-TYPE DECLARATION，并扩展了
    ARM64 与 x86-64 的 SIMD 指令支持。
  adoption_signal: SBCL 维持每月月底发布新版本的节奏，2.6.7 于 2026 年 7 月 28 日如期发布，社区贡献者持续参与。
  ecosystem_relevance: 作为 Common Lisp 主流实现，SBCL 的编译优化与平台支持直接惠及整个 Lisp 生态，Slime 等工具链深度依赖其能力。
  target_users: []
  product_signal: null
  market_signal: null
  differentiation: null
  watch_reason: SBCL 是 Common Lisp 生态的中流砥柱，其月度版本演进反映编译器与运行时技术的持续投入，且 SIMD、文档基础设施等方向与主流语言功能对标，值得长期跟踪其平台支持与性能优化进展。
  risk_notes:
  - SBCL 属于小众语言生态，其用户基数远小于主流语言，社区贡献者规模有限，功能演进速度受制于活跃开发者数量。
  score: 7.0
  article_ids:
  - d910a0c7285506c1
  evidence_snippets:
...
- object_type: project
  ...
  canonical_name: SB-MANUAL
  url: null
  positioning: SB-MANUAL 是 SBCL 2.6.7 新增的 contrib 模块，把官方手册内容嵌入各节 docstring，使文档可在 Slime
    中交互式浏览。
  technical_signal: SB-MANUAL 将手册文档放入 Lisp 定义 docstring 中，并让 Texinfo 文件改由 SB-MANUAL
    生成，docstring 内部改用 Markdown 子集格式。
  adoption_signal: 作为随 SBCL 2.6.7 发布的新模块，SB-MANUAL 随版本默认提供，开发者升级后即可通过 Slime 的 M-.
    交互式浏览手册。
  ecosystem_relevance: SB-MANUAL 将手册与代码定义天然关联，改善文档可发现性，并支持 MGL-PAX 与 fixnum.com 等外部工具进行多样化浏览。
  target_users: []
  product_signal: null
  market_signal: null
  differentiation: null
  watch_reason: SB-MANUAL 代表一种将手册融入源码 docstring 的文档组织新模式，其与 Texinfo、Markdown 的生成链路是否成为
    SBCL 官方文档唯一来源，值得持续观察其演进与采纳情况。
  risk_notes:
  - 手册来源从 Texinfo 切换为 SB-MANUAL 生成属于基础设施变更，若 Markdown 子集渲染或文档生成链路不稳定，可能影响官方文档发布质量。
  score: 6.0
  article_ids:
  - d910a0c7285506c1
  evidence_snippets:
...
- object_type: project
  ...
  canonical_name: SB-SIMD
  url: null
  positioning: SB-SIMD 是 SBCL 的 SIMD 指令集 contrib 模块，2.6.7 起新增 ARM64 架构支持，并在 x86-64
    平台引入 AVX512 指令。
  technical_signal: SB-SIMD 在 2.6.7 中新增 ARM64 支持，x86-64 平台新增 AVX512 指令，并扩展两大架构的指令覆盖，同时优化
    UTF-8 转换所用 SIMD 例程。
  adoption_signal: SIMD 支持随 SBCL 2.6.7 发布直接可用，开发者无需额外配置即可在 ARM64 与 x86-64 平台调用相关指令能力。
  ecosystem_relevance: SB-SIMD 的跨架构扩展提升了 SBCL 在现代 CPU 上的计算性能，对数值计算与多媒体等重负载 Lisp 应用具有重要意义。
  target_users: []
  product_signal: null
  market_signal: null
  differentiation: null
  watch_reason: SBCL 持续加码 SIMD 能力体现其重视现代硬件性能的技术路线，ARM64 与 AVX512 支持能否吸引更多性能敏感型 Lisp
    用户，值得持续跟踪。
  risk_notes:
  - SIMD 指令在不同架构上的行为差异较大，部分指令可能存在兼容性或错误编译风险，仍需在真实负载场景中验证稳定性。
  score: 5.0
  article_ids:
  - d910a0c7285506c1
  evidence_snippets:
...
- object_type: project
  ...
  canonical_name: MGL-PAX
  url: null
  positioning: MGL-PAX 是独立于 SBCL 项目的第三方文档浏览库，可在项目树之外浏览 SB-MANUAL 手册内容，为 Lisp 开发者提供另一种阅读方式。
  technical_signal: MGL-PAX 能够解析 SB-MANUAL 放入 docstring 中的手册内容，并与 Slime 的 M-. 交互式浏览形成互补的阅读路径。
  adoption_signal: SBCL 官方将 MGL-PAX 列为 SB-MANUAL 的推荐浏览方式之一，表明其与官方文档生态存在一定程度的适配。
  ecosystem_relevance: MGL-PAX 与 SBCL 官方文档体系形成互补，其被官方提及说明 Lisp 社区文档工具链呈现多样性与协同演进趋势。
  target_users: []
  product_signal: null
  market_signal: null
  differentiation: null
  watch_reason: MGL-PAX 被 SBCL 官方文档明确提及作为 SB-MANUAL 的浏览方式之一，其与官方文档格式的兼容性演进，以及是否被更多
    Lisp 项目采纳，值得长期观察。
  risk_notes:
  - MGL-PAX 是第三方维护的开源库，其更新节奏与 SBCL 文档格式变化可能存在不同步风险，未来兼容性存在不确定性。
  score: 3.0
  article_ids:
  - d910a0c7285506c1
  evidence_snippets:
...
- object_type: product
  ...
  canonical_name: fixnum.com
  url: https://fixnum.com/
  positioning: fixnum.com 是独立于 SBCL 项目的手册渲染网站，提供带大量链接的 PDF 与 HTML 文档，以及 Markdown 和纯文本版本的
    SBCL 手册。
  ...
  - Common Lisp 学习者
  - Lisp 文档研究者
  product_signal: fixnum.com 提供 SBCL 手册的替代渲染版本，包含大量交叉链接的 PDF 与 HTML 文档，以及 Markdown
    和纯文本格式。
  market_signal: 作为第三方手册托管站点，fixnum.com 的存在表明 SBCL 文档消费存在多样化需求，社区生态中出现了专门化的文档服务。
  differentiation: 与官方 Texinfo 手册相比，fixnum.com 以大量交叉链接和多种输出格式提供更易导航的阅读体验，且独立于官方发布流程运作。
  watch_reason: fixnum.com 作为独立于官方的手册渲染服务，其更新频率能否与 SBCL 月度发布保持同步，以及多格式输出能否沉淀为社区常用文档入口，值得持续跟踪。
  risk_notes:
  - fixnum.com 由第三方独立维护，存在更新滞后与服务不可用的风险，其内容准确性与官方手册的一致性无法完全保证。
  score: 3.0
  article_ids:
  - d910a0c7285506c1
  evidence_snippets:
...
month: check the Sourceforge File List to see the current version. The new features of all SBCL releases are listed below.
...
Back to top- minor incompatible change: the never-documented :NO-CONSTRUCTOR-DEFUN
...
Back to top- bug fix: the elftool utility finds a writeable directory in more
...
Back to top- minor incompatible change: the compiler will emit optimization notes
...
Back to top- minor incompatible change: *COMPILE-VERBOSE* and *LOAD-VERBOSE* are bound
...
Back to top- minor incompatible change: streams with an external-format specified with
...
Back to top- enhancement: The compiler now allows stack allocating vectors of any size
...
Back to top- enhancement: stack allocation via DYNAMIC-EXTENT now applies to all values
...
Back to top- enhancement: a mark-region parallel garbage collector is available as a
...
Back to top- minor incompatible change: MACROLET macro functions are now compiled with
...
Back to top- enhancement: Unicode support has been updated to support version 15.0.0 of
...
Back to top- enhancement: attempts to call non-callable objects can now provide
...
Back to top- enhancement: LET-bound anonymous closures declared dynamic extent can now
...
Back to top- incompatible change: the :ORDER long-form-option in
...
Back to top- sb-graph has been removed. To visualize IR1 in sbcl, it is recommended to
...
Back to top- enhancement: support for SLOT-VALUE and friends has been extended to
...
Back to top- minor incompatible change: support for 32-bit x86/Darwin has been removed.
...
Back to top- minor incompatible change: the compiler emits full WARNINGs for undefined
...
Back to top- minor incompatible change: support for 32-bit x86 on macOS has been
removed.
...
Back to top- minor incompatible change: SB-EXT:*DERIVE-FUNCTION-TYPES* being NIL now
...
Back to top- enhancement: better constraint propagation in the compiler. Specifically,
...
Back to top- minor incompatible change: SB-THREAD:MUTEX-OWNER may return :THREAD-DEAD
...
Back to top- incompatible change: DEFINE-ALIEN-CALLBACK, which has never been exported
...
Back to top- minor incompatible change: *COMPILE-PRINT* now defaults to NIL. T gives
...
Back to top- incompatible change: simd-pack without a specific element-type is no
...
Back to top- minor incompatible change: the experimental DEFCAS macro has been removed.
...
Back to top- minor incompatible change: the experimental DEFINE-CAS-EXPANDER macro has
been removed.
...
Back to top- incompatible change: on certain platforms (currently just x86-64),
...
Back to top- minor incompatible change: COMPILE-FILE does not merge the input file's
...
Back to top- minor incompatible change: on x86-64, the backend instruction encoders for
...
Back to top- minor incompatible change: support for the :SB-SAFEPOINT-STRICTLY,
...
Back to top- minor incompatible change: the MAKE-EA internal function, used in the
...
Back to top- minor incompatible change: (ARRAY NIL (*)) is not a subtype of STRING,
...
Back to top- minor incompatible change: the funarg given to SB-SPROF:MAP-TRACES
...
- bug fix: eliminate type errors when wall clocks go back (#1028026,
#1032111)
...
Back to top- incompatible change: HPPA and DEC Alpha architecture
...
Back to top- minor incompatible change: SB-THREAD:THREAD-OS-TID returns NIL for
...
Back to top- planned incompatible change: the defined symbols in the Metaobject
...
Back to top- incompatible change: the external format now defaults to UTF-8 and is not
...
Back to top- new feature: CMUCL-style block compilation support has been added,
...
Back to top- minor incompatible change: some symbols have been removed from *FEATURES*
...
Back to top- minor incompatible change: heap relocation now works on Windows. Since
...
Back to top- minor incompatible change: hash tables on 64-bit machines consume roughly
...
Back to top- enhancement: RISC-V support with the generational garbage collector.
...
Back to top- enhancement: restarts for missing package errors.
...
Back to top- enhancement: SB-COVER emulates IN-PACKAGE when recording source maps;
...
Back to top- minor incompatible change: ENSURE-DIRECTORIES-EXIST no longer establishes
...
Back to top- enhancement: added AVX2 instructions on x86-64, which can be used with
SB-SIMD-PACK.
...
Back to top- enhancement: attempting to build on openbsd 6.0 or newer without
...
Back to top- minor incompatible change: PRINT-TYPE and PRINT-TYPE-SPECIFIER are not
...
Back to top- minor incompatible change: a number of platform-specific elements of
...
#1793150, #1791550, #1792030, #1791059, #1790157, #1790717,
#1790703)

## New in version 1.4.11, 2018-08-28

Back to top- minor incompatible change: changes to the (internal) assembler on x86
...
Back to top- enhancement: added SB-EXT:MAKE-WEAK-VECTOR.
...
Back to top- enhancement: SB-COVER instrumentation for x86[-64] has signficantly less
...
Back to top- bug fix: redefinition of method combinations now does the expected thing;
...
Back to top- enhancement: better handling of unknown keyword arguments (#1750466)
...
Back to top- enhancement: DISASSEMBLE on a symbol naming a macro will disassemble
...
Back to top- minor incompatible change: building with/without the :sb-package-locks
...
Back to top- bug fix: pathname accessors such as PATHNAME-DIRECTORY work on
SYNONYM-STREAMs.
...
Back to top- enhancement: SLEEP respects deadlines established by SB-SYS:WITH-DEADLINE.
...
Back to top- minor incompatible change: SB-EXT:QUIT is no longer marked as deprecated
...
Back to top- optimization: faster foreign callbacks.
...
Back to top- minor incompatible change: DESTRUCTURING-BIND treats non-toplevel () as
...
Back to top- minor incompatible change: the CLOBBER-IT restart for defstruct redefintion
...
Back to top- minor incompatible change: DEF{GENERIC,METHOD} no longer accept some
...
Back to top- enhancement: specialized arrays can be printed readably without using
*read-eval*
...
Back to top- minor incompatible change: existing values of CFLAGS, ASFLAGS,
...
Back to top- enhancement: memory overhead from the garbage collector's metadata
...
Back to top- optimization: various small tweaks give around 5% faster garbage
collection
...
Back to top- minor incompatible change: the reader will when feasible create
...
Back to top- minor incompatible change: the SB-PCL walker no longer recognizes
...
Back to top- enhancement: SET triggers package locks on undefined variables.
(#1645152)
...
Back to top- enhancement: on x86-64, compiled functions loaded from fasl files
...
Back to top- minor incompatible change: SB-EXT:*INTEXP-MAXIMUM-EXPONENT* is removed.
...
Back to top- enhancement: more compact low-level error signaling code
...
Back to top- minor incompatible change: NAMESTRING prefers to return a BASE-STRING
...
Back to top- minor incompatible change: the system now understands that the CONDITION
...
Back to top- bug fix: preserve the name of the destructive function for the destroyed
...
Back to top- bug fix: do not insert conditional newlines in print-unreadable-object
(#1398290). This reverses a change made in #488979 which reversed
...
Back to top- enhancement: the platform's strtod() is exposed as SB-POSIX:STRTOD
...
Back to top- enhancement: who-sets and who-references work on DEFGLOBAL. (#1552997)
...
Back to top- enhancement: warn about argument mismatch for functions passed as
...
Back to top- enhancement: {READ,WRITE}-SEQUENCE are much faster for some combinations
...
Back to top- enhancement: SB-THREAD support for ARM64.
...
Back to top- minor incompatible change: the environment passed to a macro/setf/deftype
...
Back to top- enhancement: by default, timers with a repeat interval do not "catch up"
...
Back to top- new feature: DEPRECATION declaration for functions, variables and types
...
Back to top- minor incompatible change: The name of a compiled anonymous lambda
...
Back to top- incompatible change: on success, TRY-SEMAPHORE and WAIT-ON-SEMAPHORE
...
Back to top- minor incompatible change: the SB-C::*POLICY* variable is no longer
...
Back to top- enhancement: SET-PPRINT-DISPATCH will warn when given an expression in
...
Back to top- minor incompatible change: all SOCKINT::WIN32-* functions have been
...
Back to top- minor incompatible change and bug fix: unboxed numeric constants
...
- bug fix: more robust debugger and backtraces. (#1413850, #1099500,
#1412881, #945261, #1419205, #1409402)
...
Back to top- enhancement: better error and warning messages. (#1314767, #736383)
- enhancement: backtrace for invalid argument count produces the exact
...
Back to top- optimization: returning constant values refers to preboxed constants
...
Back to top- enhancement: SERVE-EVENTS uses the poll() system call in lieu of
...
Back to top- enhancement: sb-bsd-sockets now has basic support for IPv6
...
Back to top- enhancement: the sequence functions MAP, CONCATENATE and MERGE now call
...
Back to top- enhancement: DOCUMENTATION works on instances of
...
Back to top- incompatible change: the #\` ("backquote") reader macro was reimplemented
...
Back to top- enhancement: GENCGC is enabled on ARM.
...
Back to top- bug fix: read-time-eval backquote context mixup. (#1321047)
...
Back to top- optimization: COERCE is now more effecient for more cases when the type is
...
Back to top- enhancement: printing backtraces respects
...
Back to top- minor incompatible change: improve read/print consistency for pathnames on
...
Back to top- new feature: the iterative spilling/coloring register allocator developed
...
Back to top- optimization: complicated TYPEP tests are less opaque to the type
...
Back to top- optimization: better distribution of SXHASH over small conses of related
values. (#309443)
...
Back to top- enhancement: Add sb-bsd-sockets:socket-shutdown, for calling
...
Back to top- enhancement: support building the manual under texinfo version 5.
(#1189146)
...
Back to top- enhancement: ASDF has been updated to 3.0.2.
...
Back to top- new feature: the contrib SB-GMP links with libgmp at runtime to speed
...
Back to top- notice: The implementation of MAP-ALLOCATED-OBJECTS (the heart of
...
Back to top- enhancement: TRACE :PRINT-ALL handles multiple-valued forms.
(#457053)
...
Back to top- enhancement: the continuable error when defknown-ing over extant
...
Back to top- minor incompatible change: SB-SPROF:WITH-PROFILING no longer loops
by default.
...
Back to top- optimization: LOOP expressions using "of-type character" have slightly
...
Back to top- enhancement: warnings about bad locale settings, LANG, LC_CTYPE, etc.
(#727625)
...
Back to top- notice: System requirements for SBCL on Microsoft Windows: Windows NT 5.1
...
Back to top- enhancement: WITH-COMPILATION-UNIT no longer grabs the world-lock.
...
Back to top- enhancement: New variable, sb-ext:*disassemble-annotate* for controlling
...
Back to top- enhancement: implicit generic function warnings now specify the package
...
Back to top- RANDOM enhancements and bug fixes:
...
Back to top- bug fix: fix copy-structure. When copying from stack to heap, garbage
...
Back to top- enhancement: on 64-bit targets, in src/compiler/generic/early-vm.lisp,
...
Back to top- enhancement: ASDF has been updated to version 2.017.
...
Back to top- minor incompatible change: SB-BSD-SOCKET socket streams no longer
...
Back to top- enhancement: errors from FD handlers now provide a restart to remove
...
Back to top- minor incompatible change: WITH-LOCKED-HASH-TABLE no longer disables
...
Back to top- incompatible change: SB!KERNEL:INSTANCE-LAMBDA, deprecated for over five
...
Back to top- bug fix: fix mach port rights leaks in mach exception handling code on
...
Back to top- enhancement: largefile support on Solaris.
...
Back to top- enhancement: ~/ and ~user/ are treated specially in pathnames.
...
Back to top- enhancement: RUN-PROGRAM accepts :EXTERNAL-FORMAT argument to select the
...
Back to top- incompatible change: FD-STREAMS no longer participate in the serve-event
event-loop by default. (#316072)
...
- bug fix: scripting and build for Solaris and FreeBSD. (#615497,
#627581, thanks to Josh Elsasser and Jim Wise)
...
Back to top- bug fix: readdir now works on :inode64 darwin builds (#592897)
...
Back to top- bug fix: Backtrace from undefined function on x86 and x86-64 now show
the calling frame.
...
Back to top- incompatible change: Thread names are now restricted to SIMPLE-STRINGs
...
Back to top- enhancement: Backtrace from THROW to uncaught tag on x86oids now shows
...
Back to top- new feature: SB-EXT:TYPEXPAND-1, SB-EXT:TYPEXPAND, and
...
Back to top- new port: support added for x86-64 NetBSD. (thanks to Aymeric Vincent)
...
Back to top- optimization: faster FIND and POSITION on strings of unknown element type
...
Back to top- improvement: stack allocation is should now be possible in all nested
...
Back to top- minor incompatible change: SB-THREAD:JOIN-THREAD-ERROR-THREAD and
...
Back to top- new port: support added for x86-64 OpenBSD. (thanks to Josh Elsasser)
...
Back to top- incompatible change: an interruption (be it a function passed to
...
Back to top- incompatible change: SB-INTROSPECT:FUNCTION-ARGLIST is deprecated, to be
...
Back to top- new feature: ARRAY-STORAGE-VECTOR provides access to the underlying data
...
Back to top- enhancement: when disassembling method functions, disassembly
...
Back to top- minor incompatible change: LOAD-SHARED-OBJECT no longer by default looks
...
Back to top- new feature: the compiler is able to track the effective type of a
...
Back to top- minor incompatible change: OPTIMIZE qualities
...
Back to top- new feature: user-customizable variable SB-EXT:*MUFFLED-WARNINGS*;
...
Back to top- minor incompatible change: SB-SPROF:WITH-PROFILING now by default
...
Back to top- temporary regression: user code can no longer allocate closure
...
Back to top- minor incompatible change: revert the changes to sb-posix's error
...
Back to top- enhancement: cleaner backtraces for interactive interrupts, as
...
Back to top- new feature: SB-EXT:*EXIT-HOOKS* are called when the process exits
...
Back to top- minor incompatible change: RUN-PROGRAM now uses execvp(3) to find
...
Back to top- new feature: MAKE-HASH-TABLE now experimentally accepts a
...
Back to top- incompatible change: hash-table accessor functions are no longer
...
Back to top- minor incompatible change: the MSI installer on Windows no longer
...
Back to top- minor incompatible change: SB-SYS:OUTPUT-RAW-BYTES is deprecated.
...
Back to top- enhancement: experimental macro SB-EXT:COMPARE-AND-SWAP provides
...
Back to top- MOP improvement: support for user-defined subclasses of
...
Back to top- new contrib: sb-cover, an experimental code coverage tool, is included
as a contrib module.
...
Back to top- incompatible change: removed writer methods for host-ent-name,
...
Back to top- new platform: experimental support for x86-64/darwin (MacOS).
...
Back to top- new platform: NetBSD/PPC. (thanks to Aymeric Vincent)
- optimization: calls of the form (AREF FOO (+ INDEX <constant>)) now
...
Back to top- improvement: experimental support for mach exception handling on
...
Back to top- new platform: FreeBSD/x86-64, including support for threading.
...
Back to top- improvement: experimental support for threading on FreeBSD/x86.
...
Back to top- enhancement: SB-POSIX now supports cfsetispeed(3), cfsetospeed(3),
...
Back to top- feature: weak hash tables, see MAKE-HASH-TABLE documentation
...
Back to top- feature: implemented the READER-METHOD-CLASS and
...
Back to top- added support for the ucs-2 external format. (contributed by Ivan
Boldyrev)
...
Back to top- feature: thread support on Solaris/x86, and experimental thread support
on OS X/x86.
...
Back to top- new feature: source path information is generated for macro-expansion
...
Back to top- minor incompatible change: in sbcl-0.9.11 (but not earlier
...
Back to top- new platform: experimental support for SBCL x86/Darwin, including
...
Back to top- new feature: new SAVE-LISP-AND-DIE keyword argument :EXECUTABLE can
...
Back to top- new platform: experimental support for the Windows operating
...
Back to top- minor incompatible change: (SETF CLASS-NAME) and (SETF
...
Back to top- bug fix: add a workaround to SBCL looping infinitely at startup on
...
Back to top- new feature: timers based on Zach Beane's excellent timer package
...
Back to top- new port: the Solaris operating system on x86 processors is now
...
Back to top- New feature: Experimental support for bivalent streams: streams
...
Back to top- numerous signal handling fixes to increase stability
...
Back to top- fixed cross-compiler leakages that prevented building a 32-bit
...
Back to top- incompatible change: the --noprogrammer option, deprecated since
...
Back to top- incompatible change: thread support for non-NPTL systems has
...
Back to top- fixed inspection of specialized arrays. (thanks to Simon Alexander)
...
Back to top- new port: SBCL now works in native 64-bit mode on x86-64/Linux
...
Back to top- new feature: reloading changed shared object files with
...
Back to top- new feature: a build-time option (controlled by the :SB-UNICODE
...
Back to top- enhancement: saving cores with foreign code loaded is now
...
Back to top- incompatible change: SB-INT:*BEFORE-SAVE-INITIALIZATIONS* and
...
Back to top- incompatible change: the internal functions
...
Back to top- new feature: SB-PACKAGE-LOCKS. See the "Package Locks" section of
...
Back to top- minor incompatible change: the system no longer provides
...
Back to top- minor incompatible change: the sb-grovel contrib now treats C
...
Back to top- Support for the forthcoming 2.0 version of the NetBSD kernel
...
Back to top- deprecation of old extension: *DEBUG-PRINT-LEVEL* and
...
Back to top- minor incompatible change: parsing of namestrings on a physical
...
Back to top- When built with the :SB-FUTEX feature, threaded builds now take
...
Back to top- fixed a bootstrapping bug: the build process no longer assumes
...
Back to top- New code in contrib/sb-introspect (still probably not entirely
...
Back to top- incompatible change: The --disable-debugger command line
...
Back to top- SBCL now builds and runs on MacOS X (version 10.2), or perhaps
...
Back to top- fixed bug 148: failure to inline-expand a local function left
...
Back to top- minor incompatible change: some nonsensical specialized lambda
...
Back to top- SBCL now builds using CLISP (version of late April 2003 from CVS) as
...
Back to top- experimental native threads support (on x86 Linux >=2.4 only).
...
Back to top- a better implementation of SXHASH on (simple) bit vectors,
...
Back to top- incompatible packaging change: in line with Unix convention,
...
Back to top- minor incompatible change: code processed by the "interpreter" or
...
Back to top- fixed bug 127: DEFSTRUCT now does not clobber old structure
...
Back to top- Support for building SBCL for MIPS platforms running in
...
Back to top- minor incompatible change: The runtime (the Unix executable named
...
Back to top- A beta-quality port to the mips architecture running Linux,
...
Back to top- An alpha-quality port to the parisc architecture running Linux,
...
Back to top- bug fix: Floating point exceptions are treated much more
...
Back to top- SBCL now builds with OpenMCL (version 0.12) as the
...
Back to top- bug 147 fixed: The compiler preserves its block link/count
...
Back to top- ANSI's DEFINE-SYMBOL-MACRO is now supported. (thanks to Nathan
...
Back to top- incompatible change: The compiler is now less aggressive about
...
Back to top- major incompatible change: The default fasl file extension, i.e. the
...
Back to top- a port to the Compaq/DEC Alpha CPU, thanks to Dan Barlow
...
Back to top- incompatible change: The old SB-EXT:OPTIMIZE-INTERFACE declaration
...
Back to top- Martin Atzmueller pointed out that bugs #9 and #25 are gone in
current SBCL.
...
Back to top- A patch from Martin Atzmueller seems to have solved the SIGINT
...
Back to top- DESCRIBE now works on CONDITION objects.
...
Back to top- The system is now under CVS at SourceForge (instead of the
...
Back to top- The system has been ported to OpenBSD.
...
Back to top- DESCRIBE no longer tries to call itself recursively to describe
...
Back to top- Raymond Wiker's patches to port the system to FreeBSD have been merged.
...
Back to top- There is now a partial SBCL user manual (with some new text and some
...
Back to top- The system still can't cross-compile itself with
...
Back to top- (Note that the way that the PCL macroexpansions were rewritten
...
Back to top- changed build optimization from (SAFETY 1) to (SAFETY 3) as a short-term
...
Back to top- tidied up "make.sh" script
