...
created: '2026-06-04'
description: 'Article URL: https://www.copetti.org/writings/consoles/playstation/
  Comments URL: https://news.ycombinator.com/item?id=48382142 Points: 255 # Comments:
  50'
tags:
- clippings
...
tldr: 索尼 PlayStation 的 CPU 采用基于 MIPS R3000A 架构的 LSI CW33000 核心，运行频率 33.87 MHz，配备 32
  位数据总线和地址总线。系统包含 Geometry Transformation Engine（GTE）几何变换引擎和 Motion Decoder（MDEC）运动解码器两个专用协处理器，通过
  DMA 控制器实现高速数据传输，但未配备浮点运算单元（FPU）。
objective_summary: 本文详细拆解了索尼 PlayStation（PS1）的 CPU 架构及其历史背景。CPU 核心基于 MIPS R3000A 指令集架构（MIPS
  I），由索尼委托 LSI Logic 使用其 CoreWare 项目定制，最终在主板上的芯片型号为 CXD8530BQ。该 CPU 运行于 33.87 MHz，具备
  5 级流水线、4 KB 指令缓存和 1 KB Scratchpad（替代数据缓存），提供 2 MB EDO RAM。系统通过 DMA 控制器实现 CD-ROM、MDEC、GPU
  和 SPU 等子系统间的高速数据搬运。协处理器方面，CP0 负责系统控制（缓存、中断、异常），CP2（GTE）用于加速 3D 图形所需的向量和矩阵运算，MDEC
  则用于解码全动态视频（FMV），但缺少 CP1（浮点运算单元）。
...
entities:
  ...
  - Linus Torvalds
key_logic_flow:
- 20 世纪 90 年代初 CPU 市场从 CISC 向 RISC 架构转型，MIPS、PowerPC、SPARC、ARM 等新架构处理器开始进入主流设备。
- MIPS Computer Systems 由斯坦福大学团队创立，其 R2000 被视为首个商用 RISC 处理器，后被 Silicon Graphics Incorporated（SGI）收购并转型为
  IP 授权模式。
- 索尼需要一款性价比高的 CPU 来驱动其自研图形和音频芯片，最终选择了 MIPS 的低端产品线 R3000A 架构。
- 索尼委托 LSI Logic 使用其 CoreWare 项目中的 CW33000 核心定制 CPU 包，最终产品为 CXD8530BQ 芯片，与 MIPS R3000A
  指令集二进制兼容。
- CPU 运行频率为 33.87 MHz，采用 MIPS I 指令集、32 位数据总线和地址总线、5 级流水线和 4 KB 指令缓存，但无数据缓存（以 1 KB
  Scratchpad 替代），且未配备浮点运算单元（FPU）。
- 系统配备 DMA 控制器实现 CD-ROM、MDEC、GPU 和 SPU 间的高速数据传输，提供三个协处理器：CP0 系统控制协处理器、CP2（GTE）几何变换引擎和
  Motion Decoder（MDEC）运动解码器。
extract_result: success
object_mentions:
- object_type: product
  name: PlayStation
  canonical_name: Sony PlayStation
  url: https://www.copetti.org/writings/consoles/playstation/
  confidence: high
  article_role: primary_subject
  evidence_snippets:
  - 本文对索尼 PlayStation 游戏主机的 CPU 架构及相关协处理器进行了完整的硬件拆解与技术分析。
  article_id: 5ed299ec772151d7
- object_type: product
  name: CXD8530BQ
  canonical_name: Sony CXD8530BQ
  url: null
  confidence: high
  article_role: primary_subject
  evidence_snippets:
  - CXD8530BQ 是本文主要拆解对象，即索尼 PlayStation 主板上的 SoC 主处理器芯片，基于 LSI 的 CW33000 核心定制。
  - 索尼委托 LSI Logic 使用 CoreWare 项目中的 CW33000 核心，结合其他模块形成了 PlayStation 主板上的 CXD8530BQ
    芯片。
  article_id: 5ed299ec772151d7
- object_type: product
  name: MIPS R3000A
  canonical_name: MIPS R3000A
  url: null
  confidence: high
  article_role: mentioned_reference
  evidence_snippets:
  - MIPS R3000A 是 MIPS 公司低端 CPU 产品线的一员，LSI 的 LR33300 和 CW33300 核心与其二进制兼容。
  - 索尼最终选择 MIPS R3000A 架构作为 PlayStation 的 CPU 基础，因其在性能与成本之间取得了平衡。
  article_id: 5ed299ec772151d7
- object_type: project
  name: CoreWare
  canonical_name: LSI CoreWare
  url: null
  confidence: medium
  article_role: ecosystem_context
  evidence_snippets:
  - LSI Logic 提供名为 CoreWare 的定制 CPU 服务，使客户可从一系列构建模块中组装自定义 CPU 包。
  - CoreWare 库中包含 CW33300 核心模块，该核心源自 LSI LR33300 商业 CPU 芯片，与 MIPS R3000A 指令集兼容。
  article_id: 5ed299ec772151d7
- object_type: product
  name: Geometry Transformation Engine (GTE)
  canonical_name: PlayStation GTE
  url: null
  confidence: high
  article_role: primary_subject
  evidence_snippets:
  - GTE（CP2）是 PlayStation 的专用数学协处理器，用于加速 3D 图形所需的向量矩阵乘法、透视变换、光照计算和深度插值等运算。
  - GTE 采用定点数运算而非浮点运算，负责图形管线的初始阶段（3D 投影、光照和裁剪），为 GPU 生成渲染所需的数据。
  article_id: 5ed299ec772151d7
- object_type: product
  name: Motion Decoder (MDEC)
  canonical_name: PlayStation MDEC
  url: null
  confidence: high
  article_role: primary_subject
  evidence_snippets:
  - MDEC 是 PlayStation 的运动解码器，负责将压缩的宏块解压为 GPU 可处理的格式，每秒可解码 9000 个宏块。
  - MDEC 支持 8x8 像素、24 bpp 的位图解压，通过 DMA 实现从 CD-ROM 到内存再到 VRAM 的流式数据传输，支持 320x240 分辨率
    30 fps 的全动态视频播放。
  article_id: 5ed299ec772151d7
impact_score:
  score: 1.5
  reason: 本文是一篇关于初代PlayStation（1994年产品）硬件架构的回顾性技术分析文章，发布于社区讨论平台。该事件并非AI行业的新产品、研究成果或商业动态，而是对30年前消费电子产品的技术考古。对AI行业的短期影响力几乎为零，仅对硬件架构和游戏主机开发史感兴趣的极少数技术人有参考价值。评分依据：文章本身无时效性新闻价值，属于小众技术怀旧内容。
sentiment: neutral
developer_sentiment:
  tone: excited
  primary_focus: 初代PlayStation硬件架构设计的精妙之处与工程权衡，特别是MIPS R3000A定制SoC方案和协处理器设计
hype_assessment:
  level: low
  reason: 本文是严谨的硬件架构分析，引用了芯片型号（CXD8530BQ、CW33000）、总线宽度、主频等具体技术参数，附有历史背景考证和图片来源说明，完全没有使用'颠覆''革命性'等PR包装语言，属于实打实的干货内容。
information_entropy: high
domain_disruption:
  technical_innovation: 无。本文是对已成历史的PlayStation硬件架构的复盘分析，不涉及任何当前技术领域的突破。文中描述的SoC集成（CPU+GTE+MDEC）在1994年确是创新，但对当下AI行业无破局意义。
  business_model: 无。纯技术考古内容，不涉及商业模式影响。
engineering_complexity: production_ready
compound_value:
  score: 1.5
  reason: 本文是初代PlayStation（1994年发布）硬件架构的历史技术回顾，属于技术考古类内容，而非前瞻性趋势或新兴商业机会。虽对理解90年代RISC处理器生态、定制SoC（CoreWare模式）和Sony早期硬件整合策略有一定知识价值，但不具备任何长期复利积累的投资属性——没有新的商业模式、未满足的市场需求或可资本化的技术突破。从VC视角看，该信息不指向任何可投资的未来方向，也不影响当前AI/半导体行业的资本配置决策。
value_capture_layer: hardware_compute
moat_impact: neutral
key_beneficiaries: []
competitive_casualty: []
market_opportunities:
- AI芯片创业公司可借鉴初代PlayStation的协处理器架构思路，在通用CPU之外设计专用的矩阵运算或稀疏推理协处理器，实现功耗与性能的平衡
- RISC-V等开放指令集生态下的AI加速器定制化服务存在市场空白，可效仿LSI CoreWare模式为客户提供可配置的AI SoC构建方案
- 边缘AI设备可参考PlayStation的SoC集成设计哲学，将推理引擎、视频编解码和传感器数据处理整合到单一低功耗芯片中
risk_matrix:
  regulatory: 无
  technological: 当前AI芯片大厂（如NVIDIA、AMD）的GPU生态壁垒深厚，借鉴90年代RISC架构思路的自研AI协处理器面临软件栈兼容性和开发者生态迁移的挑战
  competitive: 科技巨头如Google（TPU）、Apple（Neural Engine）、Amazon（Trainium/Inferentia）已深度布局自研AI芯片，新进入者在资金、流片经验和客户锁定方面面临巨大竞争压力
  ethical: 专用AI芯片的普及可能加剧算力垄断，中小企业和研究机构因无法承担定制芯片成本而在AI创新上进一步边缘化
  additional: []
confidence:
  impact: low
  compound: medium
...
- object_type: product
  ...
  canonical_name: Sony PlayStation
  url: https://www.copetti.org/writings/consoles/playstation/
  positioning: 索尼第一代家用游戏主机，以自研图形芯片和 MIPS R3000A 架构 CPU 为核心，在 90 年代中期以 3D 能力和性价比奠定了游戏机市场的霸主地位。
  ...
  - 游戏开发者
  - 计算机架构研究者
  product_signal: 搭载 33.87 MHz MIPS R3000A 架构 CPU，配备 GTE 几何变换引擎和 MDEC 运动解码器两个专用协处理器，通过
    DMA 实现 CD-ROM 到 GPU 的高速数据搬运。
  market_signal: 索尼自研图形和音频芯片并委托 LSI Logic 定制 CPU 包，以高度集成方案在 90 年代游戏机市场中以 3D 图形能力成功突围并占据主导地位。
  differentiation: 采用定点数运算的 GTE 协处理器替代浮点运算单元实现 3D 几何变换，在成本与性能间取得独特平衡，是消费级专用硬件加速的早期范例。
  watch_reason: PlayStation 的架构体现了在成本约束下通过专用协处理器实现 3D 图形加速的设计哲学，其 GTE 和 MDEC 的协处理器思路对理解现代
    GPU 固定功能管线和嵌入式 SoC 的演进具有重要的历史参考价值。
  risk_notes:
  - 该产品已是 1994 年发布的成熟设备，在当代 AI 领域的直接技术关联有限。
  - 文章仅聚焦硬件拆解和技术分析，缺乏市场销量和商业表现等数据支撑。
  score: 4.0
  article_ids:
  - 5ed299ec772151d7
  evidence_snippets:
...
- object_type: product
  ...
  canonical_name: Sony CXD8530BQ
  url: null
  positioning: 索尼为 PlayStation 定制的 SoC 主处理器芯片，基于 LSI CoreWare 项目的 CW33000 核心集成 CPU
    与系统控制器，是 PlayStation 的主计算单元。
  technical_signal: null
  adoption_signal: null
  ecosystem_relevance: null
  target_users:
  - 硬件工程师
  - 芯片设计研究者
  product_signal: 基于 MIPS R3000A 指令集架构运行于 33.87 MHz，配备 5 级流水线、4 KB 指令缓存和 1 KB Scratchpad，但未配备数据缓存和浮点运算单元。
  market_signal: null
  differentiation: 索尼委托 LSI Logic 利用 CoreWare 可定制 CPU 平台进行定制设计，结合自研模块形成高度集成的 SoC 方案，体现了
    90 年代消费电子定制芯片的典型实践。
  watch_reason: CXD8530BQ 代表了消费电子领域早期定制 SoC 的典型实践，展示了在成本约束下通过协处理器架构弥补 CPU 算力不足的设计思路，对理解现代嵌入式
    SoC 架构的演进有参考价值。
  risk_notes:
  - 该芯片已停产超过 20 年，缺乏后续迭代版本或现代应用场景。
  - 文章内容仅基于硬件逆向分析，缺乏官方设计文档和规格书的交叉验证。
  score: 3.0
  article_ids:
  - 5ed299ec772151d7
  evidence_snippets:
...
- object_type: product
  ...
  canonical_name: MIPS R3000A
  url: null
  positioning: MIPS 公司低端 RISC 处理器架构，基于 MIPS I 指令集，通过 IP 授权模式被广泛用于嵌入式设备和消费电子，是 PlayStation
    CPU 的技术基础。
  technical_signal: null
  adoption_signal: null
  ecosystem_relevance: null
  ...
  - 芯片架构研究者
  - 计算机体系结构学习者
  product_signal: 采用 32 位数据总线和地址总线、MIPS I 指令集，具备 5 级流水线和 4 KB 指令缓存，运行频率为 33.87 MHz，是早期商用
    RISC 设计的代表。
  market_signal: 通过 IP 授权模式被索尼选择用于 PlayStation，同时被 LSI Logic 等厂商作为定制芯片基础，在 90 年代嵌入式市场具有广泛影响力。
  differentiation: 作为 MIPS 低端产品线，以 IP 授权模式降低客户定制成本，使 RISC 架构从工作站下沉到消费电子产品，体现了芯片授权模式的市场适配能力。
  watch_reason: MIPS R3000A 通过 IP 授权模式进入 PlayStation 是 RISC 架构从工作站下沉到消费电子的标志性案例，对理解芯片商业模式演变和
    RISC 架构的历史影响具有参考价值。
  risk_notes:
  - MIPS 架构在消费电子市场的影响力已被 ARM 大幅取代，其当代商业价值有限。
  score: 2.0
  article_ids:
  - 5ed299ec772151d7
  evidence_snippets:
...
- object_type: product
  ...
  canonical_name: PlayStation GTE
  url: null
  positioning: PlayStation 的专用 3D 图形数学协处理器（CP2），采用定点数运算加速向量矩阵乘法、透视变换和光照计算，是早期消费级 3D
    硬件加速的关键组件。
  technical_signal: null
  adoption_signal: null
  ecosystem_relevance: null
  ...
  - 3D 图形开发者
  - 计算机架构研究人员
  product_signal: 采用定点数运算而非浮点运算，负责图形管线前期的 3D 投影、光照和裁剪计算，为 GPU 生成渲染所需数据，是 PlayStation
    实现 3D 图形的核心计算单元。
  market_signal: null
  differentiation: 以定点数协处理器替代浮点运算单元实现 3D 几何变换，在 90 年代中期以较低成本提供了可用的 3D 图形加速能力，体现了专用硬件弥补通用算力不足的设计哲学。
  watch_reason: GTE 作为早期消费级 3D 图形加速协处理器，其定点数运算和固定功能管线设计思路与现代 GPU 有着深刻的技术渊源，对理解 3D
    图形硬件演进和专用加速器设计具有重要意义。
  risk_notes:
  - GTE 采用定点数运算，精度有限且编程复杂度较高，与现代 GPU 浮点管线差异显著。
  score: 4.0
  article_ids:
  - 5ed299ec772151d7
  evidence_snippets:
...
- object_type: product
  ...
  canonical_name: PlayStation MDEC
  url: null
  positioning: PlayStation 的专用运动解码器协处理器，负责将压缩宏块解压为 GPU 可处理格式，通过 DMA 实现从 CD-ROM 到 VRAM
    的流式全动态视频播放。
  technical_signal: null
  adoption_signal: null
  ecosystem_relevance: null
  ...
  - 游戏开发者
  - 视频编解码研究者
  product_signal: 支持 8x8 像素、24 bpp 位图解压，每秒可解码 9000 个宏块，通过 DMA 流式传输实现从 CD-ROM 到 VRAM
    的数据搬运，支持 320x240 分辨率 30 fps 视频播放。
  market_signal: null
  differentiation: 作为专用视频解码协处理器，在 90 年代中期的消费级设备上首次实现了 CD-ROM 全动态视频播放，体现了专用媒体硬件在嵌入式系统中的早期应用价值。
  watch_reason: MDEC 代表了早期消费电子设备中专用媒体解码协处理器的典型实践，对理解现代 GPU 中视频编解码模块的演进脉络和专用硬件加速思想的起源有历史参考价值。
  risk_notes:
  - MDEC 仅支持有限分辨率和压缩格式，与当代视频编解码标准差距显著。
  score: 2.0
  article_ids:
  - 5ed299ec772151d7
  evidence_snippets:
...
**MIPS**: Adopted by Silicon Graphics Incorporated (targeting graphics workstations).**PowerPC**: Adopted by Apple (targeting desktop publishing).**SPARC**: Developed by Sun Microsystems (targeting servers and business workstations).**ARM**: Developed by Acorn, initially targeting the consumer market before expanding into PDAs, cell phones, and other embedded devices.- … and many more ‘microcontroller’ chips that had yet to be finalised or adopted by a major industry - such as the
**Hitachi’s SH**and**NEC’s V810**. To their surprise, these were subsequently selected for the Sega Saturn and the Nintendo Virtual Boy, respectively.
...
[1]. I took this photo at the Computer History Museum (Mountain View, California), during my second visit in March 2025.
...
- The
**MIPS I**ISA: The first version of the MIPS instruction set architecture. Among many things, it uses**32-bit words**and includes multiplication and division instructions. **32 general-purpose registers**and**2 multiplication/division registers**: These are 32-bit as well. One general-purpose register (`R0`

) is hardwired to zero, a common trait in RISC designs.**32-bit data bus**: In the PS1, this bus branches into two:**Main Bus**(32-bit): Connects the MDEC and GPU.**Sub Bus**(16/8-bit): Connects the remaining components and I/O. This bus is bridged by the**Bus Interface Unit**, which also enables access to special ports of the GPU and SPU.

**32-bit address bus**: Enables access of up to 4 GB of physical memory. In other words, RAM, memory-mapped I/O, etc.**5-stage pipeline**: Allows up to five instructions to be processed simultaneously (see a previous article for a detailed explanation).**4 KB of instruction cache**: This can be ‘isolated’ as well, allowing the program to manipulate the instruction cache directly.- Oddly,
**there is no data cache**. The**1 KB of memory**normally reserved for it is mapped to a fixed address[4]. This area is called**Scratchpad**and it’s used as ‘fast SRAM’.
...
Wait, shouldn’t coprocessors only

expandCPU functions? Why is CP0 tightly coupled with the CPU?
...
The MDEC decompresses bitmaps consisting of 8x8 pixels at 24 bpp (bits per pixel). Overall, the MDEC can compute 9,000 macroblocks per second [5], enabling to stream a 320x240 px **Full-Motion Video** (FMV) at 30 frames per second.
...
**Control hazards**: Instructions may get executed when they shouldn’t.**Data hazards**: Instructions may operate with outdated data before it’s been updated.

`LW`

(load word from memory), `JAL`

(jump and link) and `BNE`

(branch on not equal) are each followed by a delay slot to prevent hazards. Instructions marked in red (found before address `800597C4`

) are fillers (useless instructions), while the rest marked in blue perform meaningful computations.Consequently, MIPS I CPUs exhibit the following behaviour:

**Any instruction following a ‘branch’ or ‘jump’ opcode is executed unconditionally**: Thus, developers have to manually fill the pipeline with modest instructions (such as`calculate 0 plus 0`

) after the branch or jump to mitigate the hazard. These fillers are called**branch delay slots**.- Modern CPUs converted this phenomenon into an advantage: Branch prediction. By adding circuitry to detect the hazard, the CPU can discard speculative computations if the branch or jump condition isn’t met. But if it holds true, then the CPU saves time.

**‘Load’ instructions don’t stall the pipeline until the retrieved data becomes available**: The second stage of the pipeline (called`RD`

or ‘Read and Decode’) gathers the operands that will be used during the third stage (`ALU`

)[10]. The fourth stage (`MEM`

, from ‘access MEMory’) looks for data in memory (e.g. main RAM or the CD reader). Now, here’s the problem: by the time a`load`

instruction gathers the data from outside, the next instruction already fetched the operands. Therefore, when an instruction depends on the data from a preceding`load`

, a filler must be inserted to ensure the correct operands are fetched on time.
...
One thing for sure is that, due to timing differences, later games (such as *Jet Moto 3*) display glitched graphics when run on VRAM-based systems. If you want to know the details, Martin Korth’s ‘Nocash PSX Specifications’ document the various timings and such [12].
...
[13]. Consequently, its camera only moves forwards or backwards.
...
**Triangles**are the most complex (and versatile) type, which can be textured and shaded.**Lines**are quicker to draw but naturally unsuitable for textured surfaces. Shading is still supported.**Rectangles**are also faster but can only fit a sprite of up to 256 x 256 pixels; larger rectangles will only duplicate the sprite’s graphic. Even so, they offer no affine transformation (aside from X/Y flipping), nor shading or effects. I suspect rectangles were only implemented to assist the development of 2D games.
...
- Analyse the triangle’s area to determine which pixels of the frame buffer they occupy. Only the portion of the polygon covering the
**sampling points**get turned into pixels.
...
**Flat shading**: Each primitive has a constant light level.**Gouraud shading**: Each vertex embeds its own light level. Then, the brightness is automatically interpolated between points.- As you can imagine, this type offers more realistic results than flat shading.
...
**Semi-transparency**: Simulates light passing through overlapping textures.**Dithering**: Softens sudden changes in colour while adhering to the same colour palette.
...
This is commonly tackled by implementing **sub-pixel resolution**, which allows the rasteriser to keep track of the fractions of pixels occupied by each triangle area [18]. As a result, anti-aliasing methods can then be applied to soften jagged edges or abrupt changes in colour.
...
[20], notice the reduction in ports over the years. From bottom to top: SCPH-1000 (1994), SCPH-3000 (1995), SCPH-5501 (1996), and SCPH-9001 (1999).

The first revision of this console carries a surprisingly wide range of video signals, accessible via the following ports:

**RFU DC**: Designed to connect an RF modulator.**RCA**: Outputs composite video and stereo audio.**S-Video**: Provides a combined Luma + Sync, and a separate Chroma signal.**AV Multi Out**: Offers all the aforementioned signals, excluding RFU. It additionally carries RGB and a 5+ Volts line.
...
**Pitch modulation**: As the name suggests, games can automatically alter the pitch of their samples instead of storing additional ones. This is useful for music sequencing.**Frequency modulation**: Channels can be assigned to modify the frequency of others. This is comparable to FM synthesis.**ADSR Envelope**: These are a set of parameters available for amplitude modulation.**Looping**: Instructs the system to play a piece of audio repeatedly.**Digital reverb**: Simulates the sample being played in a specific acoustic environment to enhance immersion.
...
- A
**DSP**: Controls the motor and laser, and processes the RF signal coming from the laser. - A
**Sub-CPU**: A CPU package made of a**Motorola 68HC05**microcontroller,**512 B of RAM**, and**16 KB of ROM**[21]. In a nutshell, the Sub-CPU runs a local program stored in ROM and controls the DSP. The Sub-CPU program implements copy-protection measures and they are enforced whether the main CPU ‘likes it or not’. - A
**CD Controller**: Serves as the middle-man between the CD subsystem and the rest of the console, receiving commands from main CPU (in a FIFO manner) and triggering interrupts after specific events. As a controller, the chip talks to the Sub-CPU and receives CD data from the DSP.- Furthermore, the controller bundles a DMA unit that is connected to the SPU, which allows it to stream audio directly.
...
The system includes a **512 KB ROM** that stores a ‘BIOS’. This program performs many services, including taking care of the startup process, displaying the user shell, and exposing a collection of I/O routines [24].
...
The CPU’s reset vector is located at `0xBFC00000`

, which points to the BIOS ROM.
...
**It is**: The controller will allow to read its contents. In turn, the CPU will look for a file named`SYSTEM.CNF`

(which specifies the location of the executable and some metadata), and continue execution from there.**It’s not**→ The CPU will display an error message.

- With no CD inserted, the CPU will display the
**shell**. - The user is now in control.
...
Just like the Sega Saturn and other consoles that made the switch to the CD format, PS1 games benefit from a new set of facilities: large storage (620 MB, in the case of the PS1), rich audio quality and a ‘not-so-slow’ read speed thanks to its 2x drive.

If you are curious about how Compact Discs (CDs) work, I’ve written a brief explanation in the Sega Saturn article.
...
This is what Sony based their copy protection on: the Sub-CPU only reads discs whose particular track - the Table of Contents (TOC) - is engraved using a defined frequency, informally known as **Wobble Groove**. This frequency is introduced during the mastering process and cannot be replicated by conventional CD burners. The TOC resides in the inner section of the CD (called ‘Lead-In’ area) and instructs the laser on how to navigate throughout the disc [28]. Moreover, it is repeated multiple times as a fault-tolerance mechanism.

Within the game’s table of contents, one of the following character strings is embedded:

**SCEA**, meaning ‘Sony Computer Entertainment of America’.**SCEE**, meaning ‘Sony Computer Entertainment of Europe’.**SCEI**, meaning ‘Sony Computer Entertainment of Japan’.
...
Later on, Sony provided a library called **Libcrypt**, which fortified copy protection with the use of two approaches [29]:

- From the
**hardware side**, checksums of specific sectors were stored in sub-channels of the disc.- CD-ROM sub-channels traditionally store metadata, mostly to guide the drive. These are not user-accessible and conventional readers rarely allow to manually write over them.

- From the
**software side**, a set of routines were embedded at different points of the game. These fetch the checksum values and mix them with others to perform verifications, in an attempt to counteract both emulators and pirated discs.