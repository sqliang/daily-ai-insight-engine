...
tldr: OpenAI 发布 GPT-5.6 系列模型（Sol/Terra/Luna），以有限预览形式向信任合作伙伴开放
objective_summary: OpenAI 于 2026 年 7 月发布 GPT-5.6 系列模型，包含旗舰 Sol、平衡 Terra 和经济 Luna 三档。Sol
  在编码、生物学和网络安全基准上创下新 SOTA，引入 max 推理强度和 ultra 子代理模式。模型采用分层安全防护，投入超 70 万 A100 等效 GPU
...
entities:
  ...
  key_people: []
key_logic_flow:
- OpenAI 发布 GPT-5.6 系列三款模型：旗舰 Sol、平衡 Terra 和经济 Luna，Terra 性能与 GPT-5.5 相当但成本降低一半，Luna
  为最低价位最强能力。
- GPT-5.6 Sol 在 Terminal-Bench 2.1（命令行编码）、GeneBench v1（基因组学与定量生物学）和 ExploitBench（网络安全）等基准上达到新
  SOTA，且在 ExploitBench 上仅用 Mythos Preview 约 1/3 输出 token 即达到同等水平。
- GPT-5.6 Sol 引入了新的 max 推理强度和 ultra 模式（通过子代理加速复杂工作），其网络安全能力经评估未达到 Preparedness Framework
  定义的 Cyber Critical 阈值。
- OpenAI 部署了迄今最强的分层安全防护：模型级训练防护、实时输出分类器（可暂停生成并调用大模型审查）、账户级跨会话审查和差异化访问，并投入超 70 万 A100
  等效 GPU 小时进行自动化红队测试。
- 应美国政府要求，GPT-5.6 先以有限预览形式向信任合作伙伴开放，OpenAI 计划在未来数周内逐步向 ChatGPT、Codex 和 API 用户广泛开放。
- GPT-5.6 引入新命名体系（数字代表世代，Sol/Terra/Luna 代表能力层级），定价为 Sol $5/$30、Terra $2.50/$15、Luna
  $1/$6（每百万 token 输入/输出），并将在 Cerebras 平台实现最高 750 token/秒的推理速度。
