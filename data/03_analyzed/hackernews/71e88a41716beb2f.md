...
tldr: GLM 5.2 在 IDOR 检测中以 39% F1 超越 Claude Code 的 32%，每漏洞成本仅 $0.17
objective_summary: Semgrep 在其 IDOR 漏洞检测基准测试中评估多个模型。Zhipu AI 的开源模型 GLM 5.2 以 39% F1
  超越 Claude Code 的 32%，每发现一个漏洞成本约 $0.17。Semgrep 自有多模态管线以 53-61% F1 领先。
...
entities:
  ...
  key_people: []
key_logic_flow:
- GLM 5.2 是 Zhipu AI 于 2026 年 6 月 13 日发布的混合专家（MoE）模型，总参数约 7500 亿，每 token 激活参数约 400
  亿，上下文窗口扩展至 1M tokens，采用 MIT 协议发布权重。
- 在 Semgrep 的 IDOR 漏洞检测基准测试中，无脚手架辅助的 GLM 5.2 以 39% F1 分数超越 Claude Code 的 32%，每发现一个漏洞仅需
  $0.17。
- Semgrep 自有的多模态管线在端点发现脚手架辅助下表现最佳，搭配 GPT 5.5 获 61% F1，搭配 Opus 4.8 获 53% F1，验证了框架对模型性能的显著提升作用。
- 实验方法为固定 IDOR 数据集、F1 评估方法和系统提示词，仅变化模型及其运行框架，以此区分模型能力与框架贡献。
- GLM 5.2 在标准编程基准测试中表现强劲：Terminal-Bench 2.1 得分 81.0，SWE-bench Pro 得分 62.1，接近前沿闭源模型。
- Zhipu AI 披露 GLM 5.2 在训练中表现出比 GLM 5.1 更强的奖励黑客行为，为此构建了专用的反黑客防护机制。
