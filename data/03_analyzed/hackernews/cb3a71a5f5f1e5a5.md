...
tldr: AMD 推出售价 $3999 的 Ryzen AI Halo 开发者迷你套件
objective_summary: AMD 发布 Ryzen AI Halo 迷你开发套件，搭载 Zen 5 Ryzen AI Max+ 395 处理器（16核/32线程）、128GB
  统一内存（256GB/s 带宽）和 Radeon 8060S 集成显卡，售价 $3,999.99。LTT Labs 实测其在 llama.
...
entities:
  ...
  key_people: []
key_logic_flow:
- AMD 推出 Ryzen AI Halo 迷你开发套件，基于 Ryzen AI Max+ 395 处理器（Zen 5，16核/32线程），配备 128GB LPDDR5x-8000
  统一内存（256GB/s 带宽）和 2TB M.2 SSD，售价 $3,999.99。
- 该设备搭载 Radeon 8060S 集成显卡（40 RDNA 3.5 计算单元）和 XDNA 2 NPU，预装 Windows 11 Pro 或基于 Debian
  13.4 的自定义 Linux 系统，通过 ROCm/HIP 或 Vulkan 后端运行 AI 推理。
- LTT Labs 使用 llama.cpp 的 llama-bench 工具对 Qwen 3.6 35B A3B、Gemma 4 31B IT 和 GLM 4.7
  Flash 模型进行默认测试和变长上下文性能测试。
- 与 Framework Desktop（同款处理器）、M2 Ultra（76核GPU）Mac Studio 和 M3 Ultra（80核GPU）Mac Studio
  对比，Apple Silicon 设备因内存带宽更高（最高 819 GB/s）在 token 生成测试中领先 2-3 倍。
- 增加上下文长度时所有模型推理性能显著下降，Gemma 4 在 Vulkan 后端、65536 token 上下文时 30 分钟内未完成测试。
- ROCm/HIP 与 Vulkan 后端在 Ryzen AI Max+ 395 上各有优劣，表现取决于模型架构、上下文大小和驱动优化，无明确胜出者。
