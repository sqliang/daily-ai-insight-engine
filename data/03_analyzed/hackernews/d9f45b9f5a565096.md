...
- object_type: project
  ...
  canonical_name: deepreinforce-ai/Ornith-1
  url: https://github.com/deepreinforce-ai/Ornith-1
  positioning: deepreinforce-ai 发布的基于 Gemma 4 和 Qwen 3.5 后训练的自改进开源编程智能体模型系列，提供 9B
    到 397B 多种规格，以 MIT 许可证面向全球开放。
  technical_signal: 采用强化学习框架同时优化解决方案生成和驱动解决方案的 scaffold，通过联合训练发现更好的搜索轨迹并生成更高质量的代码。
  adoption_signal: 在 Terminal-Bench 2.1、SWE-bench Verified、NL2Repo 和 OpenClaw 等编程基准测试中达到同尺寸开源模型最优水平，397B
    版本 SWE-bench Verified 达 82.4%。
  ecosystem_relevance: 以 MIT 许可证全球开放无地域限制，支持 vLLM、SGLang 和 Transformers 多框架部署，提供 OpenAI
    兼容接口与 MCP 协议集成能力，降低社区使用门槛。
  target_users: []
  product_signal: null
  market_signal: null
  differentiation: null
  watch_reason: Ornith-1.0 通过强化学习自改进框架同时优化 scaffold 和解决方案，在多项编程基准中达到开源模型最优水平，代表了开源编程智能体模型在
    Agentic Coding 领域的重要突破，值得跟踪其技术演进和生态发展。
  risk_notes:
  - 模型基于 Gemma 4 和 Qwen 3.5 后训练，底层能力受限于基础模型的上限和迭代节奏。
  - 397B 版本需多 GPU 节点部署，大规模部署成本较高，可能限制中小团队的广泛采用。
  - 作为新发布模型，长期稳定性和社区贡献生态尚未经过大规模生产环境验证。
  score: 8.0
  article_ids:
  - d9f45b9f5a565096
  evidence_snippets:
