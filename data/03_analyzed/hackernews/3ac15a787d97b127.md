...
tldr: OfficeCLI 开源工具，让 AI 代理一键读写 Office 文件
objective_summary: iOfficeAI 发布 OfficeCLI，一个开源命令行工具，允许 AI 代理通过单行代码创建、读取和编辑 Word、Excel
  和 PowerPoint 文件。内置 HTML 渲染引擎和 350+ Excel 函数自动求值引擎，无需安装 Office，以单个二进制文件跨平台运行。
...
entities:
  ...
  key_people: []
key_logic_flow:
- OfficeCLI 是一个开源命令行工具，让 AI 代理通过一条命令创建、读取和编辑 Microsoft Word、Excel 和 PowerPoint 文件。
- 该工具内置 HTML 渲染引擎，可将文档渲染为 HTML 或 PNG 格式，使 AI 代理能可视化文档内容，形成「渲染-查看-修正」闭环。
- OfficeCLI 以单个自包含二进制文件发布，内置 .NET 运行时，无需安装 Microsoft Office，支持 macOS、Linux 和 Windows
  平台。
- 工具内置 350 多个 Excel 函数自动求值引擎，写入公式后可直接读取计算结果，无需依赖 Office 进行重算。
- 安装方式包括 curl 脚本、Homebrew、npm 和 GitHub Releases 手动下载，安装后可自动检测并为 Claude Code、Cursor
  等 AI 编码代理安装 skill 文件。
impact_score:
  score: 5.5
