...
- object_type: project
  ...
  canonical_name: LinuxMD/linuxmd
  url: https://github.com/LinuxMD/linuxmd
  positioning: 将 Linux 内核移植到 Sega MegaDrive 游戏主机上运行的开源项目，依赖 Mega EverDrive 烧录卡突破硬件限制，在
    4MB 内存环境中实现完整内核启动。
  technical_signal: 在仅 4MB RAM 的 68000 平台上成功启动 Linux 7.1.0-rc6 内核，采用 erofs 只读文件系统和
    smolinit 最小初始化方案，展现了 Linux 在极端资源受限环境下的可移植性。
  adoption_signal: 项目处于早期阶段，主要面向复古计算爱好者和嵌入式 Linux 开发者，尚未观察到广泛的社区采用或贡献。
  ecosystem_relevance: 属于复古计算与嵌入式 Linux 交叉领域，对理解内核最小化配置和 m68k 架构移植有参考价值，与 EverDrive
    硬件生态紧密耦合。
  target_users: []
  product_signal: null
  market_signal: null
  differentiation: null
  watch_reason: 该项目成功将现代 Linux 内核移植到经典 16 位游戏主机上，展示了内核在极限资源下的适配能力，后续性能优化和功能扩展进展值得关注。
  risk_notes:
  - 当前运行速度极慢，与 EverDrive FIFO 的交互是主要瓶颈，实际可用性严重受限。
  - 依赖 Mega EverDrive 特定硬件功能，无该烧录卡则无法运行。
  score: 4.0
  article_ids:
  - 4c48e6469c8a0a85
  evidence_snippets:
...
- object_type: product
  ...
  canonical_name: Mega EverDrive
  url: https://krikzz.com/our-products/cartridges/
  positioning: 为 Sega MegaDrive 设计的多功能烧录卡，通过 SSF2 mapper 提供 4MB 额外 RAM、SD 卡文件加载和 USB
    串口通信能力，是运行自制软件和 Linux 系统的关键硬件。
  ...
  - 自制软件开发者
  - 嵌入式 Linux 爱好者
  product_signal: 通过 SSF2 mapper 提供硬件映射能力，集成 USB 串口（cdc_acm）、FIFO 通信通道和块设备驱动，作为游戏外设实现了远超原始主机的
    I/O 扩展能力。
  market_signal: 面向复古游戏发烧友和自制软件开发者的细分市场，由 krikzz 公司长期经营，在烧录卡品类中具有较高的品牌认知度。
  differentiation: 与普通烧录卡不同，SSF2 mapper 提供的 4MB 映射空间和寄存器扩展使其成为唯一能支持完整 Linux 内核在 MegaDrive
    上运行的商业化产品。
  watch_reason: 作为 LinuxMD 项目的核心硬件依赖，Mega EverDrive 超越了游戏烧录卡的常规用途，成为复古硬件与开源操作系统融合的关键使能器。
  risk_notes:
  - SSF2 mapper 为 krikzz 专有实现，存在硬件供应链和兼容性风险。
  score: 3.0
  article_ids:
  - 4c48e6469c8a0a85
  evidence_snippets:
