---
title: Gemini 3.6 Flash, 3.5 Flash-Lite, and 3.5 Flash Cyber
source: https://blog.google/innovation-and-ai/models-and-research/gemini-models/gemini-3-6-flash-3-5-flash-lite-3-5-flash-cyber/
author:
- '[[logickkk1]]'
published: '2026-07-21'
created: '2026-07-22'
manifest_dates:
- '2026-07-22'
description: 'https://console.cloud.google.com/agent-platform/publishers/g... Comments
  URL: https://news.ycombinator.com/item?id=48993414 Points: 688 # Comments: 524'
tags:
- clippings
extraction_status: success
pipeline_stage: fact_extracted
id: 09b59cb95c871d71
source_type: community_discussion
tldr: Google 发布 Gemini 3.6 Flash、3.5 Flash-Lite 和 3.5 Flash Cyber（集成于 CodeMender）三款模型；3.6
  Flash 在编码和多模态任务中效率提升且价格更低，3.5 Flash-Lite 以 350 tokens/s 的推理速度面向高吞吐生产流量，3.5 Flash
  Cyber 专用于代码安全漏洞检测与修复。
objective_summary: Google 于 2026 年 7 月通过官方博客宣布推出三款 Gemini 系列新模型。Gemini 3.6 Flash 在编码、知识工作和多模态任务上相比
  3.5 Flash 有显著提升，输出 token 消耗降低 17%，且定价更低。Gemini 3.5 Flash-Lite 是该系列最快模型，输出速度达 350
  tokens/s，定价极低，面向高吞吐代理工作流，在某些基准上甚至超越 Gemini 3 Flash。Gemini 3.5 Flash Cyber 基于 3.5
  Flash 微调，专注于网络安全漏洞检测与修复，通过 CodeMender 代理以有限访问试点计划向政府和信任合作伙伴开放。三款模型即日起通过 Gemini API、Google
  AI Studio、Android Studio、Gemini Enterprise 等渠道提供，同时 Gemini 3.5 Pro 正在测试中，Gemini
  4 已启动预训练。
event_type: application_landing
epistemic_status: pr_statement
entities:
  companies:
  - Google
  - Datacurve
  - Artificial Analysis
  - Hebbia
  - Harvey
  technologies:
  - Gemini 3.6 Flash
  - Gemini 3.5 Flash-Lite
  - Gemini 3.5 Flash Cyber
  - CodeMender
  - Gemini API
  - Google AI Studio
  - Android Studio
  - Gemini Enterprise Agent Platform
  - Gemini app
  - Gemini 3.5 Pro
  - Gemini 4
  key_people: []
key_logic_flow:
- Google 发布 Gemini 3.6 Flash，在编码、知识工作和多模态性能上相比 3.5 Flash 有显著提升，输出 token 消耗降低 17%，且定价更低。
- Google 发布 Gemini 3.5 Flash-Lite，输出速度达 350 tokens/s，定价为 $0.3/1M 输入和 $2.5/1M 输出，在代理工作流和编码任务上甚至超越
  Gemini 3 Flash。
- Google 发布 Gemini 3.5 Flash Cyber，基于 3.5 Flash 微调，专门用于网络安全漏洞检测与修复，通过 CodeMender 代码安全代理集成。
- Gemini 3.5 Flash Cyber 将仅限政府和信任合作伙伴通过有限访问试点计划获取，以防范双重用途风险。
- Gemini 3.5 Pro 正在与合作伙伴测试中，Gemini 4 已启动至今最雄心勃勃的预训练阶段。
- 3.6 Flash 和 3.5 Flash-Lite 即日起通过 Gemini API、Google AI Studio、Android Studio、Gemini
  Enterprise 和 Gemini app 等渠道提供。
object_mentions:
- object_type: product
  name: Gemini 3.6 Flash
  canonical_name: Gemini 3.6 Flash
  url: null
  confidence: high
  article_role: primary_subject
  evidence_snippets:
  - Gemini 3.6 Flash 是 Google 发布的主打工作负载模型，在编码、知识工作和多模态任务上相比 3.5 Flash 有显著提升。
  - 根据 Artificial Analysis Index，3.6 Flash 输出 token 消耗比 3.5 Flash 降低 17%，在 DeepSWE
    基准上 token 减少最高达 65%。
  - 3.6 Flash 定价为 $1.50/1M 输入 tokens 和 $7.50/1M 输出 tokens，比 3.5 Flash 更低，有助于降低代理任务的总体成本。
  article_id: 09b59cb95c871d71
- object_type: product
  name: Gemini 3.5 Flash-Lite
  canonical_name: Gemini 3.5 Flash-Lite
  url: null
  confidence: high
  article_role: primary_subject
  evidence_snippets:
  - Gemini 3.5 Flash-Lite 是 3.5 系列中速度最快的模型，输出速度达 350 tokens/s，定价为 $0.3/1M 输入和 $2.5/1M
    输出。
  - 3.5 Flash-Lite 在多个代理和编码评估上超越 Gemini 3 Flash，包括 SWE-Bench Pro（54.2% vs 49.6%）和
    OSWorld-Verified。
  - 该模型支持可配置的思考级别，开发者可针对高吞吐任务选择低延迟模式或处理多步子代理工作流。
  article_id: 09b59cb95c871d71
- object_type: product
  name: Gemini 3.5 Flash Cyber
  canonical_name: Gemini 3.5 Flash Cyber
  url: null
  confidence: high
  article_role: primary_subject
  evidence_snippets:
  - Gemini 3.5 Flash Cyber 基于 3.5 Flash 微调，专门用于发现和修复代码安全漏洞，定价低于更大模型。
  - 该模型通过 CodeMender 代码安全代理集成，使用多个 3.5 Flash Cyber 代理协作生成统一的安全报告。
  - 3.5 Flash Cyber 仅限政府和信任合作伙伴通过有限访问试点计划获取，以防止安全技术被滥用。
  article_id: 09b59cb95c871d71
- object_type: product
  name: CodeMender
  canonical_name: CodeMender
  url: null
  confidence: high
  article_role: ecosystem_context
  evidence_snippets:
  - CodeMender 是 Google 的代码安全代理，使用多个 3.5 Flash Cyber 代理协作生成统一的安全报告。
  - 3.5 Flash Cyber 将通过 CodeMender 向政府和信任合作伙伴提供，帮助防御者优先发现和修复关键漏洞。
  article_id: 09b59cb95c871d71
- object_type: product
  name: Google AI Studio
  canonical_name: Google AI Studio
  url: null
  confidence: high
  article_role: ecosystem_context
  evidence_snippets:
  - Gemini 3.6 Flash 和 3.5 Flash-Lite 即日起通过 Gemini API 在 Google AI Studio 中向开发者提供。
  article_id: 09b59cb95c871d71
- object_type: product
  name: Gemini Enterprise Agent Platform
  canonical_name: Gemini Enterprise Agent Platform
  url: null
  confidence: medium
  article_role: ecosystem_context
  evidence_snippets:
  - Gemini 3.6 Flash 通过 Gemini Enterprise Agent Platform 向企业客户提供，同时也在 Gemini Enterprise
    应用中可用。
  article_id: 09b59cb95c871d71
extract_result: success
impact_score:
  score: 6.5
  reason: Google 同时发布三款模型形成产品矩阵攻势：3.6 Flash 在编码和多模态上显著提升（MLE Bench 63.9% vs 49.7%、DeepSWE
    49% vs 37%）且输出 token 消耗降低 17%、价格更低；3.5 Flash-Lite 以 350 tokens/s 和极低定价（$0.3/$2.5
    per 1M）面向高吞吐生产流量，甚至在多个 agent 评测上超越 Gemini 3 Flash；3.5 Flash Cyber 切入代码安全垂直场景。整体是重要的产品组合更新，但所有改进均为
    Flash 系列的迭代优化而非范式突破，Gemini 4 尚在预训练阶段。短期冲击力属于'重要产品发布，改变局部竞争格局'级别。
sentiment: positive
developer_sentiment:
  tone: excited
  primary_focus: 3.5 Flash-Lite 的 350 tokens/s 超低延迟与极低定价组合对高吞吐代理工作流的成本效益冲击
hype_assessment:
  level: medium
  reason: 作为 Google 官方博客，存在一定的 PR 包装成分：引用 Artificial Analysis Index、Datacurve 等第三方评测为数据背书提升了可信度，但'最高效''显著提升'等定性表述仍带有营销色彩。3.5
    Flash Cyber 的'有限访问试点'则隐含安全能力可能被夸大的风险。整体有数据支撑但需独立验证。
information_entropy: high
domain_disruption:
  technical_innovation: 3.6 Flash 的核心改进在于 token 效率优化——在降低 17% 输出 token 消耗和更少推理步骤的同时提升质量，这种'更少
    token 更好效果'的架构调优方向具有工程示范价值；3.5 Flash-Lite 以 350 tokens/s 推理速度在 agentic 和 coding
    评测上超越更高阶的 Gemini 3 Flash，说明模型裁剪与推理效率优化的技术路线取得了实质性突破。
  business_model: 3.5 Flash-Lite 的定价（$0.3/$2.5 per 1M）将高吞吐 AI 代理的生产成本推向新低，可能加速 agentic
    工作流从实验阶段走向大规模生产部署；3.6 Flash 在同等或更好质量下降低每任务总成本，改变 LLM API 定价竞争格局，迫使竞争对手在性价比维度跟进。
engineering_complexity: production_ready
compound_value:
  score: 7.5
  reason: Google 本次发布的核心价值在于构建了覆盖多层次价格/性能点的模型矩阵策略：3.5 Flash-Lite 以 $0.30/$2.50 每百万
    token 的极致低价（350 tokens/s）瞄准高吞吐 agent 工作流，3.6 Flash 在编码和多模态任务上显著提升的同时降低输出 token
    消耗 17%，3.5 Flash Cyber 则切入高壁垒的代码安全垂直领域并捆绑 CodeMender 代理形成闭环。这种从通用到垂直、从低成本到高性能的阶梯式布局具备强复利效应——更低的推理成本驱动更多
    agent 部署，更多使用数据反哺模型优化，形成正向飞轮。同时 Gemini 4 已启动预训练，表明 Google 在基础模型上的长期持续投入。但需审慎的是：这是增量迭代而非范式突破，模型层的价格战正在压缩全行业利润率；3.6
    Flash 相比 3.5 Flash 的进步虽显著但仍属同代升级，且竞争对手（OpenAI、Anthropic）也在快速迭代。长期复利取决于 Google
    能否通过这一矩阵策略有效拉动 Google Cloud 市场份额并形成生态锁定。
value_capture_layer: foundation_model
moat_impact: democratizes_access
key_beneficiaries:
- Google
- Google Cloud
- AI 应用开发者
competitive_casualty:
- OpenAI
- Anthropic
- AI 代码安全初创公司
- 小型模型 API 提供商
market_opportunities:
- 开发者可基于 Gemini 3.6 Flash 的 17% 输出 token 节省和更低定价，重构现有 AI Agent 工作流的成本结构，将高頻次、多步骤的编码和知识任务迁移至该模型以显著降低运营成本
- 3.5 Flash-Lite 以 350 tokens/s 和极低定价（$0.3/$2.5 每百万 token）打开了高吞吐生产场景的商业空间，适合构建大规模文档处理、智能搜索和批量数据管线的
  AI 原生应用
- 3.5 Flash Cyber + CodeMender 的组合为网络安全赛道提供了专用模型 + Agent 框架的范例，安全厂商可借鉴此模式开发面向代码审计、漏洞自动修复的垂直
  AI 产品
risk_matrix:
  regulatory: 3.5 Flash Cyber 因双重用途风险仅限政府和信任合作伙伴试点，预示 AI 网络安全能力的监管门槛正在提高；同时 3.6 Flash
    内置的 CBRN 和网络攻击前沿安全护栏表明 Google 在主动迎合 AI 安全监管趋势
  technological: Google 以罕见的高频节奏迭代 Flash 系列（3.5 → 3.6，同时 3.5 Pro 在测、Gemini 4 已启动预训练），依赖单一模型
    API 的团队面临频繁迁移适配的技术债务和架构过时风险
  competitive: Google 以更低价格 + 更高效率的组合拳大幅压低推理成本线，对 OpenAI、Anthropic、Meta 等竞品形成直接价格战压力，依赖高利润率
    API 定价的第三方模型聚合商可能被挤出市场
  ethical: AI 驱动的代码安全修复能力若被滥用可大幅降低网络攻击门槛，尽管 Google 对 Cyber 模型采取访问限制，但能力本身的双重用途属性不可逆，长期存在扩散风险
  additional: []
confidence:
  impact: high
  compound: medium
  hype: medium
actionable_insight: deep_dive
object_insights:
- object_type: product
  name: Gemini 3.6 Flash
  canonical_name: Gemini 3.6 Flash
  url: https://blog.google/innovation-and-ai/models-and-research/gemini-models/gemini-3-6-flash-3-5-flash-lite-3-5-flash-cyber/
  positioning: Google 发布的主打工作负载模型，在编码、知识工作和多模态任务上相比 3.5 Flash 有显著提升，且输出 token 消耗降低
    17%。
  technical_signal: null
  adoption_signal: null
  ecosystem_relevance: null
  target_users:
  - 构建生产级 AI 代理的开发者
  - 需要高效编码和多模态能力的企业客户
  - 知识工作与文档分析场景的团队
  product_signal: 在 DeepSWE 上准确率达 49%（vs 37%），MLE Bench 达 63.9%（vs 49.7%），OSWorld-Verified
    达 83.0%，编码和多模态能力全面超越前代。
  market_signal: 定价 $1.50/1M 输入和 $7.50/1M 输出 tokens，输出 token 消耗降低 17%，以更低成本实现更高性能，代理任务性价比显著提升。
  differentiation: 以更低定价和更优 token 效率在编码、知识工作、多模态三大维度全面超越 3.5 Flash，是 Google Flash
    系列在代理工作流性价比上的重要迭代。
  watch_reason: Gemini 3.6 Flash 作为 Flash 系列主力模型，以更低成本和更高 token 效率在编码和多模态领域取得显著提升，其实际采用率和基准表现将直接影响代理工作流市场的成本效益标准，值得持续跟踪。
  risk_notes:
  - 性能数据主要来自 Google 自述，需第三方独立基准验证实际提升幅度。
  - Frontier Safety 增强机制可能在 CBRN 和网络攻防之外领域产生误拒绝。
  score: 7.0
  article_ids:
  - 09b59cb95c871d71
  evidence_snippets:
  - Gemini 3.6 Flash 是 Google 发布的主打工作负载模型，在编码、知识工作和多模态任务上相比 3.5 Flash 有显著提升。
  - 根据 Artificial Analysis Index，3.6 Flash 输出 token 消耗比 3.5 Flash 降低 17%，在 DeepSWE
    基准上 token 减少最高达 65%。
  - 3.6 Flash 定价为 $1.50/1M 输入 tokens 和 $7.50/1M 输出 tokens，比 3.5 Flash 更低，有助于降低代理任务的总体成本。
- object_type: product
  name: Gemini 3.5 Flash-Lite
  canonical_name: Gemini 3.5 Flash-Lite
  url: https://blog.google/innovation-and-ai/models-and-research/gemini-models/gemini-3-6-flash-3-5-flash-lite-3-5-flash-cyber/
  positioning: Google 3.5 系列中速度最快、性价比最高的模型，输出速度达 350 tokens/s，面向高吞吐生产流量和代理工作流场景。
  technical_signal: null
  adoption_signal: null
  ecosystem_relevance: null
  target_users:
  - 高吞吐生产流量的开发者
  - 代理搜索和文档处理场景的团队
  - 需要低成本扩展代理系统的企业
  product_signal: 输出速度达 350 tokens/s，在 SWE-Bench Pro（54.2%）和 OSWorld-Verified（74.0%）上超越
    Gemini 3 Flash，支持可配置思考级别适应不同工作负载。
  market_signal: 定价 $0.3/1M 输入和 $2.5/1M 输出 tokens，以极低价格和高吞吐量在代理和编码评估上超越前代旗舰模型，市场冲击力强。
  differentiation: 以行业极具竞争力的价格和 350 tokens/s 吞吐量在多项基准上超越 Gemini 3 Flash，可配置思考级别在低成本与高智能之间提供灵活选择。
  watch_reason: Gemini 3.5 Flash-Lite 以极低价格和 350 tokens/s 速度在多项基准上超越 Gemini 3 Flash，可能重塑低成本高吞吐
    AI API 市场的竞争格局，是代理规模化部署的关键变量。
  risk_notes:
  - 在需要复杂推理的高难度任务上能力天花板相对有限。
  - 极致低价策略能否持续吸引开发者生态并实现盈利有待验证。
  score: 7.0
  article_ids:
  - 09b59cb95c871d71
  evidence_snippets:
  - Gemini 3.5 Flash-Lite 是 3.5 系列中速度最快的模型，输出速度达 350 tokens/s，定价为 $0.3/1M 输入和 $2.5/1M
    输出。
  - 3.5 Flash-Lite 在多个代理和编码评估上超越 Gemini 3 Flash，包括 SWE-Bench Pro（54.2% vs 49.6%）和
    OSWorld-Verified。
  - 该模型支持可配置的思考级别，开发者可针对高吞吐任务选择低延迟模式或处理多步子代理工作流。
- object_type: product
  name: Gemini 3.5 Flash Cyber
  canonical_name: Gemini 3.5 Flash Cyber
  url: https://blog.google/innovation-and-ai/models-and-research/gemini-models/gemini-3-6-flash-3-5-flash-lite-3-5-flash-cyber/
  positioning: 基于 Gemini 3.5 Flash 微调的安全专用模型，通过 CodeMender 代理实现代码安全漏洞检测与修复，面向政府和信任合作伙伴。
  technical_signal: null
  adoption_signal: null
  ecosystem_relevance: null
  target_users:
  - 政府机构安全团队
  - 信任合作伙伴的网络安全团队
  - 需要代码安全审计的受信组织
  product_signal: 通过 CodeMender 代码安全代理集成，使用多个 3.5 Flash Cyber 代理协作生成统一安全报告，专注于代码漏洞检测与修复闭环。
  market_signal: 仅限政府和信任合作伙伴通过有限访问试点计划获取，暂不面向公开市场，是一种高度受限的安全 AI 产品发布策略。
  differentiation: 首个基于 Gemini Flash 系列微调的安全专用模型，通过多代理协作架构实现从漏洞发现到修复建议的完整闭环。
  watch_reason: Gemini 3.5 Flash Cyber 代表 Google 在 AI 安全垂直领域的重要探索，其有限政府合作模式和 CodeMender
    代理架构为安全 AI 产品的商业化提供了值得关注的新范本。
  risk_notes:
  - 有限访问试点计划限制了市场验证和产品迭代反馈循环。
  - 代码安全漏洞检测涉及双重用途风险，可能面临更严格监管。
  - CodeMender 代理架构相对于模型本身的差异化价值有待独立验证。
  score: 6.0
  article_ids:
  - 09b59cb95c871d71
  evidence_snippets:
  - Gemini 3.5 Flash Cyber 基于 3.5 Flash 微调，专门用于发现和修复代码安全漏洞，定价低于更大模型。
  - 该模型通过 CodeMender 代码安全代理集成，使用多个 3.5 Flash Cyber 代理协作生成统一的安全报告。
  - 3.5 Flash Cyber 仅限政府和信任合作伙伴通过有限访问试点计划获取，以防止安全技术被滥用。
---

# Introducing Gemini 3.6 Flash, 3.5 Flash-Lite, and 3.5 Flash Cyber

Developers and customers building production AI agents need higher token efficiency, lower latency, and more reliable performance. Our Flash series of models is built to meet the sweet spot of efficiency and quality to enable scaling agentic workflows. Building on Gemini 3.5 Flash, we’re introducing new Gemini models:

**3.6 Flash:**Our workhorse model that delivers better coding, knowledge work, and multimodal performance. According to the Artificial Analysis Index, it reduces output token usage by 17% compared to 3.5 Flash, and in some benchmarks like DeepSWE by Datacurve, we observe up to 65%, all at a lower cost per output token.**3.5 Flash-Lite:**Our fastest, most cost-effective 3.5-class model, delivering 350 output tokens per second according to the Artificial Analysis Index, also significantly outperforming prior Flash-Lite generations in agentic workflows.**3.5 Flash Cyber in CodeMender:**Successful cybersecurity applications require careful orchestration of a model alongside an agent infrastructure. We’re introducing a combination of a new, highly efficient, specialized cyber-focused model paired with our CodeMender code security agent that delivers competitive performance at the frontier.

Beyond today’s releases, Gemini 3.5 Pro is currently testing with partners and we plan to make it broadly available as soon as it’s ready. In parallel, our team is already focusing on building the next generation of models. We have started our most ambitious pre-training run yet, for Gemini 4, and are excited by the progress.

## 3.6 Flash: More efficient and better quality than 3.5 Flash

Gemini 3.6 Flash builds directly on developer and customer feedback from 3.5 Flash. 3.6 Flash not only delivers a step up in coding and knowledge work, but it does this while meaningfully improving token efficiency. For example, on the Artificial Analysis Index, we see 3.6 Flash consuming 17% fewer output tokens than 3.5 Flash. It also takes fewer reasoning steps and tool calls to accomplish multi-step workflows.

This enhanced efficiency is also combined with a lower price than 3.5 Flash. At $1.50/1M input tokens and $7.50/1M output tokens, 3.6 Flash reduces the overall cost per agentic task, making agents more cost-effective to build and run.

3.6 Flash shows better token efficiency and reduced verbosity than 3.5 Flash in an OSWorld verified task (API)

Even while being more efficient, 3.6 Flash sees performance gains compared to 3.5 Flash across use cases:

- 3.6 Flash delivers higher precision with fewer unwanted code edits and reduced execution loops, as seen in DeepSWE (49% vs. 37%), and shows significant improvement in ML Research, as seen in MLE Bench (63.9% vs. 49.7%).
- It has improved computer use capabilities as seen in OSWorld-Verified (83.0% vs. 78.4%). Computer use is now a built-in client side tool via the Gemini API and Gemini Enterprise.
- It outperforms 3.5 Flash in knowledge work, as shown by benchmarks like GDPval-AA v2 (1421 vs. 1349). Customers like Hebbia and Harvey have found it particularly capable at multimodal tasks like document parsing, chart and data analysis, and report drafting.

Customers report 3.6 Flash is a step forward in both cost and quality, balancing token efficiency, accuracy, and speed across complex workflows and knowledge-based tasks:

## Built with safety

3.6 Flash is shipping with enhanced Frontier Safety safeguards in the domains of Chemical, Biological, Radiological, and Nuclear (CBRN) and cyber offense misuses. These safeguards make the model substantially more resistant to jailbreaks. At the same time, the model has been trained to minimize refusals for beneficial uses.

For more information, see the 3.6 Flash model card.

## 3.5 Flash-Lite: Built to scale agentic workflows

Beyond Flash, we’re also releasing Gemini 3.5 Flash-Lite, designed for both low-latency tasks and tasks where high throughput is critical for developers workflows, like agentic search and document processing.

3.5 Flash-Lite is the fastest model in the 3.5 series. As measured by Artificial Analysis, it runs at 350 output tokens/s. Priced at $0.3/1M input tokens and $2.5/1M output tokens and with significantly better quality than 3.1 Flash-Lite, 3.5 Flash-Lite offers a strong price-to-performance ratio for developers and customers running high throughput production traffic.

3.5 Flash-Lite executes high volume tasks at a lower latency than 3.5 Flash.

3.5 Flash-Lite enables efficient scaling for agentic systems. Across thinking levels, the model significantly outperforms 3.1 Flash-Lite. Depending on the workload, developers can configure the model to prioritize low-latency, low-cost execution for high-volume tasks with the minimal and low thinking levels, or engage higher thinking levels to process multi-step subagent workloads. The model now also has computer use as a built-in tool to reliably support these agentic tasks across surfaces.

It’s a significant step up in coding and agentic tasks as seen in Terminal-Bench 2.1 (54% vs 31%), long context as seen in GDM-MRCR v2 (72.2% vs. 60.1%), and real-world task execution as seen in GDPval-AA v2 (1140 vs. 642).

In fact, on many agentic and coding evals, 3.5 Flash-Lite even outperforms 3 Flash, including on SWE-Bench Pro (54.2% vs. 49.6%) and OSWorld-Verified (74.0% vs. 65.1%), making it a faster & more capable option for workloads on both 2.5 and 3 Flash.

Early customers of 3.5 Flash-Lite are highlighting its unique combination of speed, intelligence, and cost efficiency for scaling agentic workflows and data processing tasks:

For more information about the model, see the 3.5 Flash-Lite model card.

## 3.5 Flash Cyber in CodeMender: finding and fixing vulnerabilities efficiently

AI models have become capable of finding security vulnerabilities faster than current systems can fix them. Tackling this growing threat requires an approach to securing software that is highly capable and efficient.

Flash’s performance and efficiency makes it an ideal foundation to detect, validate, and patch code security issues at scale. Gemini 3.5 Flash Cyber is built on top of 3.5 Flash, and fine-tuned for finding and fixing cybersecurity vulnerabilities at a lower price per token than larger models.

Within CodeMender, which uses multiple 3.5 Flash Cyber agents working together to produce a single combined report, 3.5 Flash Cyber reaches competitive performance at the frontier on the popular benchmark CyberGym.

Given the dual-use nature of this technology, we have taken an intentional approach to deploying 3.5 Flash Cyber. The model will be exclusively available to governments and trusted partners via CodeMender soon as part of a limited-access pilot program. This will give frontline defenders a head start in finding and fixing critical vulnerabilities before they can be exploited, while mitigating against broader misuse.

## 3.6 Flash and 3.5 Flash-Lite: Get started today

3.6 Flash and 3.5 Flash-Lite are available starting today:

- For developers in the Gemini API via Google AI Studio and Android Studio. 3.6 Flash is also available in Google Antigravity. Get started with the Developer Guide.
- For enterprises in Gemini Enterprise Agent Platform. 3.6 Flash is also available in the Gemini Enterprise app.
- For everyone via the Gemini app. 3.5 Flash-Lite is also rolling out in Google Search.

As you start building with 3.6 Flash and 3.5 Flash-Lite, we welcome your feedback to improve future Gemini models and look forward to releasing 3.5 Pro soon.