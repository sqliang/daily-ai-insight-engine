...
tldr: zeroserve 是一款零配置 HTTPS 服务器，将 eBPF 程序作为唯一配置和中间件，在单核 HTTPS 场景下静态服务和动态中间件性能均超过
  nginx 和 Caddy，支持 TLS 1.3、ECH 和 OIDC 登录等高级功能。
objective_summary: zeroserve 的作者发布了一款零配置的 HTTPS 服务器，支持将网站打包为单一 tar 文件直接部署，并通过 eBPF
  程序进行请求处理的完全脚本化。该服务器所有 I/O 通过 io_uring 提交，采用单线程事件循环模型；eBPF 脚本在用户态通过 async-ebpf 运行时
  JIT 编译为本地代码运行，无需内核 BPF 子系统或 CAP_BPF 权限，并通过指针笼机制保证内存安全。基准测试显示，在单核 HTTPS 场景下，zeroserve
  在小文件静态服务性能上比 nginx 快约百分之十七，在动态中间件处理上也比 nginx Lua 方案快约百分之五十。该服务器内置 TLS 1.3、Encrypted
  Client Hello、SNI 证书选择、JA4 指纹识别和 OIDC 认证等高级功能，支持反向代理和热重载部署。
...
entities:
  ...
  key_people: []
key_logic_flow:
- zeroserve 是一款零配置 HTTPS 服务器，以单一 tar 包部署整个网站，直接从 tar 包中索引文件并通过字节范围读取提供服务，无需解压到磁盘。
- eBPF 脚本替代传统配置文件，在用户态通过 async-ebpf 运行时 JIT 编译为本地代码运行，由指针笼机制保证内存安全，无需内核 BPF 子系统或 CAP_BPF
  权限。
- 脚本采用链式执行模型，按文件名排序顺序运行，共享每请求元数据映射，支持通过 zs_respond 或 zs_reverse_proxy 短路以及抢占式调度。
- 基准测试表明在单核 HTTPS 场景下，zeroserve 在 174B 小文件静态服务上达到 36,681 req/s，比 nginx 快约 17%，在动态中间件处理上也比
  nginx Lua 方案快约 50%。
- 脚本内置丰富的辅助函数库，支持请求检查与修改、SHA-256 哈希、JSON 解析、令牌桶限速、AWS SigV4 签名和 OIDC 登录认证（Authorization
  Code + PKCE）。
- 服务器内置 TLS 1.3、Encrypted Client Hello、SNI 证书选择、JA4 指纹识别和透明 ECH 中继模式，所有网络和磁盘 I/O 通过
  io_uring 提交。
extract_result: success
object_mentions:
- object_type: project
  name: zeroserve
  canonical_name: zeroserve
  url: null
  confidence: high
  article_role: primary_subject
  evidence_snippets:
  - zeroserve 是作者发布的一款零配置 HTTPS 服务器，支持将网站打包为单一 tar 文件直接部署，从 tar 包中索引文件并直接通过字节范围读取提供服务，无需解压到磁盘。
  - 该服务器通过 eBPF 程序实现请求处理的完全脚本化，eBPF 脚本在用户态通过 async-ebpf 运行时 JIT 编译为本地代码运行，无需内核 BPF
    子系统或 CAP_BPF 权限。
  - 基准测试显示在单核 HTTPS 场景下，zeroserve 在小文件静态服务方面比 nginx 快约百分之十七，在动态中间件处理上比 nginx Lua 方案快约百分之五十。
  article_id: dc3275f4556dcc3e
- object_type: project
  name: async-ebpf
  canonical_name: async-ebpf
  url: null
  confidence: medium
  article_role: mentioned_reference
  evidence_snippets:
  - zeroserve 使用 async-ebpf 运行时在用户态加载 eBPF 字节码，通过 uBPF 的 JIT 编译为本地机器码执行，并通过指针笼机制保证内存安全。
  - async-ebpf 在普通无特权进程中运行，使 eBPF 脚本无需内核 BPF 子系统或 CAP_BPF 权限即可在用户态沙箱中执行。
  article_id: dc3275f4556dcc3e
impact_score:
  score: 4.5
...
- object_type: project
  ...
  canonical_name: zeroserve
  url: null
  positioning: zeroserve 是一款零配置 HTTPS 服务器，以 eBPF 脚本替代传统配置文件作为唯一配置方式，支持单 tar 包部署和热重载，旨在替代
    nginx 和 Caddy。
  technical_signal: 所有网络和磁盘 I/O 通过 io_uring 提交，采用单线程事件循环模型；eBPF 脚本在用户态通过 async-ebpf
    运行时 JIT 编译为本地代码运行，无需内核 BPF 子系统或 CAP_BPF 权限。
  adoption_signal: null
  ecosystem_relevance: zeroserve 将 eBPF 从内核可观测性领域引入 Web 服务器配置，挑战 nginx 和 Caddy 的声明式配置范式，但目前该项目尚无社区验证和生态支持。
  target_users: []
  product_signal: null
  market_signal: null
  differentiation: null
  watch_reason: zeroserve 提出了一种将 eBPF 作为 Web 服务器唯一配置语言的创新范式，在单核 HTTPS 场景下性能超越 nginx
    和 Caddy，内置 TLS 1.3、ECH、OIDC 等高级功能，但其社区发展和生产环境成熟度有待验证，值得持续跟踪。
  risk_notes:
  - 项目目前仅由个人作者开发维护，缺乏社区贡献者和第三方生态支持，生产环境可靠性尚未验证。
  - eBPF 脚本化配置模型学习曲线陡峭，相比传统声明式配置文件更难理解和调试。
  - 单线程事件循环模型在多核场景下扩展能力受限，需要依赖多进程扩展。
  score: 7.0
  article_ids:
  - dc3275f4556dcc3e
  evidence_snippets:
...
- object_type: project
  ...
  canonical_name: async-ebpf
  url: null
  positioning: async-ebpf 是一款用户态 eBPF 运行时，支持在无特权进程中加载和 JIT 编译 eBPF 字节码，为 zeroserve
    提供无需内核 BPF 子系统的脚本执行能力。
  technical_signal: async-ebpf 通过 uBPF 实现 eBPF 字节码的用户态 JIT 编译为本地机器码，以指针笼机制替代内核验证器保证内存安全，并支持运行时抢占式调度。
  adoption_signal: null
  ecosystem_relevance: async-ebpf 作为 zeroserve 的底层运行时，构成了用户态 eBPF 执行生态的基础组件，但其生态影响力目前完全依附于
    zeroserve 项目。
  target_users: []
  product_signal: null
  market_signal: null
  differentiation: null
  watch_reason: async-ebpf 实现了一种无需内核 BPF 子系统的用户态 eBPF 执行方案，其指针笼内存安全机制和抢占式调度设计为解决内核依赖问题提供了新思路，但在生态独立性和项目成熟度方面仍存在较大不确定性。
  risk_notes:
  - async-ebpf 目前完全依赖于 zeroserve 项目，缺乏独立生态系统和社区支持。
  - 用户态 eBPF 运行时的性能和安全性在大规模生产环境下尚未得到充分验证。
  score: 4.0
  article_ids:
  - dc3275f4556dcc3e
  evidence_snippets:
