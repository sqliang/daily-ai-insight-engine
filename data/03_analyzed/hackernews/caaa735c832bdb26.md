...
tldr: OpenAI 正式发布 GPT-5.6 系列模型，包含旗舰 Sol、均衡 Terra 和高效 Luna 三款，在编码、知识工作、网络安全和科学等多领域基准测试中取得领先成绩，并引入
  ultra 多智能体并行推理和 Programmatic Tool Calling 等新能力。
objective_summary: 2026年7月17日，OpenAI 宣布 GPT-5.6 系列模型全面上市。该系列包括旗舰模型 Sol、日常均衡模型 Terra
  和成本最优模型 Luna。Sol 在 Agents' Last Exam 上取得 53.6 分，在 Coding Agent Index 上以 80 分超越 Claude
  Fable 5，并支持 max 和 ultra 等多级推理设置。新版本还引入了 Programmatic Tool Calling 和 Responses API
  多智能体 beta 功能，在设计判断、知识工作和网络安全方面实现显著改进。
...
entities:
  ...
  key_people: []
key_logic_flow:
- OpenAI 正式发布 GPT-5.6 模型家族，包含旗舰模型 Sol、日常均衡模型 Terra 和高效低成本模型 Luna，面向不同使用场景全面上市。
- GPT-5.6 Sol 在 Agents' Last Exam 上取得 53.6 分，在 Coding Agent Index 达到 80 分，均超越 Anthropic
  的 Claude Fable 5，且推理成本显著更低。
- GPT-5.6 引入 ultra 多智能体并行推理模式，默认协调 4 个 agent 协同完成复杂任务，可在 BrowseComp、SEC-Bench Pro
  等测试中提升分数并缩短延迟。
- OpenAI 推出 Programmatic Tool Calling 功能，允许模型编写和运行轻量程序来自动协调工具调用过程，减少模型往返次数和 token
  消耗。
- GPT-5.6 在网络安全领域取得显著进步，ExploitBench 2 得分从 GPT-5.5 的 47.9% 提升至 73.5%，ExploitGym 峰值通过率提升至
  24.9%。
- GPT-5.6 在 OpenAI 内部被广泛采用，研究员日均输出 token 量达到 GPT-5.5 时期最高水平的两倍以上，内部编码推理所用研究算力增长 100
  倍。
...
- object_type: model
  ...
  evidence_snippets:
  - GPT-5.6 Sol 是 OpenAI 最新发布的旗舰模型，在 Agents' Last Exam 上以 53.6 分超越 Claude Fable 5
    达 13.1 分，是该基准测试的最高分。
  - GPT-5.6 Sol 在 Coding Agent Index 上以 80 分刷新纪录，比 Fable 5 高 2.8 分，同时输出 token 量不到后者一半，耗时少一半以上。
  - GPT-5.6 Sol 支持 ultra 模式，可默认协调 4 个智能体并行处理复杂任务，显著提升时间敏感型任务的效果并缩短延迟。
  article_id: caaa735c832bdb26
- object_type: model
  name: GPT-5.6 Terra
  ...
  evidence_snippets:
  - Terra 是 GPT-5.6 家族中面向日常工作的均衡模型，在多个基准测试中以约十六分之一于 Fable 5 的成本取得更好的表现。
  - Terra 在 Coding Agent Index 上得分略高于 Fable 5，但耗时仅为后者约三分之一，输出 token 量约一半，成本约四分之一。
  article_id: caaa735c832bdb26
- object_type: model
  name: GPT-5.6 Luna
  ...
  article_role: ecosystem_context
  evidence_snippets:
  - Luna 是 OpenAI 最具成本效益的模型，在 OSWorld 2.0 上几乎达到 GPT-5.5 峰值性能，但估算成本不到后者一半。
  - Luna 在 Coding Agent Index 上超越 Opus 4.8，耗时约为后者三分之一，输出 token 量约一半，成本约四分之一。
  article_id: caaa735c832bdb26
- object_type: product
  name: Responses API
  ...
  article_role: ecosystem_context
  evidence_snippets:
  - 开发人员可通过 Responses API 的多智能体 beta 功能构建类似 ultra 体验的多 agent 并行推理工作流。
  - Programmatic Tool Calling 作为 Responses API 中的新功能，可过滤大量中间数据并动态调整执行流程。
  article_id: caaa735c832bdb26
- object_type: product
  name: Programmatic Tool Calling
  canonical_name: OpenAI Programmatic Tool Calling
  url: https://developers.openai.com/api/docs/guides/tools-programmatic-tool-calling
  confidence: medium
  article_role: mentioned_reference
  evidence_snippets:
  - Programmatic Tool Calling 允许模型编写和运行轻量程序来协调工具调用、处理中间结果并自主选择下一步行动。
  - 该功能在工具密集型任务中可减少 token 消耗、模型往返次数和人工干预，无需开发者逐步骤编写脚本。
  article_id: caaa735c832bdb26
impact_score:
  score: 7.5
...
- object_type: product
  ...
  canonical_name: OpenAI Programmatic Tool Calling
  url: https://developers.openai.com/api/docs/guides/tools-programmatic-tool-calling
  positioning: OpenAI 推出的程序化工具调用功能，允许模型编写和运行轻量程序来自动协调工具调用流程，减少模型往返和 token 消耗。
  ...
  - AI Agent 框架构建者
  - 企业自动化工程师
  product_signal: 作为 GPT-5.6 在 Responses API 中的新功能，允许模型在工具密集型任务中自主编写程序协调调用，无需开发者逐步骤编写脚本。
  market_signal: 该功能降低了 AI Agent 在复杂工具链场景中的开发门槛，有望推动更多企业采用大模型驱动的自动化工作流。
  differentiation: 与传统逐步骤工具编排不同，该功能让模型自主编写程序来协调调用过程、筛选中间数据并调整工作流，大幅减少人工干预和 token 消耗。
  watch_reason: Programmatic Tool Calling 代表了一种全新的工具交互范式，让 AI 模型从被动执行预定义脚本转变为主动编写程序来协调工作流。如果被开发者广泛采用，可能显著改变
    AI Agent 与外部工具系统的交互方式，降低复杂自动化任务的实现和运维成本。
  risk_notes:
  - 作为 GPT-5.6 新推出的 API 功能，开发者实际采用效果和稳定性仍需时间验证，可能面临复杂生产环境下的可靠性挑战。
  - 目前该功能仅集成在 Responses API 中，开发者生态覆盖范围有限，推广至更多主流平台和框架尚需时间和社区反馈积累。
  score: 5.0
  article_ids:
  - caaa735c832bdb26
  evidence_snippets:
  - Programmatic Tool Calling 是 OpenAI 在 GPT-5.6 中推出的新功能，允许模型编写和运行轻量程序来协调工具调用、处理中间结果并自主选择下一步行动，在工具密集型任务中可减少
    token 消耗和模型往返次数。
  - 与传统逐步骤脚本编排不同，Programmatic Tool Calling 让模型自主编写程序、筛选中间数据并调整工作流，无需开发者逐步骤编写脚本或传递每次工具响应回模型。
---
