...
tldr: Apple和Google通过设备端AI模型对推送通知进行摘要、重排和改写，平台从传输管道演变为主动中介，发送方逐渐丧失对通知呈现的控制权。
objective_summary: 文章分析了Apple和Google在过去十五年间如何逐步将推送通知从简单的传输通道转变为主动中介平台。自2009年APNs推出以来，两大平台通过通知渠道、Focus模式、运行时权限等手段持续缩减发送方控制权。2024年起，Apple
  Intelligence和Gemini
...
entities:
  ...
  - Scott Forstall
key_logic_flow:
- 推送通知起源于2009年的电池问题——iPhone无法承受每个应用独立维持后台轮询，因此Apple推出APNs作为单一持久TLS连接的中继通道，Google随后推出C2DM/GCM/FCM形成双平台垄断格局。
- 2017年至2022年间，Android通过通知渠道（Android 8）和运行时权限（Android 13）将控制权从发送方转移到用户和平台，iOS则通过Focus模式、Scheduled
  Summary和四级中断分类（iOS 15）实现类似的控制权迁移，Android推送选择加入率从85%降至67%。
- 2024年起，设备端AI模型成为推送通知的新编辑层：Apple Intelligence运行30亿参数的设备端基础语言模型，通过LoRA适配器实现摘要、实体提取和优先级排序；Gemini
  Nano在Android AICore中运行，执行类似的通知摘要功能。
- 推送通知的中介化比电子邮件更不透明：邮件至少提供Postmaster Tools等可观测性工具和DKIM等认证标准，而推送几乎没有发送方可观测性——无法判断通知是否被摘要、被Focus模式隐藏或被设备端模型降权。
- Apple在BBC投诉AI摘要生成虚假标题后，于iOS 18.3中禁用了News和Entertainment类别的AI摘要，改为斜体渲染AI摘要并添加错误警告，同时提供每应用开关，但默认仍开启摘要功能。
- 推送通知缺乏电子邮件的开放协议优势：推送权限绑定在特定设备的特定安装上，令牌可被平台随时撤销，发送方不持有可迁移的用户列表；Web推送拓宽了发送渠道但通知仍落在同一托盘受同一设备端模型编辑。
impact_score:
  score: 5.5
