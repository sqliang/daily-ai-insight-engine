...
tldr: 安全研究员 mrbruh 发现 AMD AutoUpdate 软件中的远程代码执行漏洞（RCE），漏洞根源在于更新 XML 清单中可执行文件下载链接使用
  HTTP 而非 HTTPS，且程序未对下载文件进行签名验证。AMD 的漏洞奖励计划以 MITM 攻击超出范围为理由拒绝奖励，但经过 124 天后最终发布补丁。
objective_summary: 2026 年 1 月 27 日，安全研究员 mrbruh 在其新游戏 PC 上发现 AMD AutoUpdate 软件存在远程代码执行漏洞。该软件在
  app.config 中存储的更新 URL 虽使用 HTTPS，但所指向的 XML 清单文件中的可执行程序下载链接全部使用 HTTP，攻击者可通过中间人攻击替换恶意文件，且程序不进行任何签名验证就直接执行。研究员于
  2 月 6 日向 AMD 报告此漏洞，但被第三方漏洞平台 Intigriti 以 MITM 超出范围为由关闭。经 Hacker News 发酵后 AMD PSIRT
  团队重新介入，要求研究员删除博文并接受 124 天禁言期。最终 AMD 在 Ryzen Master 中将自动更新移至应用层并改用 HTTPS，但签名验证仅为
  CRC-32 校验，不具备密码学安全性。
...
entities:
  ...
  - mrbruh
key_logic_flow:
- 安全研究员 mrbruh 发现 AMD AutoUpdate 软件的更新 XML 清单中所有可执行文件下载链接均使用不加密的 HTTP 协议。
- AMD AutoUpdate 在下载后不进行任何证书验证或签名检查，直接执行下载的文件，使得局域网或 ISP 级别的中间人攻击者可以植入任意恶意代码。
- 研究员于 2026 年 2 月 6 日向 AMD 报告此漏洞，但被第三方漏洞平台 Intigriti 以涉及 MITM 攻击场景超出奖励范围为由关闭。
- 该漏洞报告在 Hacker News 发酵后，AMD PSIRT 团队在次日主动联系研究员，要求其删除公开博文并接受长于行业标准 90 天的禁言期。
- 经过 124 天的漫长等待后，AMD 的修复方案是将 Ryzen Master 的自动更新功能移至应用层并强制使用 HTTPS，但签名验证仅采用 CRC-32
  校验而非密码学签名。
- 此外，AMD AutoUpdate 程序自身因域名迁移（从 ati.com 到 drivers.amd.com）导致的 HTTP 重定向处理缺陷已经崩溃失效，形成了无法通过自更新修复漏洞的
  Catch-22 困境。
extract_result: success
object_mentions:
- object_type: product
  name: AMD AutoUpdate
  canonical_name: AMD AutoUpdate
  url: null
  confidence: high
  article_role: primary_subject
  evidence_snippets:
  - AMD AutoUpdate 软件的 app.config 中使用了指向 XML 清单的 HTTPS URL，但其中列出的可执行文件下载链接全部使用 HTTP
    协议。
  - AMD AutoUpdate 下载可执行文件后不进行任何签名验证或证书校验，直接执行下载的程序。
  - 由于 AMD 将软件包托管域名从 ati.com 迁移至 drivers.amd.com，AutoUpdate 程序无法处理 HTTP 重定向，导致崩溃而无法完成自更新。
  article_id: 44b83346de3bb024
- object_type: product
  name: Ryzen Master
  canonical_name: AMD Ryzen Master
  url: null
  confidence: high
  article_role: mentioned_reference
  evidence_snippets:
  - AMD 在修复方案中表示已将 Ryzen Master 的自动更新功能从安装程序移至应用层。
  - Ryzen Master 是受此 RCE 漏洞影响的多个 AMD 软件产品之一。
  article_id: 44b83346de3bb024
- object_type: company
  name: Intigriti
  canonical_name: Intigriti
  url: null
  confidence: high
  article_role: mentioned_reference
  evidence_snippets:
  - Intigriti 是 AMD 使用的第三方漏洞奖励平台，负责初步审核和分类安全报告。
  - Intigriti 将 mrbruh 提交的漏洞报告以 MITM 攻击超出范围为理由标记为 won't fix 并关闭。
  article_id: 44b83346de3bb024
impact_score:
  score: 6.5
  reason: 该漏洞本身属于经典 MITM 攻击类型，技术复杂度不高，但涉及 AMD 这一 AI 芯片关键供应商的官方自动更新软件，潜在影响面达数百万用户。更关键的冲击在于
    AMD 安全响应流程暴露的深层问题：124 天才给出不完整修复（仅 HTTPS 化 + CRC-32 而非加密签名）、漏洞赏金计划以 MITM 超出范围为由拒绝、利用保密期约束研究者却不支付赏金。对于依赖
    AMD GPU 进行 AI 训练的厂商而言，这一事件敲响了供应链软件安全的警钟。虽不构成行业范式转移，但将推动硬件厂商重新审视软件安全投入和漏洞响应流程。
sentiment: negative
developer_sentiment:
  tone: frustrated
  primary_focus: AMD 安全响应流程严重拖延（124 天）、修复方案敷衍（CRC-32 代替加密签名）、以及利用保密条款约束研究者却不给予合理补偿
hype_assessment:
  level: low
  reason: 文章是安全研究员的一手技术披露，包含完整的漏洞复现路径（反编译 .NET 程序集、XML 配置分析、HTTP 明文下载证据）、124 天完整时间线记录以及与
    AMD PSIRT 的原始邮件通信截图。内容详实可信，不存在任何概念包装或 PR 话术。
information_entropy: high
domain_disruption:
  technical_innovation: 无正面技术突破。漏洞本质是基础安全设计缺陷：AutoUpdate 通过 HTTP 明文传输可执行文件下载链接且无任何签名验证，属于行业已知的安全反模式。值得关注的是研究者通过反编译
    .NET 程序集进行漏洞取证的方法论，以及 AutoUpdater 自身重定向缺陷导致无法自我更新的 Catch-22 困局。
  business_model: 该事件冲击 AMD 作为 AI 基础设施供应商的品牌信任度，暴露了第三方漏洞赏金平台（Intigriti）与厂商内部安全团队（PSIRT）之间的流程断裂。可能推动硬件厂商重新评估漏洞赏金计划的范围定义（MITM
    攻击是否应纳入）以及安全修复的质量标准（CRC-32 不应替代加密签名）。
engineering_complexity: production_ready
compound_value:
  score: 2.0
  reason: 该事件是单一安全漏洞披露案例，不构成技术趋势或平台级机会。虽然事件暴露了 AMD 在软件安全流程上的系统性缺陷（124 天未完成 HTTP→HTTPS
    的简单修复、CRC-32 伪签名），可能轻微加速企业客户对硬件厂商软件供应链安全的审查意识，但缺乏长期复利效应和可规模化积累的资产。情绪冲击>实质性格局变化，3-5
    年后大概率被遗忘。
...
- 安全审计工具开发者可聚焦硬件厂商配套软件的更新机制审计，AMD、NVIDIA、Intel等芯片厂商的驱动程序自动更新组件普遍存在类似安全问题，是蓝海细分市场
- 针对企业级用户的软件供应链安全验证服务存在机会，帮助企业检测下游硬件/软件供应商的更新管道是否存在HTTP明文传输、签名缺失等基础安全缺陷
- 漏洞披露协调与争议仲裁平台存在创业窗口，为安全研究员与厂商之间提供中立的披露时间线管理和公关支持，解决类似AMD利用保密期拖延修复的问题
risk_matrix:
  regulatory: 该事件可能触发各国对硬件厂商软件附随产品的监管审查，特别是欧盟《网络弹性法案》(Cyber Resilience Act)要求数字产品必须具有安全的更新机制，AMD的HTTP明文下发+CRC-32签名方案可能不符合合规要求
  technological: AMD最终仅将HTTP改为HTTPS并用CRC-32替代加密签名，CRC-32作为完整性校验在对抗场景下完全无效（可被轻易碰撞），技术修复不彻底，未来可能被更深入利用
  competitive: 竞争对手（Intel、NVIDIA）可能利用该事件在政企采购场景中加强自身安全口碑宣传，间接挤压AMD在数据中心和企业级市场的安全信任度
  ethical: AMD在知情后拖延124天修复已知RCE漏洞，期间数百万用户持续暴露在可被利用的风险中，且修复方案（CRC-32）流于形式而非真正保障用户安全，损害用户信任
  additional:
  - Catch-22困局：AutoUpdater本身存在重定向缺陷导致程序崩溃，用户无法接收修复更新，形成'先修更新程序再修漏洞'的循环依赖，暴露了AMD软件工程质量管控的系统性问题
  - 漏洞赏金计划的公信力风险：AMD以MITM攻击超出范围为理由拒绝赏金，但该漏洞的实际利用场景（公共WiFi、ISP级攻击）不可忽视，此先例可能降低安全社区向AMD报告漏洞的意愿
confidence:
  impact: high
  compound: medium
  hype: medium
...
- object_type: product
  ...
  canonical_name: AMD AutoUpdate
  url: null
  positioning: AMD AutoUpdate 是 AMD 官方驱动与工具软件中负责自动下载更新的系统组件，通过 XML 清单文件分发可执行程序。
  ...
  - AMD 驱动与工具软件用户
  - Ryzen Master 用户
  product_signal: 更新 XML 清单中可执行文件下载链接全部使用 HTTP 而非 HTTPS，且下载后不进行任何签名验证直接执行。
  market_signal: AMD 依赖的第三方漏洞平台 Intigriti 以 MITM 超出范围为由关闭报告，124 天后才在 Hacker News 发酵后推动修复。
  differentiation: 与行业标准的安全更新机制相比，AMD 未对更新文件使用 HTTPS 传输也未实施代码签名验证，修复后仅采用 CRC-32 校验。
  watch_reason: AMD AutoUpdate 的 RCE 漏洞暴露了 AMD 在软件供应链安全上的系统性缺陷——从 HTTP 更新通道到缺失签名校验再到漏洞奖励机制失效，修复耗时
    124 天且仅采用 CRC-32 校验，反映了硬件巨头在软件安全实践上的严重滞后。
  risk_notes:
  - 修复方案仅采用 CRC-32 校验而非密码学签名，不具备实质性安全防护能力。
  - AutoUpdate 程序自身因域名迁移导致的 HTTP 重定向崩溃，形成无法自更新的 Catch-22 困境。
  - AMD 漏洞奖励机制以 MITM 场景超出范围为理由拒绝奖励，可能抑制安全研究者的披露意愿。
  score: 5.0
  article_ids:
  - 44b83346de3bb024
  evidence_snippets:
...
- object_type: product
  ...
  canonical_name: AMD Ryzen Master
  url: null
  positioning: AMD Ryzen Master 是 AMD 官方提供的 Ryzen 处理器超频与性能监控工具，是受 AutoUpdate RCE 漏洞影响的软件之一。
  technical_signal: null
  adoption_signal: null
  ecosystem_relevance: null
  target_users:
  - Ryzen 系列 CPU 用户
  - 超频与性能调优爱好者
  product_signal: 作为受 AMD AutoUpdate RCE 漏洞影响的多个 AMD 软件之一，其修复方案将自动更新功能从安装程序移至应用层并强制使用
    HTTPS。
  market_signal: null
  differentiation: null
  watch_reason: Ryzen Master 是受 AMD AutoUpdate RCE 漏洞影响的多个软件之一，其修复方案将自动更新从安装程序移至应用层并改用
    HTTPS，可作为观测 AMD 全产品线安全修复覆盖面的窗口。
  risk_notes:
  - 该产品的安全修复依赖于整个 AutoUpdate 组件的架构重构，无法独立修复原有漏洞。
  - AMD 耗时 124 天才完成修复，暴露了其内部安全响应流程的效率问题。
  score: 4.0
  article_ids:
  - 44b83346de3bb024
  evidence_snippets:
  - AMD 在修复方案中表示已将 Ryzen Master 的自动更新功能从安装程序移至应用层。
  - Ryzen Master 是受此 RCE 漏洞影响的多个 AMD 软件产品之一，该漏洞修复共耗时 124 天。
---
