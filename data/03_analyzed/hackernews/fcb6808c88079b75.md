...
tldr: Zig 将包管理功能从编译器移入构建系统，SPIR-V 后端大改，LLVM 后端整数降级重构
objective_summary: 2026-06-30，Zig 官方 devlog 发布三项核心变更：Andrew Kelley 将包管理逻辑（HTTP、TLS、Git
  协议等）从编译器移至构建系统，编译器体积缩小 4%；Ali Cheraghi 重写 SPIR-V 后端，新增 @SpirvType 并支持多线程代码生成；
...
entities:
  ...
  - Jacob Young
key_logic_flow:
- Andrew Kelley 将包管理功能（包获取、HTTP 客户端、TLS、Git 协议、压缩算法、build.zig.zon 解析）从编译器可执行文件移入 maker（构建系统）进程，使编译器体积从
  14.1 MiB 缩小至 13.5 MiB（-4%）。
- Zig 的进程模型从 'zig build → builder' 变为 'zig build → maker（构建系统+包管理器）→ configurer（用户
  build.zig 逻辑）'，使构建服务器在配置变更时无需退出重启。
- Ali Cheraghi 为 SPIR-V 后端新增 @SpirvType 内建函数以表达 Zig 类型系统无法直接描述的类型，执行模式和能力/扩展声明改为调用约定和
  CPU 特性驱动，不再依赖内联汇编。
- SPIR-V 后端实现多线程代码生成（Mir 调度）、对象文件链接（.spv 文件），行为测试通过率提升约 10% 至 49%，std.gpu 更名为 std.spirv。
- Matthew Lugg 重构 LLVM 后端整数降级：任意位宽整数在 SSA 中使用 LLVM 位整数类型，在内存中零/符号扩展为 ABI 尺寸类型，以规避
  LLVM 对位整数类型的优化不足和误编译问题。
- '@bitCast 内建函数采用 Jacob Young（2024）提案的新语义，由 Legalize 阶段将复杂 @bitCast 转换为简单操作，已在 LLVM、C
  后端和 comptime 执行中统一实现。'
