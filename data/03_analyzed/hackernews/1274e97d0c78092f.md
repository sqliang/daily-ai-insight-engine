...
- object_type: project
  ...
  canonical_name: iroh
  url: https://www.iroh.computer
  positioning: Iroh 是一个基于密钥寻址替代 IP 地址的开源 P2P 网络通信库，提供 NAT 穿越、多路径 QUIC 传输与端到端加密直连能力。
  technical_signal: 使用 IETF 开放标准构建，自研 QUIC 多路径传输与 NAT 穿越实现，支持 WASM 编译和自定义传输协议（BLE、LoRa、Tor
    等）。
  adoption_signal: 公共中继在 30 天内处理超 2 亿个端点创建请求，已被用于视频流、大模型训练、Agent 通信和文件传输等场景。
  ecosystem_relevance: 支持 Rust、Python、Node.js、Swift 和 Kotlin 五种语言 FFI，可直接嵌入 iOS 和
    Android 应用，推动去中心化网络生态发展。
  target_users: []
  product_signal: null
  market_signal: null
  differentiation: null
  watch_reason: Iroh 重新定义了互联网设备寻址方式，1.0 稳定版标志着基于密钥的网络通信进入生产可用阶段，其多语言支持和去中心化理念在 AI
    Agent 通信和边缘计算场景具有长期跟踪价值。
  risk_notes:
  - 基于密钥寻址的 P2P 网络对传统中心化架构构成挑战，企业级大规模采用和市场接受度尚需验证。
  - 公共中继服务依赖持续运营投入，免费服务的速率限制和商业模式的长期可持续性存在不确定性。
  - 作为底层通信基础设施，其安全性和稳定性需在更大规模的生产环境中经受长期考验。
  score: 7.0
  article_ids:
  - 1274e97d0c78092f
  evidence_snippets:
