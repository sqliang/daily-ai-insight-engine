...
tldr: LibrePods 逆向实现 AirPods 私有协议，让非 Apple 平台使用全部功能
objective_summary: LibrePods 通过逆向工程实现 AirPods 与 Apple 设备间的私有通信协议（AACP/ATT），让 Linux
  和 Android 平台用户可使用切换降噪模式、入耳检测、头部手势等原本 Apple 独占的功能。项目完全开源，采用 GPLv3 协议。
...
entities:
  ...
  - Kavish
key_logic_flow:
- LibrePods 逆向实现了 AirPods 与 Apple 设备间的专有协议（AACP、ATT），使 Linux 和 Android 平台可调用切换降噪模式、入耳检测、电池状态、头部手势和通话感知等原本
  Apple 独占的功能
- 项目分为 Android 和 Linux 两套实现：Linux 通过修改蓝牙 DeviceID 为 Apple 的 VendorID 来解锁特殊功能；Android
  端需要 Xposed 框架支持才能启用 VendorID 欺骗
- 头部手势功能、部分协议代码（aacp.rs、att.rs 的 Kotlin 到 Rust 翻译）以及 Troubleshooter 工具由 AI 辅助生成，其余核心代码为手动编写
- 部分高级功能如 Find My 网络、高质量双向音频、头部追踪空间音频尚在规划或无法实现，立体声空间化超出项目范围
- 项目遵循 GPLv3 协议开源，与 Apple 公司无任何关联，AirPods 商标归 Apple 所有
