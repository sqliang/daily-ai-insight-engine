---
title: Every new iOS 27 feature that’s worth knowing about
source: https://techcrunch.com/2026/06/20/every-new-ios-27-feature-thats-worth-knowing-about/
author:
- '[[Lauren Forristal]]'
published: '2026-06-20'
created: '2026-06-21'
description: While it's not flashy like Apple’s new Siri AI and Apple Intelligence
  upgrades, there are still a number of additions to iOS 27 worth looking at.
tags:
- clippings
extraction_status: success
pipeline_stage: fact_extracted
id: 039c4efbe8623f97
source_type: news_media
tldr: 苹果在 WWDC 上发布 iOS 27，带来地图、查找、钱包和支付等多项功能更新
objective_summary: 苹果在 2026 年 6 月的 WWDC 上发布了 iOS 27，涵盖 Apple Maps 的 Flyover 增强与 Local
  Lists、Find My 的自定义时长位置共享、Apple Wallet 的智能账单分拆与会员卡数字化、Apple Pay 的结账体验重新设计等功能更新，
event_type: application_landing
epistemic_status: verified_fact
entities:
  companies:
  - Apple
  technologies:
  - Apple Intelligence
  - Apple Cash
  - Apple Pay
  key_people: []
key_logic_flow:
- Apple Maps 升级了 Flyover 功能，提供更精细的 3D 城市地标视觉效果和更流畅的导航体验，并新增 Local Lists 功能让用户发现和收藏热门餐厅、景点推荐。
- Find My 将在今年秋季支持自定义时长位置共享（从几分钟到特定日期时间），并新增对单个联系人暂停位置共享至当天结束的功能。
- Apple Wallet 新增基于 Apple Intelligence 的智能账单分拆功能，用户扫描收据后系统自动识别物品、计算每人应摊税费和小费，并通过 Messages
  或 Wallet 完成还款。
- Apple Wallet 支持扫描条码或数字卡以数字化保存会员卡和实体会员卡，并扩展至 Apple Watch，可将通行证固定在 Smart Stack 中。
- 酒店钥匙体验升级，参与酒店的用户可在 Wallet 中查看行程详情、活动安排、服务信息和实时更新。
- Apple Pay 重新设计了网页和应用内结账界面，支持更便捷地切换支付卡并查看奖励余额、借记账户余额及分期选项，今年晚些时候将支持直接向符合条件的借记卡充值。
impact_score:
  score: 2.5
  reason: iOS 27 是苹果年度常规 OS 更新，Apple Intelligence 驱动的收据扫描和账单分拆属于成熟的 OCR+语义解析组合方案，未涉及模型架构或训练方法创新。对整个
    AI 行业格局无实质冲击，属于 Apple 生态内的日常功能迭代，非范式级发布。
sentiment: neutral
developer_sentiment:
  tone: neutral
  primary_focus: Apple Intelligence 在消费者场景的落地深度有限，账单分拆等功能的 AI 门槛低，且未开放给第三方开发者
hype_assessment:
  level: low
  reason: TechCrunch 的报道以事实性功能列举为主，未出现'颠覆''革命性''突破'等 PR 滥用词汇，语言克制且附带了具体使用场景说明，符合科技媒体产品报道的正常风格，无过度包装迹象
information_entropy: medium
domain_disruption:
  technical_innovation: 无。收据识别与智能分账是 OCR + NLP 语义解析的成熟技术组合，Apple Intelligence 在此场景中仅作为品牌标签，并无底层模型能力的突破性进展
  business_model: 通过 Wallet 生态的粘性增强（会员卡数字化、酒店钥匙、智能分账）进一步将用户锁定在 Apple 金融支付闭环中，强化其作为数字钱包平台的中介地位，削弱第三方支付和记账应用的生存空间
engineering_complexity: production_ready
compound_value:
  score: 5.0
  reason: iOS 27 的功能更新本质上是 Apple 现有生态系统的防御性深挖，而非开辟新赛道。从 VC 视角看，这些更新（智能账单分拆、会员卡数字化、酒店钥匙、Local
    Lists）的核心逻辑是：利用 OS 级权限和 Apple Intelligence 将用户更牢固地锁定在 Apple 服务闭环内。长期复利价值中等——每项功能单独看都是增量改进，但组合起来显著提升了
    Apple Wallet 作为金融服务入口和 Maps 作为本地生活入口的战略纵深。真正的复利效应在于 Apple Cash 的交易频次提升和 Apple
    Pay 对支付流程的控制加强，但这需要时间验证用户习惯迁移速度。不构成独立投资机会（Apple 是非交易标的），但对于持有 Apple 生态相关资产（如第三方支付、地图数据商）的投资人来说，这是重要的竞争信号。
value_capture_layer: end_application
moat_impact: strengthens_monopoly
key_beneficiaries:
- Apple
- Apple Cash
competitive_casualty:
- Venmo
- Splitwise
- Google Maps
- Yelp
- 独立酒店 App
- CardFlex/Storable (第三方会员卡管理)
market_opportunities:
- 第三方开发者可针对 Apple Intelligence 账单分拆的 iOS 独占特性，构建跨平台（Android/Web）的 AI 收据扫描与费用分摊应用，填补非苹果生态用户的需求缺口
- 本地生活服务平台和商户应重视 Apple Maps Local Lists 的 AI 推荐流量入口，积极优化在 Apple Maps 上的商户信息与评价数据，抢占本地生活场景的智能分发红利
- 酒店与旅游业 SaaS 服务商可开发与 Apple Wallet 深度集成的数字化行程管理方案，通过钱包端的行程聚合提升住客黏性和服务触点密度
risk_matrix:
  regulatory: Apple Pay 和 Apple Cash 的智能分账功能涉及跨境支付结算和消费者金融保护法规；AI 扫描收据处理用户消费图像数据，需符合
    GDPR/CCPA 等隐私法规对生物特征信息和消费数据的采集与存储要求
  technological: Apple Intelligence 功能仅限 iOS 生态闭环内使用，Android/Web 跨平台方案仍有差异化竞争空间；第三方分账和会员卡管理应用面临被系统级功能直接替代的技术挤出风险
  competitive: Apple 在 AI 支付（挤压 Venmo/Google Pay）、AI 本地发现（抗衡 Google Maps/Yelp）、AI
    旅行服务（挑战 TripIt/Google Travel）多个赛道同时发起生态挤压，独立厂商需加速差异化创新以抵御降维打击
  ethical: AI 自动识别收据内容涉及用户消费行为和偏好的深度挖掘，隐私边界模糊；Find My 精细时长位置共享功能存在被滥用为恶意跟踪工具的社会风险；Apple
    Intelligence 的自动分摊算法可能存在计算偏见或公平性缺陷
  additional: []
confidence:
  impact: high
  compound: medium
  hype: medium
actionable_insight: monitor
---

While Apple’s new Siri AI and Apple Intelligence announcements dominated the WWDC spotlight earlier this month, the tech giant also packed iOS 27 with a number of upgrades across its everyday apps and services, including smarter bill splitting in Apple Wallet, new ways to share locations in Find My, and improved Apple Maps features.

## Apple Maps

Apple introduced an improvement to “Flyover,” the feature that provides immersive 3D views of cities and landmarks. The feature has been refreshed with more detailed visuals and smoother navigation.

The company also launched “Local Lists,” a new way for users to discover and save recommendations for restaurants, attractions, and other spots within Maps by seeing what’s trending. This could be Apple’s way to entice users away from using third-party apps for local exploration, like Google Maps or even Instagram and TikTok.

## Find My

Find My is becoming more flexible with new location-sharing controls. Beginning this fall, users will be able to share their location for a custom amount of time, whether that’s a few minutes, several hours, multiple days, or until a specific date and time.

Apple is also adding the ability to temporarily pause location sharing with individual contacts until the end of the day. The changes are designed for various situations, like keeping a surprise birthday party under wraps.

## Apple Wallet

Apple Wallet is getting a significant boost. One of the standout additions allows users to scan receipts using their iPhone camera and automatically split bills with friends using Apple Cash. Powered by Apple Intelligence, the feature can identify individual items on a receipt, calculate each person’s share of taxes and tips, and facilitate repayment directly through Messages or Wallet.

Wallet is also gaining support for digitizing physical membership and loyalty cards. Users can simply point their iPhone camera at a barcode or scan a digital card to save it directly into Wallet. Support is extending to Apple Watch, so passes can even be pinned to the Apple Watch Smart Stack for faster retrieval.

Travelers will benefit from a revamped hotel key experience. In addition to unlocking rooms and amenities with an iPhone or Apple Watch, guests staying at participating hotels and resorts will be able to access trip details, activity schedules, service information, and real-time updates directly within Wallet.

## Apple Pay

Apple Pay is receiving a redesigned checkout experience online and in apps. Users will be able to swipe between payment cards more easily while viewing information such as rewards balances, debit account balances, and available pay-later options. Later this year, Apple says users will also be able to add funds directly to eligible debit cards through Wallet or during Apple Pay checkout.