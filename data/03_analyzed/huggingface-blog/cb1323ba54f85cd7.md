---
title: 'OlmoEarth v1.1: A more efficient family of Earth observation models'
source: https://huggingface.co/blog/allenai/olmoearth-v1-1
author: []
published: '2026-05-19'
created: '2026-05-21'
description: ''
tags:
- clippings
extraction_status: success
id: cb1323ba54f85cd7
source_type: tech_blog
tldr: Allen AI 发布 OlmoEarth v1.1 地球观测模型系列，推理计算成本降低最多3倍，性能与v1持平。
objective_summary: Allen AI 在 OlmoEarth v1（2025年11月发布）基础上推出 v1.1 模型系列，通过优化 token 序列长度和模型规模两个杠杆，将推理计算成本降低至原来的三分之一，同时保持在研究基准和合作伙伴实际任务上的性能不变。模型基于
  Transformer 架构，处理
event_type: application_landing
epistemic_status: pr_statement
entities:
  companies:
  - Allen AI
  - Allen Institute for AI
  technologies:
  - OlmoEarth
  - Transformer
  - Sentinel-2
  - remote sensing
  - Earth observation
  key_people: []
key_logic_flow:
- OlmoEarth v1 于2025年11月发布，此后被合作伙伴广泛应用于红树林变化追踪、森林丧失驱动因素分类、国家级作物类型制图等任务
- 处理卫星影像时，计算成本是完整生命周期中最高的环节，推理效率直接决定可支持的合作伙伴规模和部署速度
- Transformer 模型的效率由两个关键杠杆控制：模型规模（因此发布多尺寸系列）和 token 序列长度（计算成本与序列长度呈二次方关系）
- OlmoEarth v1.1 的核心优化在于重新设计 token 表示方式——将 Sentinel-2 影像按空间分块，每块在每个时间步和每个分辨率层级生成对应
  token
- v1.1 在保持与 v1 相同性能的前提下，将计算成本降低最多 3 倍，使更多合作伙伴能在 OlmoEarth 平台上以更低成本运行模型
pipeline_stage: fact_extracted
impact_score:
  score: 3.5
  reason: OlmoEarth v1.1 是对 v1 的增量优化版本，核心卖点是将推理计算成本降低 3 倍且性能持平。这一改进对地球观测/遥感领域的实际部署具有显著经济意义——可降低合作伙伴的运行成本、扩大平台覆盖范围——但本质上属于工程优化而非架构范式转移。v1→v1.1
    的版本号本身也表明这是迭代而非革命。该发布影响范围集中在遥感 AI 这一垂直领域，对更广泛的 AI 行业冲击力有限，故评为 3.5 分（日常更新之上、局部格局改变之下）。
sentiment: positive
developer_sentiment:
  tone: neutral
  primary_focus: 推理计算成本降低 3 倍的 token 表示方式重新设计是否可复用到其他遥感模型
hype_assessment:
  level: low
  reason: 博文措辞克制，未出现'颠覆''革命性'等 PR 滥用词汇。核心声称（3 倍成本降低、性能持平）均有明确技术解释支撑——从 token 序列长度与计算量的二次方关系出发，解释了按空间分块、按时间步和分辨率层级生成
    token 的优化逻辑，并用 MAC 指标量化效率。属于实打实的技术沟通而非炒作。
information_entropy: medium
domain_disruption:
  technical_innovation: 重新设计 Sentinel-2 卫星影像的 token 表示方式：将影像按空间分块（patch size p），每个分块在每个时间步和每个分辨率层级（10m/20m/60m）生成对应
    token，通过控制序列长度来利用 Transformer 计算量与序列长度的二次方关系，从而在保持模型规模不变的前提下降低计算成本。属于精巧的工程优化而非根本性的架构突破。
  business_model: 推理成本降低 3 倍直接改变了 OlmoEarth 平台的经济账：更多中小型环保组织、社区团体和发展中国家机构能够在有限预算内使用先进的
    AI 遥感分析能力，推动该平台从服务于资金充裕的合作伙伴向更广泛的公益场景下沉，加速'AI for Earth'使命的规模化落地。
engineering_complexity: production_ready
compound_value:
  score: 6.0
  reason: 地球观测 AI 是确定性长赛道（气候、农业、ESG、国防），OlmoEarth 通过 v1→v1.1 展示了两项关键能力：① 工程方法论清晰，精准锁定
    token 序列长度这一核心成本杠杆，优化路径可复用迭代；② 已有真实合作伙伴验证（红树林追踪、森林丧失分类、国家级作物制图），证明产品-市场匹配。开源策略短期牺牲直接变现，但长期通过生态锁定形成'地球观测领域的
    Llama'——成为默认开源标准后，平台化变现（OlmoEarth Platform）和衍生商业实体均有想象空间。扣分项在于 Allen AI 作为非营利研究机构的商业化路径尚不明确，且遥感
    AI 赛道天花板低于通用 AI。综合评估：3-5 年内大概率仍是该细分赛道的核心基础设施，但需持续关注商业化进展。
value_capture_layer: foundation_model
moat_impact: democratizes_access
key_beneficiaries:
- Allen AI
- Planet Labs
- AWS
- Google Earth Engine
- NVIDIA
competitive_casualty:
- Descartes Labs
- Orbital Insight
- 小型闭源遥感 AI 初创公司
- 传统 GIS 分析服务商
market_opportunities:
- 基于 OlmoEarth v1.1 的 3 倍推理成本降低，地理信息企业和环保 NGO 可在红树林监测、森林退化分类、国家级作物制图等场景中，以更低算力预算部署遥感
  AI 模型，推动垂直行业解决方案的商业化落地。
- OlmoEarth 采用的「空间分块 + 多分辨率层级 token 化」效率优化策略可迁移至其他遥感模型（如气象预测、灾害评估），模型开发者可借鉴该思路优化自有架构，降低卫星影像推理的二次方计算开销。
- 随着 OlmoEarth 合作伙伴生态从科研向工程落地扩展，围绕模型部署、数据预处理流水线、推理优化服务的工具链和 MLOps 基础设施存在创业机会，尤其是面向发展中国家的低成本遥感
  AI 服务平台。
risk_matrix:
  regulatory: 卫星遥感数据涉及跨境地理信息合规问题，不同国家对高分辨率地理数据的采集、处理和分发有差异化管制（如测绘法、数据本地化要求），OlmoEarth
    的全球化部署需关注各国地理信息法规的适配成本。
  technological: 模型强依赖 Sentinel-2 单一数据源（12 通道、10m/20m/60m 分辨率），若欧洲航天局调整数据开放政策或新一代卫星传感器规格发生重大变化，当前
    token 化方案可能需要重新设计，存在架构锁定风险。
  competitive: Google Earth Engine、Microsoft Planetary Computer 等平台已构建「数据+算力+模型」一体化生态，具备强大的渠道锁定效应；OlmoEarth
    虽开源但在算力基础设施和开发者生态上处于劣势，可能面临价格战和生态挤压。
  ethical: 遥感 AI 模型具有双重用途潜力——虽以环境保护为初衷，但同类型技术也可用于军事侦察、跨境监控等敏感场景；此外，对发展中国家地理信息的规模化分析可能引发数据主权争议和当地社区的知情同意问题。
  additional:
  - 开源模型被商业云平台封装为付费服务后，原始开发方（Allen AI）可能面临品牌声誉风险，但缺乏对下游滥用的约束手段
  - 地球观测 AI 的训练数据和基准可能隐含地理偏见——主要标注数据来自欧美地区，在非洲、东南亚等生态脆弱区的泛化能力未经充分验证
confidence:
  impact: medium
  compound: medium
  hype: low
actionable_insight: monitor
---

We released OlmoEarth (v1) in November 2025. Since then, partners have applied it across a wide range of tasks, from tracking mangrove change to classifying drivers of forest loss to producing country-scale crop-type maps in days, scaling deployments to national, continental, and global areas. Every release moves us closer to our mission: bringing state-of-the-art AI to organizations and communities working to protect people and our planet.

When OlmoEarth processes satellite imagery to make predictions across tens to hundreds of thousands of square kilometers, efficiency shapes what’s possible. Over the full lifecycle of running OlmoEarth – data export, preprocessing, inference, and post-processing – compute is by far the highest cost. A more efficient model means we can support more partners on the OlmoEarth Platform, and that anyone running OlmoEarth on their own can leverage this technology faster and at lower expense.

That’s why we built **OlmoEarth v1.1**: a new family of models that cuts compute costs by up to **3x** while maintaining OlmoEarth v1's performance on a mix of research benchmarks and tasks we’ve constructed with partners.

The OlmoEarth models are transformer-based models, one of the dominant architectures in machine learning today. To process remote sensing data, we first convert it into a sequence of *tokens* the model can ingest.

Two important levers control efficiency in transformer-based models: **model size** (this is why we release a family of models, so users can pick the size that fits their compute budget) and **token sequence length**. Compute costs scale quadratically with the token sequence length, so even small reductions can meaningfully cut the cost of running the model.

*MACs, or multiply-accumulate operations, estimate the computation needed for one model forward pass; lower MACs generally mean cheaper, faster inference. The y-axis is inverted because lower average rank is better. Labels show model family and size. All plotted points use the pasted MAC/rank values.*

This raises an important question for transformer-based remote sensing models: **what should a token represent?**

Take Sentinel-2 imagery, a common modality we process. A Sentinel-2 input will be some tensor with a height and width (H, W representing the latitudinal and longitudinal pixels), a temporal dimension T, and 12 Sentinel-2 channels ([H, W, T, D=12]).

Currently, we split the data into *resolution-based patches.* Concretely, this means that we will pick some spatial patch size p, and split our overall Sentinel-2 image into patches of size p x p:

For each patch, we create a token per timestep per resolution. So a Sentinel-2 input with 2 timesteps yields 6 tokens per patch (2 timesteps x 3 resolutions, 10m, 20m, and 60m).

In total, a[H, W, T, D=12] Sentinel-2 input will yield H/p x W/p x T x 3 tokens.