...
14. **specializedBrief.githubHighlights**: ONLY when there are github-trending articles in the input. Use the GitHub statistics provided in the user prompt. Do NOT fabricate project names or domain distributions. `articleCount` must match the actual number of deduplicated GitHub articles provided. `topProjects` should list 3-5 notable project names (owner/repo format preferred). `summary` must be one sentence in Chinese.
15. **specializedBrief.productHighlights**: ONLY when there are product articles (source_dir == "producthunt" or "whytryai") in the input. Use the product statistics provided in the user prompt. Do NOT fabricate product names. `articleCount` must match the actual number of deduplicated product articles provided. `notableProducts` should list 3-5 notable product names. `summary` must be one sentence in Chinese.
...
17. **Paper brief**: Do NOT output paperHighlights in this version. The paper specialized page currently reads arxiv-cs-ai articles directly from all_articles.json; a synthesized paperHighlights block may be added in a future iteration.
"""
