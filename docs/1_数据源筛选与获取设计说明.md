...
- name: "arxiv-cs-ai"          # 唯一标识，对应对应 parser 注册表中的 key
  ...
  enabled: true                 # 是否启用，false 时 scout.py 自动跳过
  description: "..."            # 人类可读的描述
  url: "https://..."            # 数据入口 URL
...
**解析器注册表的设计优势**：
- `scout.py` 不感知具体解析逻辑，只需按 `source.name` 查找对应的 parser 函数并调用
...
| **Scout 编排器** | `pipeline/ingestion/scout.py` | 遍历启用的数据源，按 `fetch_strategy` 分发抓取，调用过滤器，生成 manifest JSON |
